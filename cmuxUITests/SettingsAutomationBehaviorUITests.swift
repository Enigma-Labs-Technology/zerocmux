...
///     not exist in the default `zerocmux processes only` mode, and choosing a
...
final class SettingsAutomationBehaviorUITests: SettingsUITestCase {
    ...
    private enum L {
        ...
        static let modeCmuxOnly = "zerocmux processes only"
    ...
    /// Default mode is `zerocmux processes only`, so the Socket Password row must
    /// be absent. Selecting `Password mode` must reveal it (title + the
    /// "no password set" subtitle), and selecting `zerocmux processes only`
    ...
    func testPasswordModeRevealsAndHidesSocketPasswordRow() {
        ...
        XCTAssertFalse(
            ...
            "Socket Password row should be hidden in the default zerocmux-only mode"
        )
        ...
        // Switch back to zerocmux-only → row disappears again.
        XCTAssertTrue(selectMode(window, app, L.modeCmuxOnly), "could not select zerocmux-only mode")
        XCTAssertTrue(
            poll(timeout: 4.0) { !passwordTitle.exists },
    ...
    //   toggles — when enabled, zerocmux injects session-tracking + notification
    //   hook env into newly launched `claude`/`cursor`/`gemini` processes
    //   inside zerocmux terminals. The observable consequence (sidebar session
    ...
    //   these select which `claude` / `rg` executable zerocmux resolves
