...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        XCTAssertTrue(
            ...
            "Expected zerocmux to be foreground before sending notification shortcut. state=\(app.state.rawValue)"
        )
    ...
    func testNotificationsPopoverCanCloseViaShortcutAndEscape() {
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected zerocmux to be foreground before opening notifications popover. state=\(app.state.rawValue)"
        )
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected zerocmux to be foreground before opening empty notifications popover. state=\(app.state.rawValue)"
        )
        app.typeKey("i", modifierFlags: [.command])
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        XCTAssertTrue(
            ensureAppForegroundForInteraction(app, timeout: 6.0),
            "Expected zerocmux to be foreground before typing delayed notify command. state=\(app.state.rawValue)"
        )
        ...
        // bundled `zerocmux notify` command has finished while the app stayed
    ...
    func testNewWorkspaceCLIWindowFlagTargetsSpecificWindow() throws {
        ...
        XCTAssertEqual(
            ...
            "1",
            "Expected bundled `zerocmux --window` route test hook to finish. failure=\(setup["windowRouteFailure"] ?? "")"
        )
    ...
    private func stableSocketPath() -> String {
        // Mirrors CmuxStateDirectory.url() + zerocmux.sock (non-TCC ~/.local/state/zerocmux;
        // see https://github.com/kernelalex/zerocmux/issues/5146).
