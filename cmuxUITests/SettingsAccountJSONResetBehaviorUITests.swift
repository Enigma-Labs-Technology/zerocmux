...
/// control whose effect is observable purely inside the zerocmux app surface
...
///   external editor — leaves the zerocmux process, no in-app element).
...
final class SettingsAccountJSONResetBehaviorUITests: SettingsUITestCase {
    ...
    /// (TIER 3 below), which XCUITest cannot observe inside zerocmux.
    ...
    /// Presence guard only — both controls leave the zerocmux process when
    ...
    //   application. zerocmux opens no in-app window and changes no in-app element,
    //   so XCUITest (scoped to the zerocmux process) cannot observe the effect.
    ...
    //   zerocmux sign-in page in the system browser for an OAuth-style round trip;
    ...
    //   — a SwiftUI `Link` to https://cmux.com/docs/configuration#zerocmux-json that
    //   opens in the system browser. Cross-app navigation with no zerocmux-side
