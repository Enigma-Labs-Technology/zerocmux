...
final class SettingsBrowserBehaviorUITests: SettingsUITestCase {
    ...
    // TIER 2 (needs runtime seam): Enable zerocmux Browser (BrowserEnabledToggle) —
    ...
    //   mirrors both keys). So toggling Enable zerocmux Browser in the new
    //   Settings UI produces no XCUITest-observable change (the command
    //   palette still shows "Disable zerocmux Browser"). Verifying this
    ...
    //   zerocmux without a warning prompt. The effect (warning prompt shown or
