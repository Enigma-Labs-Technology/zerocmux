...
export function resolveDiffNavigationURL(rawURL: string): string {
  ...
  try {
    ...
    if (
      window.location.protocol === "zerocmux-diff-viewer:" &&
      (target.protocol === "http:" || target.protocol === "https:")
    ) {
      const rest = target.pathname.split("/").filter(Boolean).slice(1).join("/");
      return `zerocmux-diff-viewer://${window.location.host}/${rest}`;
    }
    return target.href;
  } catch {
    return rawURL;
...
// `http://`, `zerocmux-diff-viewer://`, `data:`). A root-relative path (`/foo?x`)
