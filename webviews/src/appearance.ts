...
export function resolveDiffViewerAppearance(appearance?: DiffViewerAppearance): ResolvedDiffViewerAppearance {
  ...
  return {
    ...
    theme: {
      light: appearance?.theme?.light ?? lightTheme.name ?? "zerocmux-ghostty-light",
      dark: appearance?.theme?.dark ?? darkTheme.name ?? "zerocmux-ghostty-dark",
    },
    themes: {
      light: lightTheme,
      dark: darkTheme,
...
export function applyDiffViewerAppearance(appearance?: DiffViewerAppearance) {
  ...
  // `--zerocmux-diff-bg` stays opaque: it is the base color the page blends against
  // for text, borders, and floating overlays (menus).
  rootStyle.setProperty("--zerocmux-diff-bg-light", colorString(lightTheme.background, "#ffffff"));
  rootStyle.setProperty("--zerocmux-diff-bg-dark", colorString(darkTheme.background, "#000000"));
  ...
  rootStyle.setProperty("--zerocmux-diff-surface-fill-light", "transparent");
  rootStyle.setProperty("--zerocmux-diff-surface-fill-dark", "transparent");
  rootStyle.setProperty("--zerocmux-diff-fg-light", colorString(lightTheme.foreground, "#000000"));
  rootStyle.setProperty("--zerocmux-diff-fg-dark", colorString(darkTheme.foreground, "#ffffff"));
  rootStyle.setProperty("--zerocmux-diff-addition-fg-light", semanticPaletteColor(lightTheme, ["10", "2"], "#257a3e"));
  rootStyle.setProperty("--zerocmux-diff-addition-fg-dark", semanticPaletteColor(darkTheme, ["10", "2"], "#8fd88f"));
  rootStyle.setProperty("--zerocmux-diff-deletion-fg-light", semanticPaletteColor(lightTheme, ["9", "1"], "#b42318"));
  rootStyle.setProperty("--zerocmux-diff-deletion-fg-dark", semanticPaletteColor(darkTheme, ["9", "1"], "#ff8a80"));
  rootStyle.setProperty("--zerocmux-diff-selection-bg-light", colorString(lightTheme.selectionBackground, "#abd8ff"));
  rootStyle.setProperty("--zerocmux-diff-selection-bg-dark", colorString(darkTheme.selectionBackground, "#3f638b"));
  rootStyle.setProperty("--zerocmux-diff-code-font-family", codeFontFamily(appearance.fontFamily));
  rootStyle.setProperty("--zerocmux-diff-font-size", `${metric(appearance.fontSize, 10)}px`);
  rootStyle.setProperty("--zerocmux-diff-line-height", `${metric(appearance.lineHeight, 20)}px`);
}
