  ...
  return (
    <div id="app" data-file-search-open={state.fileSearchOpen}>
      <Toolbar
        ...
        state={state}
      />
      <section id="content" style={{ "--zerocmux-diff-files-width": `${state.filesWidth}px` } as React.CSSProperties}>
...
function useDiffComments({
  ...
  const saveDraft = (item: DiffItem, message: string) => {
    ...
    save
      ...
      .catch((error) => console.warn("zerocmux diff comment save failed", error));
  };
  ...
  const editMessage = (
    ...
    save
      ...
      .catch((error) => console.warn("zerocmux diff comment edit failed", error));
  };
  ...
  const remove = (comment: DiffCommentRecord) => {
    ...
    if (bridgeAvailable && repoRoot != null) {
      bridgeDeleteComment(repoRoot, comment.id)
        .catch((error) => console.warn("zerocmux diff comment delete failed", error));
    }
    ...
    <div className="diff-loading-placeholder" aria-hidden="true">
      {fileSkeletonWidths.map((width, index) => (
        <div key={`${width}-${index}`} className="grid h-6 grid-cols-[16px_minmax(0,1fr)_44px] items-center gap-2 rounded-[5px] px-[7px]">
          <span className="size-4 rounded-[5px] border border-[color-mix(in_lab,var(--zerocmux-diff-fg)_18%,transparent)]" />
          <span className="h-[11px] rounded bg-[var(--zerocmux-diff-muted-bg)]" style={{ width }} />
          <span className="h-[11px] justify-self-end rounded bg-[var(--zerocmux-diff-muted-bg)] opacity-70" style={{ width: index % 2 === 0 ? "34px" : "24px" }} />
        </div>
      ))}
    </div>
  );
...
function LoadingDiffSkeleton() {
  return (
    <div className="diff-loading-placeholder mx-3.5 mt-3.5 border-t border-[var(--zerocmux-diff-border)] pt-3" aria-hidden="true">
      <div className="mb-3 grid h-9 grid-cols-[72px_minmax(0,1fr)_96px] items-center gap-3 rounded-md bg-[color-mix(in_lab,var(--zerocmux-diff-fg)_5%,transparent)] px-3">
        <span className="h-3 rounded bg-[var(--zerocmux-diff-muted-bg)]" />
        <span className="h-3 w-2/5 rounded bg-[var(--zerocmux-diff-muted-bg)]" />
        <span className="h-3 rounded bg-[var(--zerocmux-diff-muted-bg)] opacity-70" />
      </div>
      <div className="space-y-[13px] px-3 py-1">
        {diffSkeletonWidths.map((width, index) => (
          <div key={`${width}-${index}`} className="grid grid-cols-[42px_minmax(0,1fr)] items-center gap-4">
            <span className="h-px bg-[color-mix(in_lab,var(--zerocmux-diff-fg)_10%,transparent)]" />
            <span className="h-3 rounded bg-[var(--zerocmux-diff-muted-bg)]" style={{ width }} />
          </div>
        ))}
      </div>
    </div>
...
function useWorkerRenderOptionsSync(
  ...
  useEffect(() => {
    ...
    workerPool.setRenderOptions(highlighterOptions)
      ...
      .catch((error: unknown) => console.warn("zerocmux diff worker render options update failed", error));
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        await streamPatch({
          ...
          onComplete: (metrics) => {
            ...
            preloadHighlighter({ themes, langs: langs.length > 0 ? langs : ["text"] })
              .catch((error) => console.warn("zerocmux diff highlighter preload failed", error));
          },
          onMetrics: (metrics) => {
        ...
        if (!empty) {
          console.error("zerocmux diff viewer render failed", error);
        }
        const emptyMessage = typeof payload.emptyMessage === "string" ? payload.emptyMessage : label("noFileDiffs");
        dispatch({
          type: "set-status",
...
function usePendingReplacement(
  ...
  useEffect(() => {
    ...
    if (payload.pendingReplacement === true) {
      ...
      if (window.location.protocol === "zerocmux-diff-viewer:") {
        return;
      }
      fetch("/__cmux_diff_viewer_wait" + window.location.pathname, { cache: "no-store" })
        ...
        .catch((error) => {
          ...
          console.warn("zerocmux diff viewer deferred load failed", error);
        });
      return;
    }
