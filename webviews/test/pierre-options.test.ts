...
test("code view CSS keeps Pierre structural surfaces transparent", () => {
  ...
  expect(css).toContain("--diffs-bg-buffer-override: color-mix(in srgb, var(--zerocmux-diff-fg) 12%, transparent)");
  ...
  expect(css).toContain("--zerocmux-diff-surface-bg: transparent");
  expect(css).toContain("--zerocmux-diff-header-bg: color-mix(in srgb, var(--zerocmux-diff-bg) 42%, transparent)");
  expect(css).not.toContain("color-mix(in srgb, var(--zerocmux-diff-bg) 94%, #3e3d32)");
  ...
  expect(css).toContain("background-color: var(--zerocmux-diff-header-bg) !important");
  ...
  expect(css).not.toContain("border-block: 1px solid var(--zerocmux-diff-border)");
...
test("file tree sticky overlays use a non-transparent surface", () => {
  ...
  expect(css).toContain("background-color: var(--zerocmux-diff-sidebar-bg)");
  expect(css).toContain("--zerocmux-diff-tree-sticky-bg: var(--zerocmux-diff-bg)");
  expect(css).toContain("[data-file-tree-sticky-overlay-content]");
  expect(css).toContain("background-color: var(--zerocmux-diff-tree-sticky-bg) !important");
