...
test("custom-scheme pending pages wait for native navigation without HTTP polling", () => {
  dom = createDom("zerocmux-diff-viewer://0123456789abcdef/opening.html");
  let fetched = false;
  installDomGlobals(dom, () => {
    fetched = true;
    throw new Error("unexpected fetch");
...
test("custom-scheme pending pages stream exactly one typed Rust session", async () => {
  dom = createDom("zerocmux-diff-viewer://0123456789abcdef/branch.html");
  const requests: any[] = [];
          ...
          return {
            ...
            result: {
              ...
              value: {
                ...
                patch: {
                  id: "zerocmux-diff-viewer://0123456789abcdef/diff-session.patch",
  ...
  expect(fetched).toEqual(["zerocmux-diff-viewer://0123456789abcdef/diff-session.patch"]);
...
test("typed Rust empty diffs keep the localized source-specific message", async () => {
  dom = createDom("zerocmux-diff-viewer://0123456789abcdef/unstaged.html");
  let fetched = false;
  installDomGlobals(dom, () => {
    fetched = true;
    return new Response("", { status: 200 });
...
test("Last Turn reveals repo selection after switching to a typed git source", async () => {
  dom = createDom("zerocmux-diff-viewer://0123456789abcdef/last-turn.html");
  const requests: any[] = [];
  installDomGlobals(dom, () => new Response("", { status: 200 }));
  (dom.window as any).webkit = {
    messageHandlers: {
...
function contentFilesWidth(): string | undefined {
  return dom?.window.document.getElementById("content")?.style.getPropertyValue("--zerocmux-diff-files-width");
}
