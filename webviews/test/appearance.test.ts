...
describe("appearanceBackgroundColor", () => {
  ...
  test("applies readable semantic diff colors from the terminal palette", () => {
    ...
    expect(style.getPropertyValue("--zerocmux-diff-deletion-fg-light")).toBe("#cc0000");
    expect(style.getPropertyValue("--zerocmux-diff-addition-fg-light")).toBe("#007a00");
    expect(style.getPropertyValue("--zerocmux-diff-deletion-fg-dark")).toBe("#ff8a80");
    expect(style.getPropertyValue("--zerocmux-diff-addition-fg-dark")).toBe("#a6e22e");
  });
  ...
  test("keeps the page surface transparent for opaque themes", () => {
    ...
    const style = dom.window.document.documentElement.style;
    expect(style.getPropertyValue("--zerocmux-diff-surface-fill-light")).toBe("transparent");
    expect(style.getPropertyValue("--zerocmux-diff-surface-fill-dark")).toBe("transparent");
  });
