...
This is the source-owned React bundle for embedded zerocmux webviews. It currently ships the `zerocmux diff` viewer and is structured to host more React-backed webviews.
...
The sample opener caches local clones under `/tmp/zerocmux-diff-viewer-stress`, checks out the sample refs, then runs `zerocmux diff --base <ref>` from inside the repository so the stress path matches normal local git diffs.
