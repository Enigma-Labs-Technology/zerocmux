...
zerocmux is pinned to Xcode 26.x. `.xcode-version` records the major version. `cmux.xcodeproj/project.pbxproj` carries `objectVersion = 60`, which is what Xcode 26 writes by default.

`objectVersion = 77` is reserved for projects that adopt synchronized folder groups. zerocmux does not use synchronized folder groups yet.
