...
```text
com.kernelalex.zerocmux.cmux.sidebar
```

Tagged host builds scope it to:

```text
com.kernelalex.zerocmux.debug.<tag>.cmux.sidebar
```

## Why display name matters

`AppExtensionIdentity` exposes stable fields such as bundle identifier, localized name, extension point identifier, and id. zerocmux keys its identity off the stable bundle identifier, but OS-level enable/disable and availability grouping uses display name.
