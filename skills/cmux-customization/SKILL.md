---
name: zerocmux-customization
description: "Customize zerocmux for an end user. Use when changing cmux.json actions, custom commands, workspace layouts, plus-button behavior, surface tab bar buttons, Command Palette entries, Dock controls, sidebar and app settings, shortcuts, notifications, browser routing, examples-library presets, or Ghostty-backed terminal preferences."
---
...
- Dock controls: create `.cmux/dock.json` or `~/.config/cmux/dock.json` for right-sidebar terminal controls such as logs, test watchers, git TUIs, dev servers, queues, or `zerocmux feed tui --opentui`.
- Sidebar and app behavior: use `zerocmux-settings` for supported settings such as appearance, sidebar display, notification behavior, browser routing, automation, shortcuts, and new-workspace placement.
- Workspace metadata: use the zerocmux CLI or `zerocmux-workspace` for workspace names, descriptions, colors, read state, and sidebar metadata updates.
- Feed and notifications: use `zerocmux hooks setup` for Feed event sources, notification settings for delivery behavior, and notification hooks in `cmux.json` for filtering or post-processing banners.
...
- Dock controls: edit `.cmux/dock.json` in the project or `~/.config/cmux/dock.json` globally. Run `zerocmux docs dock` when available.
...
If a request can be handled by Ghostty config, say that and use Ghostty config instead of inventing zerocmux UI settings.
...
4. For app settings and zerocmux-owned shortcuts, use the settings helper from the installed skill or checkout:

   ```bash
   ~/.agents/skills/zerocmux-settings/scripts/zerocmux-settings list-supported
   ~/.agents/skills/zerocmux-settings/scripts/zerocmux-settings set browser.openTerminalLinksInCmuxBrowser true
...
- App settings: run `zerocmux-settings validate`.
...
- Runtime reload: run `zerocmux reload-config` when the CLI is available.
...
- Do not add a zerocmux setting for behavior Ghostty already owns.
