...
This reference expands zerocmux package extraction and refactor architecture rules.
...
App-target packages link into both `zerocmux` and `zerocmux-unit`, so tests can import and inject them. A package linked by the app but not `zerocmux-unit` can make the app build pass while the test target fails.
