...
zerocmux browser --surface <surface> back|forward|reload
zerocmux browser --surface <surface> get url|title
```
...
zerocmux browser --surface <surface> get html body
zerocmux browser --surface <surface> get value "#email"
zerocmux browser --surface <surface> get attr "#email" --attr placeholder
zerocmux browser --surface <surface> get count ".row"
zerocmux browser --surface <surface> get box "#submit"
zerocmux browser --surface <surface> get styles "#submit" --property color
zerocmux browser --surface <surface> eval '<js>'
```

## Interaction

```bash
zerocmux browser --surface <surface> click|dblclick|hover|focus <selector-or-ref>
zerocmux browser --surface <surface> fill <selector-or-ref> [text]
zerocmux browser --surface <surface> type <selector-or-ref> <text>
zerocmux browser --surface <surface> press|key|keydown|keyup [--key <key> | <key>]
zerocmux browser --surface <surface> select <selector-or-ref> <value>
zerocmux browser --surface <surface> check|uncheck <selector-or-ref>
zerocmux browser --surface <surface> scroll [--selector <css>] [--dx <n>] [--dy <n>]
```
...
```bash
zerocmux browser --surface <surface> wait --selector "#ready" --timeout-ms 10000
zerocmux browser --surface <surface> wait --text "Done" --timeout-ms 10000
zerocmux browser --surface <surface> wait --url-contains "/dashboard" --timeout-ms 10000
zerocmux browser --surface <surface> wait --load-state complete --timeout-ms 15000
zerocmux browser --surface <surface> wait --function "document.readyState === 'complete'" --timeout-ms 10000
```
...
```bash
zerocmux browser --surface <surface> cookies get|set|clear ...
zerocmux browser --surface <surface> cookies clear --url https://app.example.com/
zerocmux browser --surface <surface> cookies clear --domain app.example.com
zerocmux browser --surface <surface> cookies clear --all
zerocmux browser --surface <surface> storage local|session get|set|clear ...
zerocmux browser --surface <surface> tab list|new|switch|close ...
zerocmux browser --surface <surface> state save|load <path>
zerocmux browser --surface <surface> console list|clear
zerocmux browser --surface <surface> errors list|clear
zerocmux browser --surface <surface> highlight <selector>
zerocmux browser --surface <surface> screenshot
zerocmux browser --surface <surface> download wait --timeout-ms 10000
```
