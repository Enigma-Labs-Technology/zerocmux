...
/// Behavior tests for the remote-tmux SSH auth path that backs `zerocmux ssh-tmux`:
...
@Suite struct RemoteTmuxAuthTests {
    ...
    @Test func controlModeArgumentsUseRemoteTmuxResolverAfterDestinationGuard() throws {
        ...
        #expect(remoteCommand.hasSuffix("'zerocmux-remote-tmux' '-CC' 'attach-session' '-t' 'work session'"))
    }
    ...
    @Test func pastePaneCommandsProtectOptionLookingText() throws {
        ...
        #expect(commands.setBuffer == "set-buffer -b zerocmux-paste-7 -- '-n not-an-option'")
        #expect(commands.pasteBuffer == "paste-buffer -p -d -b zerocmux-paste-7 -t %7")
    }
    ...
    // MARK: - Interactive auth invocation (what `zerocmux ssh-tmux` runs in the tty)
