...
/// Regression coverage for https://github.com/kernelalex/zerocmux/issues/5032.
///
...
struct ExtensionWorktreeSpawnArgsTests {
    private func makeResult(setupCommand: String) -> CmuxExtensionWorktreeCreationResult {
        CmuxExtensionWorktreeCreationResult(
            worktreePath: "/tmp/project/.cmux/worktrees/zerocmux-sidebar-123",
            workspaceTitle: "zerocmux-sidebar-123",
    ...
    func setupCommandRunsAsShellInput() {
        ...
        // interactive shell, matching the `zerocmux new-workspace --cwd` contract.
    ...
    func worktreePathIsWorkingDirectory() {
        ...
        #expect(args.workingDirectory == "/tmp/project/.cmux/worktrees/zerocmux-sidebar-123")
        #expect(args.inheritWorkingDirectory == false)
        #expect(args.title == "zerocmux-sidebar-123")
    }
