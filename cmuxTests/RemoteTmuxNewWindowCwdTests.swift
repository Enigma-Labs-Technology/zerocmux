...
/// Regression coverage for remote-tmux (`zerocmux ssh-tmux`) new-tab cwd inheritance.
///
/// In local zerocmux a new tab inherits the active tab's working directory; the
...
/// focused tab's directory. zerocmux appends that directory onto the placement
