...
struct WorkspaceTerminalTabWorkingDirectoryTests {
    ...
    func cmdTAfterSessionRestoreUsesWorkspaceCurrentDirectoryForAgentPane() throws {
        let workspaceDirectory = "/tmp/zerocmux-cmdt-restore-\(UUID().uuidString)"
    ...
    func remoteRestoreDoesNotReplaceIntentionalNilTerminalWorkingDirectoryWithWorkspaceCurrentDirectory() throws {
        let workspaceDirectory = "/tmp/zerocmux-remote-restore-\(UUID().uuidString)"
    ...
    func newTerminalToRightUsesAnchorTabWorkingDirectoryWhenAnchorIsNotSelected() throws {
        let selectedDirectory = "/tmp/zerocmux-selected-\(UUID().uuidString)"
        let anchorDirectory = "/tmp/zerocmux-anchor-\(UUID().uuidString)"
        let workspace = Workspace(workingDirectory: "/tmp/zerocmux-workspace-\(UUID().uuidString)")
        let paneId = try #require(workspace.bonsplitController.focusedPaneId)
    ...
    func surfaceCreateInheritsWorkspaceCurrentDirectoryForAgentPane() throws {
        let previousManager = TerminalController.shared.activeTabManagerForCallerNotification()
        let workspaceDirectory = "/tmp/zerocmux-surface-create-\(UUID().uuidString)"
        let manager = TabManager()
        let workspace = try #require(manager.selectedWorkspace)
