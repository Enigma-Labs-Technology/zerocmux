...
final class SessionPersistenceTests: XCTestCase {
    ...
    func testWorkspaceSessionSnapshotRestoresMarkdownPanel() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-session-markdown-\(UUID().uuidString)", isDirectory: true)
    ...
    func testSessionSnapshotSkipsTransientRemoteListeningPorts() throws {
        ...
        let configuration = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/zerocmux-test.sock",
    ...
    func testSaveAndLoadRoundTripWithCustomSnapshotPath() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-session-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    func testScrollbackReplayEnvironmentWritesReplayFile() {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-scrollback-replay-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    // Regression for https://github.com/kernelalex/zerocmux/issues/5165.
    //
    ...
    func testNormalizedExportedScreenPathAcceptsAbsoluteAndFileURL() {
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath("/tmp/zerocmux-screen.txt"),
            "/tmp/zerocmux-screen.txt"
        )
        XCTAssertEqual(
            TerminalController.normalizedExportedScreenPath(" file:///tmp/zerocmux-screen.txt "),
            "/tmp/zerocmux-screen.txt"
        )
    }
    ...
    func testShouldRemoveExportedScreenDirectoryOnlyWithinTemporaryRoot() {
        let tempRoot = URL(fileURLWithPath: "/tmp")
            .appendingPathComponent("zerocmux-export-tests-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeRestorableAgentIndex(
        ...
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-agent-hook-store-\(UUID().uuidString)", isDirectory: true)
...
final class SocketListenerAcceptPolicyTests: XCTestCase {
    func testClaudeResumeCommandRoutesThroughWrapperInsteadOfCapturedRealBinary() {
        ...
        // zerocmux's `claude` wrapper, which is what injects the hooks, so resumed
        // sessions silently lost SessionStart/Stop/Notification. Resume must use
        // the bare `claude` wrapper. https://github.com/kernelalex/zerocmux/issues/5427
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .claude,
            sessionId: "claude-session-123",
            workingDirectory: "/tmp/zerocmux project",
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/zerocmux project' 2>/dev/null || [ ! -d '/tmp/zerocmux project' ] && /bin/sh -c "
    ...
    func testClaudeForkCommandRoutesThroughWrapperInsteadOfCapturedRealBinary() throws {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                arguments: [
                    ...
                    "sonnet",
                    "--settings",
                    #"{"hooks":{"SessionStart":[{"hooks":[{"command":"zerocmux claude-hook session-start"}]}]}}"#,
                    "--session-id",
                    "old-session"
                ],
                workingDirectory: "/tmp/zerocmux project",
        ...
        // https://github.com/kernelalex/zerocmux/issues/5427
        ...
        XCTAssertFalse(command.contains("zerocmux claude-hook session-start"), command)
        XCTAssertFalse(command.contains("old-session"), command)
    }

    /// Regression for https://github.com/kernelalex/zerocmux/issues/5639.
    ///
    ...
    /// where zerocmux's shell integration (the `claude()` function + per-surface PATH shim)
    ...
    /// user's *real* binary, the zerocmux wrapper was bypassed, no hook `--settings` was
    ...
    /// and asserts the launched invocation routed through the zerocmux wrapper (its injected
    ...
    func testClaudeResumeCommandExecutesThroughWrapperInsideLoginShellLauncher() throws {
        ...
        let sandbox = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-5639-\(UUID().uuidString)", isDirectory: true)
        let shimDir = sandbox.appendingPathComponent("zerocmux-cli-shims", isDirectory: true)
        ...
        let wrapperURL = sandbox.appendingPathComponent("zerocmux-claude-wrapper", isDirectory: false)
        ...
        // Stand-in for zerocmux-claude-wrapper: re-inject the hook --settings on --resume
        ...
        """)
        // Per-surface shim that zerocmux installs and points CMUX_CLAUDE_WRAPPER_SHIM at.
        ...
        XCTAssertTrue(
            ...
            "Resumed claude must route through the zerocmux wrapper (which injects the hook --settings). Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "Resume must exec the zerocmux wrapper, not the user's real claude binary. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
    }
    ...
    /// under `zsh -lic`. https://github.com/kernelalex/zerocmux/issues/5639
    func testClaudeResumeCommandExecutesThroughWrapperInsideTcshLauncher() throws {
        ...
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "tcsh-dispatched resume must parse and exec the zerocmux wrapper. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.contains("--settings"),
    ...
    /// is a shipped zerocmux integration (`Resources/shell-integration/fish/`). The restore
    ...
    /// (`cd … || [ ! -d … ] && …`, https://github.com/kernelalex/zerocmux/issues/6285) is
    /// exercised too — older fish rejects the POSIX `{ …; }` grouping the guard used to
    /// emit. https://github.com/kernelalex/zerocmux/issues/5639
    func testClaudeResumeCommandExecutesThroughWrapperInsideFishLauncher() throws {
        ...
        XCTAssertTrue(
            recorded.hasPrefix("wrapper "),
            "fish-dispatched resume must parse and exec the zerocmux wrapper. Recorded invocation: \(recorded.isEmpty ? "<none>" : recorded)"
        )
        XCTAssertTrue(
            recorded.contains("--settings"),
    ...
    /// restore-launcher regression tests: a recording stand-in for `zerocmux-claude-wrapper`
    ...
    func testRestorableAgentResumeStartupInputEscapesNonAsciiWorkingDirectoryAsAsciiShellInput() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-agent-resume-\(UUID().uuidString)", isDirectory: true)
        let cwdURL = root
        ...
        // (https://github.com/kernelalex/zerocmux/issues/5639) exceed the inline
    ...
    func testSessionEntryClaudeResumeCommandChangesToSessionCwdBeforeResume() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-claude-resume-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testRestorableAgentStartupInputUsesLauncherScriptWhenCommandExceedsTerminalInputBudget() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-agent-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testCodexTeamsResumeCommandUsesWrapperSubcommand() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codexTeams",
                executablePath: "/usr/local/bin/zerocmux",
                arguments: [
                    "/usr/local/bin/zerocmux",
                    "codex-teams",
                    "--model",
                    "gpt-5.4",
                    "--image",
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/zerocmux' 'codex-teams' 'resume' '019dad34-d218-7943-b81a-eddac5c87951' '-c' 'check_for_update_on_startup=false' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testCodexTeamsResumeCommandDropsOriginalForkTarget() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/zerocmux' 'codex-teams' 'resume' '019dad34-d218-7943-b81a-eddac5c87952' '-c' 'check_for_update_on_startup=false' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testForkCommandsUseVerifiedAgentForkSyntaxAndPreserveContext() {
        ...
        XCTAssertEqual(
            ...
            "cd -- '/Users/example/repo' 2>/dev/null || [ ! -d '/Users/example/repo' ] && 'env' 'CODEX_HOME=/tmp/codex home' '/usr/local/bin/zerocmux' 'codex-teams' 'fork' 'codex-teams-session' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/zerocmux' 'omo' '--session' 'opencode-session-123' '--fork' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
            omoOpenCodeFork.forkCommand,
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/zerocmux' 'omo' '--session' 'opencode-child-session' '--fork' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
    ...
    func testOpenCodeForkSupportProbesFromLaunchWorkingDirectory() async throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-opencode-probe-\(UUID().uuidString)", isDirectory: true)
    ...
    func testOpenCodeForkSupportSkipsLocalProbeForRemoteLikeContext() async {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            workingDirectory: "/remote/zerocmux/project-\(UUID().uuidString)",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "opencode",
    ...
    func testAgentForkSupportRejectsRemoteForksThatNeedLauncherScript() async {
        let longPath = "/Users/zerocmux/" + String(repeating: "nested-project-", count: 120)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
            workingDirectory: "/Users/zerocmux/project",
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "codex",
                executablePath: "/Users/example/.bun/bin/codex",
                arguments: [
    ...
    func testOpenCodeForkSupportRejectsMissingLocalExecutable() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-opencode-missing-executable-\(UUID().uuidString)", isDirectory: true)
        defer { try? fileManager.removeItem(at: root) }
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
    ...
    func testOpenCodeForkSupportCachesUnsupportedVersion() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-opencode-probe-cache-\(UUID().uuidString)", isDirectory: true)
        let executable = root.appendingPathComponent("opencode", isDirectory: false)
    ...
    func testProcessDetectedOpenCodeResolvesBareExecutableWithCapturedPath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-opencode-path-\(UUID().uuidString)", isDirectory: true)
    ...
    func testProcessDetectedOpenCodeLaunchArgumentsPreserveSafeForkContext() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-opencode-argv-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
        let executable = bin.appendingPathComponent("opencode", isDirectory: false)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testClaudeTeamsResumeCommandPreservesRemoteControlLauncher() {
        ...
        XCTAssertEqual(
            snapshot.resumeCommand,
            "cd -- '/tmp/team repo' 2>/dev/null || [ ! -d '/tmp/team repo' ] && 'env' 'CMUX_CUSTOM_CLAUDE_PATH=/opt/Claude Code/bin/claude' '/Applications/zerocmux.app/Contents/Resources/bin/zerocmux' 'claude-teams' '--resume' 'claude-team-session' '--teammate-mode' 'auto' '--model' 'sonnet' '--remote-control-session-name-prefix' 'zerocmux-team' '--permission-mode' 'auto'"
        )
    }
    ...
    func testClaudeResumeCommandStripsStaleCmuxNodeOptionsRestoreModule() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require=/tmp/zerocmux-claude-node-options/restore-node-options.cjs --max-old-space-size=4096 --trace-warnings"
                ],
                capturedAt: nil,
                source: nil
            )
    ...
    func testClaudeResumeCommandDropsEmptyStaleCmuxNodeOptionsEnvironment() {
        let snapshot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                ...
                workingDirectory: nil,
                environment: [
                    "NODE_OPTIONS": "--require /tmp/zerocmux-claude-node-options/restore-node-options.cjs --max-old-space-size 4096"
                ],
                capturedAt: nil,
                source: nil
            )
    ...
    func testOpenCodeWrapperResumeCommandAndUnsupportedOhMyLaunchers() {
        ...
        let omx = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omx",
                executablePath: "/usr/local/bin/zerocmux",
                arguments: ["/usr/local/bin/zerocmux", "omx", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        let omc = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "omc",
                executablePath: "/usr/local/bin/zerocmux",
                arguments: ["/usr/local/bin/zerocmux", "omc", "team"],
                workingDirectory: nil,
                environment: nil,
                capturedAt: nil,
                source: nil
        ...
        XCTAssertEqual(
            omo.resumeCommand,
            "cd -- '/tmp/opencode repo' 2>/dev/null || [ ! -d '/tmp/opencode repo' ] && 'env' 'OPENCODE_CONFIG_DIR=/tmp/opencode config' '/usr/local/bin/zerocmux' 'omo' '--session' 'opencode-session-123' '--model' 'anthropic/claude-sonnet-4-6' '/tmp/opencode repo'"
        )
        XCTAssertEqual(
        ...
        let processSnapshot = CmuxTopProcessSnapshot(
            processes: [
                CmuxTopProcessInfo(
                    ...
                    cmuxAttributionReason: "zerocmux-test",
                    processGroupID: nil,
...
extension SessionPersistenceTests {
    ...
    // Regression for https://github.com/kernelalex/zerocmux/issues/6285: the cwd guard runs
    ...
    // A binding persisted by an older zerocmux with the legacy braced guard must be
    ...
    func testAgentHookSurfaceResumeStartupInputRunsWhenSavedWorkingDirectoryWasDeleted() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-surface-resume-missing-cwd-\(UUID().uuidString)", isDirectory: true)
        let bin = root.appendingPathComponent("bin", isDirectory: true)
    ...
    func testSurfaceResumeBindingStartupInputUsesLauncherScriptWhenLong() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-surface-resume-test-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tempDir) }
    ...
    func testSurfaceResumeApprovalMissingRecordResetsStalePromptPolicy() throws {
        ...
        let effectiveBinding = SurfaceResumeApprovalStore.applyingStoredApproval(
            to: binding,
            fileURL: URL(fileURLWithPath: "/tmp/zerocmux-missing-\(UUID().uuidString).json"),
    ...
    func testSurfaceResumeApprovalWritesRecordsIntoCmuxJSON() throws {
        ...
        let initialSettings = """
        {
          "$schema": "https://raw.githubusercontent.com/kernelalex/zerocmux/main/web/data/cmux.schema.json",
          // keep root comment
          "schemaVersion": 1,
          "terminal": {
    ...
    func testHermesAgentHookSurfaceResumeBootstrapsSubrouterAndRewritesStaleCodexProvider() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-hermes-surface-resume-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeSurfaceResumeApprovalStoreURL() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-surface-resume-approvals-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock {
    ...
    private func makeSurfaceResumeApprovalCmuxSettingsURL() throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-surface-resume-settings-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        addTeardownBlock {
            try? FileManager.default.removeItem(at: root)
        }
    ...
    func testRestoreRunsSurfaceResumeBindingFromBindingCwd() throws {
        ...
        let bindingCwd = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-surface-binding-cwd-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRestoreDoesNotPassDeletedAgentHookCwdToTerminalRuntime() throws {
        try withAutoResumeAgentSessionsEnabled {
            ...
            let missingCwd = FileManager.default.temporaryDirectory
                .appendingPathComponent("zerocmux-deleted-agent-hook-cwd-\(UUID().uuidString)", isDirectory: true)
                .appendingPathComponent("repo", isDirectory: true)
            let bindingIndex = SurfaceResumeBindingIndex(bindingsByPanel: [
                SurfaceResumeBindingIndex.PanelKey(workspaceId: source.id, panelId: sourcePanelId): SurfaceResumeBindingSnapshot(
                    name: "Codex",
            ...
            // The guard is the fish-safe, brace-free form (https://github.com/kernelalex/zerocmux/issues/6285):
    ...
    func testRestorePreservesUnmountedVolumeCwdBindingsWhenInitialReportsAreScrambled() throws {
        try withAutoResumeAgentSessionsEnabled {
            ...
            let volumeName = "zerocmux-issue-5278-\(UUID().uuidString)"
    ...
    func testMarkdownFileLinkResolverPrefersCurrentMarkdownDirectory() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-link-resolver-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownFileLinkResolverFallsBackToProcessWorkingDirectory() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-link-resolver-cwd-\(UUID().uuidString)", isDirectory: true)
        let docs = root.appendingPathComponent("docs", isDirectory: true)
        let openedFile = docs.appendingPathComponent("index.md")
    ...
    func testMarkdownFileLinkResolverRejectsMissingAndNonMarkdownFiles() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-link-resolver-reject-\(UUID().uuidString)", isDirectory: true)
