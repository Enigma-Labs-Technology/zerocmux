...
final class WorkspaceSplitStartupCommandTests: XCTestCase {
    ...
    func testTabManagerSplitCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/zerocmux-split-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/zerocmux-tmux-command-\(UUID().uuidString).sh"
    ...
    func testNewTerminalSurfaceCarriesRequestedWorkingDirectoryAndStartupCommand() {
        ...
        let requestedDirectory = "/tmp/zerocmux-surface-startup-\(UUID().uuidString)"
        let startupCommand = "/tmp/zerocmux-surface-command-\(UUID().uuidString).sh"
        let tmuxStartCommand = "node /opt/oh-my-codex/dist/omx.js hud --watch"
    ...
    func testRespawnTerminalSurfacePreservesPaneTabAndSurfaceIdentity() throws {
        ...
        let requestedDirectory = "/tmp/zerocmux-respawn-\(UUID().uuidString)"
        ...
        XCTAssertTrue(
            ...
            "Respawn should replace the registered terminal surface for the existing zerocmux surface id"
        )
    }
    ...
    func testSessionRestoreRelaunchesOMXHudTmuxStartCommand() throws {
        let workspace = Workspace()
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let requestedDirectory = "/tmp/zerocmux-hud-restore-\(UUID().uuidString)"
        let originalStartupScript = "/tmp/zerocmux-tmux-command-\(UUID().uuidString).sh"
        ...
        XCTAssertTrue(restoredStartupScript.contains("zerocmux-session-terminal-command"))
    ...
    func testSessionSnapshotDoesNotPersistGenericTmuxStartCommand() throws {
        ...
        let panel = try XCTUnwrap(workspace.newTerminalSurface(
            inPane: paneId,
            focus: false,
            initialCommand: "/tmp/zerocmux-command-\(UUID().uuidString).sh",
