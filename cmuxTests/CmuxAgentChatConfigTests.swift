...
struct CmuxAgentChatConfigTests {
    ...
    @MainActor
    @Test func commandPaletteNewAgentChatContributionIsAlwaysAvailable() throws {
        // zerocmux: the PostHog rollout flag is removed; the agent-chat
        // palette entry is always contributed (browser availability is
        // enforced by the contribution's `when` predicate).
