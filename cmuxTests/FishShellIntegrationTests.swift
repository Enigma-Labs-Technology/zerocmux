...
struct FishShellIntegrationTests {
    ...
    func testApplyManagedFishStartupEnvironmentPreservesUserConfigHome() {
        ...
        TerminalSurface.applyManagedFishStartupEnvironment(
            integrationDir: "/Applications/zerocmux.app/Contents/Resources/shell-integration",
            to: &environment,
            protectedKeys: &protectedKeys
        )

        expectEqual(environment["XDG_CONFIG_HOME"], "/Users/example/.config")
        expectEqual(environment["CMUX_FISH_INTEGRATION_FILE"], "/Applications/zerocmux.app/Contents/Resources/shell-integration/fish/config.fish")
    ...
    func testApplyManagedFishStartupEnvironmentAvoidsRecursiveConfigHome() {
        var environment = ["XDG_CONFIG_HOME": "/Applications/zerocmux.app/Contents/Resources/shell-integration/"]
        var protectedKeys: Set<String> = []
        ...
        expectEqual(environment["XDG_CONFIG_HOME"], "/Applications/zerocmux.app/Contents/Resources/shell-integration/")
        expectEqual(environment["CMUX_FISH_INTEGRATION_FILE"], "/Applications/zerocmux.app/Contents/Resources/shell-integration/fish/config.fish")
        expectEqual(environment["CMUX_FISH_USER_CONFIG_ALREADY_LOADED"], "1")
        expectFalse(protectedKeys.contains("XDG_CONFIG_HOME"))
        expectTrue(protectedKeys.contains("CMUX_FISH_INTEGRATION_FILE"))
    ...
    func testManagedFishShellCommandLetsGhosttyApplyLoginExecWrapper() {
        let command = TerminalSurface.managedFishShellCommand(shell: "/Applications/zerocmux DEV fishsh.app/fish")

        expectEqual(command, "'/Applications/zerocmux DEV fishsh.app/fish' -il --init-command 'source \"$CMUX_FISH_INTEGRATION_FILE\"'")
    ...
    func testFishIntegrationSourcesUserConfigAndReportsShellState() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-fish-shell-integration-\(UUID().uuidString)")
        ...
        let result = try runInteractiveFish(
            ...
            extraEnvironment: [
                ...
                "CMUX_SOCKET_PATH": root.appendingPathComponent("zerocmux-test.sock").path,
    ...
    func testFishIntegrationRelayPromptReportsPWD() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-fish-relay-pwd-\(UUID().uuidString)")
        ...
        let cmuxPath = binDir.appendingPathComponent("zerocmux", isDirectory: false)
    ...
    func testGeneratedFishBootstrapStagesIntegrationAndPreservesUserConfigHome() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-fish-shell-bootstrap-\(UUID().uuidString)")
    ...
    private func writeUserFishConfig(at userConfigHome: URL) throws {
        ...
        let userFishConfD = userConfigHome.appendingPathComponent("fish/conf.d/zerocmux-test.fish", isDirectory: false)
        let userFishFunction = userConfigHome.appendingPathComponent("fish/functions/cmux_test_function.fish", isDirectory: false)
        let userFishCompletion = userConfigHome.appendingPathComponent("fish/completions/zerocmux-test.fish", isDirectory: false)
        ...
        try "complete -c zerocmux-test -f\n"
    ...
    private func runInteractiveFish(
        ...
        try writeExecutableShellFile(at: testBinDir.appendingPathComponent("zerocmux-claude-wrapper"), body: "#!/bin/sh\necho zerocmux-claude-wrapper \"$@\"\n")
        try writeExecutableShellFile(at: testBinDir.appendingPathComponent("grok"), body: "#!/bin/sh\necho zerocmux-grok-wrapper \"$@\"\n")
