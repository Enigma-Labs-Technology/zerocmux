...
final class CMUXOpenCommandTests: XCTestCase {
    ...
    func testDiffCommandGeneratesCodeViewAndOpensBrowserSplit() throws {
        ...
        let cmuxConfigURL = homeURL
            .appendingPathComponent(".config", isDirectory: true)
            .appendingPathComponent("zerocmux", isDirectory: true)
            .appendingPathComponent("cmux.json", isDirectory: false)
        let cmuxAppSupportConfigURL = homeURL
            .appendingPathComponent("Library", isDirectory: true)
            .appendingPathComponent("Application Support", isDirectory: true)
            .appendingPathComponent("com.kernelalex.zerocmux", isDirectory: true)
        ...
        let serverHandled = startMockServer(listenerFD: listenerFD, state: state) { line in
            ...
            guard method == "browser.open_split",
                  ...
                  viewerURL.scheme == "zerocmux-diff-viewer",
        ...
        XCTAssertEqual(viewerURL.scheme, "zerocmux-diff-viewer")
        ...
        XCTAssertTrue(html.contains("<script id=\"zerocmux-diff-viewer-config\" type=\"application/json\">") && html.contains("background: transparent;"), html)
        XCTAssertTrue(html.contains("<div id=\"root\"></div>"), html)
        XCTAssertTrue(html.contains("<script type=\"module\" src=\"./assets/zerocmux-diff-viewer-app/main.mjs\"></script>"), html)
        ...
        let appAssetDirectory = viewerFileURL.deletingLastPathComponent()
            .appendingPathComponent("assets", isDirectory: true)
            .appendingPathComponent("zerocmux-diff-viewer-app", isDirectory: true)
        ...
        XCTAssertTrue(html.contains("\"dark\":\"zerocmux-ghostty-dark-"), html)
        XCTAssertTrue(html.contains("\"light\":\"zerocmux-ghostty-light-"), html)
        ...
        XCTAssertTrue(files.contains { file in
            file["request_path"] as? String == "/assets/zerocmux-diff-viewer-app/main.mjs" &&
    ...
    func testDiffCommandUsesTaggedSocketAppAssetsWithoutServer() throws {
        ...
        let socketPath = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("zerocmux-debug-\(tag).sock", isDirectory: false)
            .path
        unlink(socketPath)
        let listenerFD = try bindUnixSocket(at: socketPath)
        let rootURL = FileManager.default.temporaryDirectory
        ...
        let appEntry = try XCTUnwrap(files.first { file in
            (file["request_path"] as? String)?.hasSuffix("/assets/zerocmux-diff-viewer-app/main.mjs") == true
        })
        ...
        XCTAssertEqual(URL(string: rawURL)?.scheme, "zerocmux-diff-viewer")
    }
    ...
    func testDiffViewerServerBoundsDeferredWaitRequests() throws {
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-diff-viewer-wait-\(UUID().uuidString)", isDirectory: true)
    ...
    func testDiffCommandKeepsRequestedGitSourceAndLoadsAlternativesOnDemand() throws {
        ...
        try runGit(["config", "user.name", "zerocmux tests"], in: repoURL)
        try runGit(["config", "user.email", "zerocmux@example.invalid"], in: repoURL)
        try "one\n".write(to: fileURL, atomically: true, encoding: .utf8)
    ...
    func testDiffCommandSupportsGitSourcesAndSurfaceScopedLastTurn() throws {
        ...
        try runGit(["config", "user.name", "zerocmux tests"], in: siblingRepoURL)
        try runGit(["config", "user.email", "zerocmux@example.invalid"], in: siblingRepoURL)
    ...
    func testAgentTurnDiffBaselineStoresUntrackedSnapshotsOutsideGit() throws {
        ...
        let untrackedRefs = try runGitStdout(
            ["for-each-ref", "--format=%(refname)", "refs/zerocmux/last-turn/untracked"],
            in: repoURL
        )
    ...
    private func diffViewerConfig(from html: String) throws -> [String: Any] {
        let marker = "<script id=\"zerocmux-diff-viewer-config\" type=\"application/json\">"
