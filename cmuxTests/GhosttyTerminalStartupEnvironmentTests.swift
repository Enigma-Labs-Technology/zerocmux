...
struct GhosttyTerminalStartupEnvironmentTests {
    ...
    func testInstallClaudeCommandShimCreatesExecutableOutsideBundleBin() throws {
        ...
        let bundleBin =
            root
            .appendingPathComponent("zerocmux.app", isDirectory: true)
        ...
        let wrapperURL = bundleBin.appendingPathComponent("zerocmux-claude-wrapper", isDirectory: false)
        try """
        #!/usr/bin/env bash
        set -euo pipefail
        {
    ...
    func testClaudeCommandShimFallsBackToCurrentBundledWrapperWhenEmbeddedWrapperWasReaped() throws {
        ...
        let staleWrapperURL = oldBundleBin.appendingPathComponent("zerocmux-claude-wrapper", isDirectory: false)
        try writeExecutable(staleWrapperURL, """
        #!/usr/bin/env bash
        printf 'stale %s\\n' "$*" > "$CMUX_TEST_LOG"
        """)

        let currentWrapperURL = currentBundleBin.appendingPathComponent("zerocmux-claude-wrapper", isDirectory: false)
        try writeExecutable(currentWrapperURL, """
        #!/usr/bin/env bash
        printf 'current %s\\n' "$*" > "$CMUX_TEST_LOG"
        """)
        let currentCLIURL = currentBundleBin.appendingPathComponent("zerocmux", isDirectory: false)
    ...
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvCMUXKeys() {
        let replayPath = "/tmp/zerocmux-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
    ...
    func testMergedStartupEnvironmentDoesNotDeriveHermesCodexBaseURLForGenericTerminals() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-hermes-codex-startup-\(UUID().uuidString)", isDirectory: true)
