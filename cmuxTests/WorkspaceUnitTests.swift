...
final class KeyboardShortcutSettingsFileStoreTests: XCTestCase {
    ...
    func testSettingsFileStoreUsesLegacyFallbackWhenCanonicalConfigHasNoSetting() throws {
        ...
        try writeSettingsFile(
            """
            {
              "$schema": "https://raw.githubusercontent.com/kernelalex/zerocmux/main/web/data/zerocmux-settings.schema.json",
              "shortcuts": {
    ...
    func testReloadConfigurationMenuActionReloadsRegisteredCmuxConfigStore() throws {
        ...
        let appMenuItem = NSMenuItem(title: "zerocmux", action: nil, keyEquivalent: "")
        let appMenu = NSMenu(title: "zerocmux")
    ...
    func testBootstrapCreatesCommentedTemplateWhenPrimaryAndFallbackAreMissing() throws {
        ...
        XCTAssertTrue(contents.contains(#""$schema": "https://raw.githubusercontent.com/kernelalex/zerocmux/main/web/data/cmux.schema.json""#))
...
final class WorkspaceCreationWorkingDirectoryInheritanceTests: XCTestCase {
    ...
    func testExplicitWorkspaceWorkingDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/zerocmux-source-\(UUID().uuidString)"
            let explicitCwd = "/tmp/zerocmux-explicit-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceTransferDirectoryWinsWhenInheritanceIsDisabled() throws {
        try withWorkspaceWorkingDirectoryInheritanceSetting(false) {
            let sourceCwd = "/tmp/zerocmux-source-\(UUID().uuidString)"
            let transferCwd = "/tmp/zerocmux-detached-\(UUID().uuidString)"
            let manager = TabManager(
                initialWorkingDirectory: sourceCwd,
                autoWelcomeIfNeeded: false
            )
    ...
    func testDetachedWorkspaceDoesNotPersistProcessDetectedResumeBinding() throws {
        let manager = TabManager(
            initialWorkingDirectory: "/tmp/zerocmux-source-\(UUID().uuidString)",
            autoWelcomeIfNeeded: false
        )
        let source = try XCTUnwrap(manager.selectedWorkspace)
...
final class WorkspaceCreationConfigSanitizationTests: XCTestCase {
    private final class UnsafeConfigSnapshotTabManager: TabManager {
        ...
        func installInjectedConfig(fontSize: Float) {
            ...
            config.workingDirectory = "/tmp/zerocmux-workspace-snapshot"
...
final class WorkspaceSplitWorkingDirectoryTests: XCTestCase {
    ...
    func testNewTerminalSplitFallsBackToRequestedWorkingDirectoryWhenReportedDirectoryIsStale() {
        ...
        let requestedDirectory = "/tmp/zerocmux-requested-split-cwd-\(UUID().uuidString)"
...
final class WorkspacePanelGitBranchTests: XCTestCase {
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesRemoteStartupCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/zerocmux-fork-remote.sock",
        ...
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/zerocmux/project")
    ...
    func testForkAgentConversationInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/zerocmux-fork-fallback-remote.sock",
                terminalStartupCommand: "ssh cmux-macmini"
            ),
            autoConnect: false
        )
        workspace.currentDirectory = "/Users/zerocmux/fallback repo"
        let sourcePanelId = try XCTUnwrap(workspace.focusedPanelId)
        let snapshot = SessionRestorableAgentSnapshot(
            kind: .codex,
            sessionId: "019dad34-d218-7943-b81a-eddac5c87951",
        ...
        XCTAssertEqual(forkPanel.surface.debugInitialCommand(), "ssh cmux-macmini")
        XCTAssertNil(forkPanel.requestedWorkingDirectory)
        XCTAssertEqual(workspace.panelDirectories[forkPanel.id], "/Users/zerocmux/fallback repo")
        XCTAssertEqual(
            forkPanel.surface.initialInput,
            "cd -- '/Users/zerocmux/fallback repo' 2>/dev/null || [ ! -d '/Users/zerocmux/fallback repo' ] && '/Users/example/.bun/bin/codex' 'fork' '019dad34-d218-7943-b81a-eddac5c87951'\n"
        )
    }
    ...
    func testSessionIndexRemoteSplitDoesNotInjectRemoteStartupCommand() throws {
        ...
        let hookStateRoot = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-session-drop-hook-state-\(UUID().uuidString)", isDirectory: true)
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/zerocmux-session-drop-remote.sock",
                terminalStartupCommand: "ssh cmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspacePreservesRemoteContext() throws {
        let workspace = Workspace()
        let agentSocketPath = "/tmp/zerocmux-fork-agent.sock"
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                destination: "cmux-macmini",
                port: 2222,
                identityFile: "/Users/example/.ssh/zerocmux",
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/zerocmux-fork-remote.sock",
                terminalStartupCommand: "ssh -p 2222 -i /Users/example/.ssh/zerocmux -o ServerAliveInterval=30 -o ForwardAgent=yes -tt cmux-macmini",
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/zerocmux/project")
        XCTAssertNil(launch.terminalWorkingDirectory)
        XCTAssertEqual(
            launch.initialTerminalCommand,
            "ssh -p 2222 -i /Users/example/.ssh/zerocmux -o ServerAliveInterval=30 -o ForwardAgent=yes -tt cmux-macmini"
        )
        ...
        XCTAssertEqual(launch.remoteConfiguration?.identityFile, "/Users/example/.ssh/zerocmux")
    ...
    func testForkAgentWorkspaceLaunchFromPersistentSSHPTYDoesNotReuseParentRelayOrDaemonSlot() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/zerocmux-fork-persistent.sock",
        ...
        XCTAssertEqual(
            startupCommand,
            "ssh -p 2222 -i /Users/example/.ssh/zerocmux -tt cmux-macmini"
        )
    }
    ...
    func testForkAgentWorkspaceLaunchInRemoteWorkspaceUsesFallbackDirectoryInForkCommand() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/zerocmux-workspace-fallback-remote.sock",
                terminalStartupCommand: "ssh cmux-macmini"
            ),
            autoConnect: false
        )
        ...
        XCTAssertEqual(launch.workingDirectory, "/Users/zerocmux/fallback repo")
        XCTAssertNil(launch.terminalWorkingDirectory)
    ...
    func testForkAgentConversationInRemoteConfiguredLocalWorkspaceUsesForkVerb() throws {
        ...
        XCTAssertEqual(forkPanel.surface.initialInput, " zerocmux fork codex 019dad34-d218-7943-b81a-eddac5c87951\n")
        ...
        XCTAssertEqual(launch.terminalWorkingDirectory, "/Users/zerocmux/project")
        XCTAssertNil(launch.initialTerminalCommand)
        XCTAssertFalse(launch.autoConnectRemoteConfiguration)
        XCTAssertNil(launch.remoteConfiguration)
        XCTAssertEqual(launch.initialTerminalInput, " cmux fork codex 019dad34-d218-7943-b81a-eddac5c87951\n")
    ...
    func testForkAgentConversationFromLocalTerminalInRemoteWorkspaceStaysLocal() throws {
        ...
        workspace.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                relayToken: String(repeating: "a", count: 64),
                localSocketPath: "/tmp/zerocmux-fork-local-remote.sock",
                terminalStartupCommand: "ssh cmux-macmini"
            ),
            autoConnect: false
        )
    ...
    func testUpdatingFocusedPanelPullRequestWithSameStateDoesNotRepublishWorkspace() {
        ...
        let pullRequestURL = URL(string: "https://github.com/kernelalex/zerocmux/pull/2388")!
    ...
    func testSidebarDirectoryDisplayLabelUpgradesSharedRowWhileFilesystemVariantKeepsPath() {
        ...
        let sharedDirectory = "/tmp/zerocmux-display-label-shared"
    ...
    func testSidebarDerivedCollectionsMatchWhenUsingPrecomputedPanelOrder() {
        ...
        workspace.updatePanelPullRequest(
            ...
            number: 101,
            label: "PR",
            url: URL(string: "https://github.com/kernelalex/zerocmux/pull/101")!,
            status: .open
        )
        workspace.updatePanelPullRequest(
            ...
            number: 18,
            label: "MR",
            url: URL(string: "https://gitlab.com/manaflow/zerocmux/-/merge_requests/18")!,
            status: .merged
        )
...
final class ExtensionWorktreePrototypeTests: XCTestCase {
    ...
    func testCreateWorktreeKeepsCmuxDirectoryLocallyIgnored() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-worktree-prototype-\(UUID().uuidString)", isDirectory: true)
        ...
        _ = try runGit([
            "-c", "user.name=zerocmux Test",
            "-c", "user.email=zerocmux@example.invalid",
            "commit",
            "-m",
            "initial"
        ], in: projectRoot)
        ...
        XCTAssertTrue(result.workspaceTitle.hasPrefix("zerocmux-sidebar-"))
