...
struct ShellStartupMatrixTests {
    ...
    func zshStartupPreservesUserZdotdirAndLoadsGhosttyIntegration() throws {
        let bundled = try makeBundledIntegrationDir(files: [".zshenv": "# zerocmux zsh bootstrap stub\n"])
    ...
    func bashStartupUsesBundledPromptCommandAndHonorsGhosttyMode() {
        let integrationDir = "/Applications/zerocmux.app/Contents/Resources/shell-integration"
        ...
        _ = TerminalSurface.applyManagedShellSpecificStartupEnvironment(
            ...
            protectedKeys: &protectedKeys,
            readFile: { path in
                expectEqual(path, "\(integrationDir)/zerocmux-bash-bootstrap.bash")
                return """
                # comment
    ...
    func fishStartupReturnsManagedCommandAndPreservesUserConfigHome() throws {
        let bundled = try makeBundledIntegrationDir(files: ["fish/config.fish": "# zerocmux fish bootstrap stub\n"])
        defer { try? FileManager.default.removeItem(at: bundled.root) }
        let integrationDir = bundled.integrationDir
    ...
    func generatedSshBootstrapDoesNotBlockOnRelayCliWarmup() throws {
        ...
        expectTrue(
            ...
            "zerocmux ssh bootstrap waited for relay CLI warmup: \(formatSeconds(result.process.duration))"
        )
    }

    /// Regression for #6352: running Claude Code (or any full-screen TUI) inside
    /// a `zerocmux ssh` remote workspace garbled the output because the remote
    ...
    func remoteTerminalSetupInstallsGhosttyTerminfoBeforeChoosingTerm() throws {
        ...
        guard fileManager.isExecutableFile(atPath: "/usr/bin/tic"),
            ...
            // cannot be exercised here. zerocmux CI runners ship both binaries.
            return
        }

        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-terminfo-\(UUID().uuidString)")
        ...
        let terminfoSource = """
        xterm-ghostty|zerocmux ghostty regression terminfo,
    ...
    private func runGeneratedBootstrap(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent("zerocmux-shell-matrix-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        ...
        if let fakeCmuxDelay {
            try writeExecutableCmuxFile(at: bin.appendingPathComponent("zerocmux"), delay: fakeCmuxDelay)
        }
    ...
    private func writeExecutableShellFile(at url: URL, capturePath: URL) throws {
        ...
        #!/bin/sh
        {
          ...
          if [ -r "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-zsh-integration.zsh" ] && grep -q 'cmux_zsh_marker=1' "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-zsh-integration.zsh"; then
            printf 'ZSH_INTEGRATION_HAS_MARKER=yes\\n'
          fi
          if [ -r "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-bash-integration.bash" ] && grep -q 'cmux_bash_marker=1' "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-bash-integration.bash"; then
    ...
    private func makeBundledIntegrationDir(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-shell-matrix-bundle-\(UUID().uuidString)")
