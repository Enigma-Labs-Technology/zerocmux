...
final class CmuxSSHURLRequestTests: XCTestCase {
    ...
    private var supportedScheme: String {
        CmuxSSHURLRequest.primaryScheme
    }
    ...
    func testCommandPreviewIncludesSocketPathWhenProvided() throws {
        ...
        case .success(.some(let request)):
            XCTAssertEqual(
                request.cliPreview(socketPath: "/tmp/zerocmux-urlcmd.sock"),
                "zerocmux --socket /tmp/zerocmux-urlcmd.sock ssh --name \"Dev SSH\" dev.example.com"
            )
        case .success(nil):
            XCTFail("Expected SSH URL request")
        case .failure(let error):
    ...
    func testParsesStableNightlyAndDevSchemes() throws {
        for scheme in ["zerocmux", "zerocmux-nightly", "zerocmux-dev"] {
            var components = URLComponents()
...
final class CmuxNavigationURLRequestTests: XCTestCase {
    private let supportedScheme = "zerocmux-test"
    ...
    func testIgnoresOtherCmuxRoutesAndInactiveSchemes() throws {
        ...
        let inactiveURL = try XCTUnwrap(URL(string: "zerocmux-other://workspace/\(workspaceId.uuidString)"))
