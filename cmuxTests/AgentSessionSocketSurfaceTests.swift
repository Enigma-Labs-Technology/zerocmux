    ...
    func testWorkspaceSessionSnapshotPersistsAgentSessionWorkingDirectory() throws {
        ...
        let panel = try #require(
            workspace.newAgentSessionSurface(
                ...
                providerID: .codex,
                rendererKind: .react,
                workingDirectory: "/tmp/zerocmux-agent-session-cwd",
                focus: true
            )
        )
        ...
        expectEqual(panelSnapshot.directory, "/tmp/zerocmux-agent-session-cwd")
        expectEqual(panelSnapshot.agentSession?.workingDirectory, "/tmp/zerocmux-agent-session-cwd")
    }
}
