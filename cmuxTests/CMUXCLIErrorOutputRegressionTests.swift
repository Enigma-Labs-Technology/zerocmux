...
@Suite(.serialized) struct CMUXCLIErrorOutputRegressionTests: Sendable {
    ...
    @Test func testBundledCLIInTaggedDebugAppPrefersItsOwnSocketWithoutEnvironmentOverride() throws {
        ...
        let taggedSocketPath = "/tmp/zerocmux-debug-\(tagSlug).sock"
        let home = try makeTemporaryHome()
        defer { try? FileManager.default.removeItem(at: home) }
    ...
    @Test func testBundledCLIInTaggedDebugAppPreservesExplicitStableEnvSocketWhenTaggedSocketIsMissing() throws {
        ...
        let stableSocketURL = fixedHomeURL
            .appendingPathComponent(".local/state/zerocmux", isDirectory: true)
            .appendingPathComponent("zerocmux.sock", isDirectory: false)
    ...
    @Test func testBundledCLIInTaggedDebugAppPreservesExplicitUserScopedStableEnvSocket() throws {
        let cliPath = try bundledCLIPath()
        let fixedHomeURL = URL(fileURLWithPath: "/tmp/zerocmux-cli-home-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: fixedHomeURL) }
        let stableSocketURL = fixedHomeURL
            .appendingPathComponent(".local/state/zerocmux", isDirectory: true)
            .appendingPathComponent("zerocmux-\(getuid()).sock", isDirectory: false)
        let stableSocketPath = stableSocketURL.path
        try FileManager.default.createDirectory(
            at: stableSocketURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
    ...
    @Test func testBundledStableCLIPreservesSymlinkedExplicitEnvSocket() throws {
        ...
        let symlinkedStableSocketPath = socketDirectoryURL
            ...
            .path
        let symlinkTargetSocketPath = "/tmp/zerocmux-symlink-target-\(UUID().uuidString).sock"
        try writeStableSocketMarker(home: fixedHomeURL)
    ...
    @Test func testBundledCLIInTaggedDebugAppPreservesExplicitCustomEnvSocket() throws {
        ...
        let home = try makeTemporaryHome()
        defer { try? FileManager.default.removeItem(at: home) }
        let explicitSocketPath = "/tmp/zerocmux-explicit-legacy-\(UUID().uuidString.lowercased()).sock"
    ...
    @Test func testThemesSetReloadsRunningAppAfterEveryThemeWrite() async throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-themes-socket-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let socketPath = "/tmp/zerocmux-theme-\(UUID().uuidString.prefix(8)).sock"
        ...
        let bundleIdentifier = "com.kernelalex.zerocmux.debug.issue-4355-test."
    ...
    func verifyThemesSetTargetsResolvedTaggedSocketWhenBundleEnvironmentIsStale() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-themes-stale-bundle-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        // `cmux-debug-<slug>.sock` becomes `com.kernelalex.zerocmux.debug.<slug>`, where every run of
        ...
        let socketPath = "/tmp/zerocmux-debug-active-theme-\(uniqueSuffix).sock"
        let staleBundleIdentifier = "com.kernelalex.zerocmux.debug.stale.theme"
        let targetBundleIdentifier = "com.kernelalex.zerocmux.debug.active.theme.\(uniqueSuffix)"
    ...
    @Test func testThemesSetNightlyOverridePathIsReadableByNightlyAppConfigResolution() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-themes-nightly-path-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        // consulted: `cmux-nightly-<slug>.sock` becomes `com.kernelalex.zerocmux.nightly.<slug>`.
        ...
        let uniqueSuffix = UUID().uuidString.lowercased().replacingOccurrences(of: "-", with: "")
        let socketPath = "/tmp/zerocmux-nightly-\(uniqueSuffix).sock"
        let bundleIdentifier = "com.kernelalex.zerocmux.nightly.\(uniqueSuffix)"
        var environment = ProcessInfo.processInfo.environment
        for key in Array(environment.keys) where key.hasPrefix("CMUX_") {
            environment.removeValue(forKey: key)
        }
    ...
    func verifyBareInteractiveThemesReloadsRunningAppAfterPickerExits() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-themes-picker-socket-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let socketPath = "/tmp/zerocmux-theme-picker-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
        let bundleIdentifier = "com.kernelalex.zerocmux.debug.theme-picker.\(UUID().uuidString.lowercased())"
        let reloadExpectation = expectation(description: "bare zerocmux themes posts final reload notification")
        let notificationQueue = OperationQueue()
        notificationQueue.maxConcurrentOperationCount = 1
        let notificationLock = NSLock()
        var observedReloads: [(bundleIdentifier: String?, phase: String?)] = []
    ...
    @Test func testBareInteractiveThemesTreatsSigintAsSilentCancel() throws {
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-themes-picker-cancel-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
        ...
        let socketPath = "/tmp/zerocmux-theme-picker-cancel-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(path: socketPath, response: "OK")
        defer { responder.stop() }
    ...
    @Test func testBrowserDownloadWaitUsesRequestedTimeoutForSocketResponse() throws {
        let cliPath = try bundledCLIPath()
        let socketPath = "/tmp/zerocmux-dw-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testDotPathOpenBypassesProtectedSocketForExternalCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-cli-external-open-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/zerocmux-external-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside zerocmux can connect"
        )
        defer { responder.stop() }
        ...
        environment["CMUX_SOCKET_PATH"] = socketPath
        environment["CMUX_SOCKET"] = "/tmp/zerocmux-stale-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testBareRelativeDirectoryPathOpenBypassesProtectedSocketForExternalCLI() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-cli-bare-open-\(UUID().uuidString)", isDirectory: true)
        let workingDirectory = root.appendingPathComponent("project", isDirectory: true)
        try FileManager.default.createDirectory(at: workingDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let socketPath = "/tmp/zerocmux-bare-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside zerocmux can connect"
        )
    ...
    @Test func testKnownCommandStillUsesSocketWhenMatchingBareRelativePathExists() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-cli-command-path-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(
        ...
        let socketPath = "/tmp/zerocmux-command-path-\(UUID().uuidString.prefix(8)).sock"
    ...
    @Test func testCaseVariantBareRelativeDirectoryPathOpenBypassesProtectedSocket() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-cli-case-path-\(UUID().uuidString)", isDirectory: true)
        ...
        let socketPath = "/tmp/zerocmux-case-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
            response: "ERROR: Access denied — only processes started inside zerocmux can connect"
        )
    ...
    @Test func testExplicitSocketPathOpenUsesRequestedSocket() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-cli-explicit-open-\(UUID().uuidString)", isDirectory: true)
        let workingDirectory = root.appendingPathComponent("project", isDirectory: true)
        try FileManager.default.createDirectory(at: workingDirectory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let socketPath = "/tmp/zerocmux-explicit-open-\(UUID().uuidString.prefix(8)).sock"
        let responder = try UnixSocketResponder(
            path: socketPath,
    ...
    /// developer's real `~/.local/state/zerocmux` (issue #5146).
    ...
    private func stableSocketURL(home: URL) throws -> URL {
        ...
        return directory.appendingPathComponent("zerocmux.sock", isDirectory: false)
    }
    ...
    private func fakeTaggedBundledCLIPath(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-cli-socket-\(UUID().uuidString)", isDirectory: true)
        let appURL = root.appendingPathComponent("zerocmux DEV \(tagSlug).app", isDirectory: true)
        ...
        let info: [String: Any] = [
            "CFBundleIdentifier": bundleIdentifier ?? "com.kernelalex.zerocmux.debug.\(tagSlug.replacingOccurrences(of: "-", with: "."))",
            "CFBundleName": bundleName ?? "zerocmux DEV \(tagSlug)",
            "CFBundlePackageType": "APPL"
        ]
        ...
        let fakeCLIURL = binURL.appendingPathComponent("zerocmux", isDirectory: false)
