...
@Suite(.serialized) struct CLITmuxCompatRemoteSplitTests {
    @Test func splitPrintCarriesPreMutationRemoteRejection() throws {
        ...
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-tmux-compat-print-\(UUID().uuidString)", isDirectory: true)
    ...
    private func respawnPaneForwardedCommand(
        ...
        let cliPath = try BundledCLITestSupport.bundledCLIPath(for: CLITmuxCompatRemoteSplitBundleToken.self)
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-tmux-compat-respawn-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmpDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmpDir) }
    ...
    /// teammate command via `respawn-pane -k -- "cd <dir> && env … <claude> …"`. zerocmux
    ...
    /// that gate on `CLAUDE_CODE_SANDBOXED`. Teammate panes are respawned by zerocmux
    /// (not by `zerocmux claude-teams`), so they do not inherit the launcher env and must
    /// be re-supplied it. Because the gate is a real safety boundary, zerocmux waives it
