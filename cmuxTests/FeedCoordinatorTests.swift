...
struct FeedCoordinatorTests {
    @Test func codexTeamsResolvesExplicitWorkingDirectoryFlags() {
        let base = "/tmp/zerocmux-base"

        #expect(
            CodexTeamsApprovalBridge.resolvedWorkingDirectory(
                commandArgs: ["-C", "child", "prompt"],
                baseDirectory: base
            ) == "/tmp/zerocmux-base/child"
        )
        #expect(
            CodexTeamsApprovalBridge.resolvedWorkingDirectory(
                commandArgs: ["--cwd=/tmp/zerocmux-review", "--cd", "/tmp/zerocmux-final"],
                baseDirectory: base
            ) == "/tmp/zerocmux-final"
        )
        #expect(
    ...
    @Test func codexTeamsValidatesExplicitWorkingDirectoryExists() throws {
        let existing = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-teams-cwd-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func codexAppServerApprovalBuildsActionableFeedEvent() throws {
        let event = CodexTeamsApprovalBridge.feedEvent(
            ...
            params: [
                ...
                "command": "touch /tmp/zerocmux-security-review",
                "cwd": "/tmp/project",
                ...
                "commandActions": [[
                    "type": "write",
                    "path": "/tmp/zerocmux-security-review",
        ...
        #expect(toolInput["command"] as? String == "touch /tmp/zerocmux-security-review")
