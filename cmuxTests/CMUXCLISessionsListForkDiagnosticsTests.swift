...
extension CMUXCLIErrorOutputRegressionTests {
    @Test func testSessionsListReportsForkDiagnosticsAndAcceptsWorkspaceRefs() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-fork-\(UUID().uuidString)", isDirectory: true)
            ...
            "sessions": [
                sessionId: [
                    ...
                    "launchCommand": [
                        "launcher": "codex",
                        "arguments": [],
                        "workingDirectory": "/tmp/zerocmux/debug",
                        "environment": [
    ...
    @Test func testSessionsListForkStartupInputAvailabilityCanDifferFromForkSupport() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-fork-startup-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
    ...
    @Test func testSessionsListIgnoresUntrustedLaunchCaptureForStartupInput() throws {
        ...
        let session = try sessionsListDiagnosticSession(
            ...
            arguments: ["/usr/local/bin/zerocmux", "claude-teams"],
    ...
    @Test func testSessionsListDoesNotReportLiveMatchingPIDAsStaleRestoreBlocker() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-live-pid-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let codexHome = root.appendingPathComponent(".codex", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
    ...
    func sessionsListDiagnosticSession(
        ...
        environment: [String: String] = [:], workingDirectory: String = "/tmp/zerocmux/debug", pid: Int? = nil,
        transcriptPath: String? = nil
    ) throws -> [String: Any] {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-codex-diag-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        let sessionId = "019ef275-74e3-7777-9773-9dcb118ed5aa"
    ...
    @Test func testSessionsListForkStartupInputCountsSelectedEnvironment() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-fork-env-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        try FileManager.default.createDirectory(at: stateDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    @Test func testSessionsListDoesNotReportLocalOpenCodeForkSupportedWithoutVersionProbe() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-opencode-fork-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
    ...
    @Test func testSessionsListDoesNotExecuteCapturedOpenCodeProbeCommand() throws {
        let cliPath = try bundledCLIPath()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-sessions-list-opencode-probe-\(UUID().uuidString)", isDirectory: true)
        let stateDir = root.appendingPathComponent("state", isDirectory: true)
        let repoDir = root.appendingPathComponent("repo", isDirectory: true)
