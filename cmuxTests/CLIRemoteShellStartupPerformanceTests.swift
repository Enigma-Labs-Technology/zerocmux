...
struct CLIRemoteShellStartupPerformanceTests {
    ...
    func generatedSSHStartupDoesNotBlockOnRelayRPCWarmup() throws {
        ...
        environment["CMUX_BUNDLED_CLI_PATH"] = root.bin.appendingPathComponent("zerocmux").path
        environment["CMUX_SOCKET_PATH"] = "/tmp/zerocmux-test.sock"
    ...
    private func makeFakeRemoteShellRoot() throws -> FakeRemoteShellRoot {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("zerocmux-cli-shell-perf-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        let bin = root.appendingPathComponent("bin")
        try FileManager.default.createDirectory(at: home, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: bin, withIntermediateDirectories: true)
        try writeExecutable(at: bin.appendingPathComponent("zerocmux"), contents: """
        #!/bin/sh
    ...
    private func bundledCLIPath() throws -> String {
        ...
        let direct = appBundle.appendingPathComponent("Contents/Resources/bin/zerocmux")
        ...
        throw NSError(domain: "cmux.tests", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Bundled zerocmux CLI not found in \(appBundle.path)",
        ])
    }
