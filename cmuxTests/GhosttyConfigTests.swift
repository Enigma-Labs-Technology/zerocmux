...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testLaunchGhosttyResourcesPreferCurrentBundleOverInheritedEnvironment() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-launch-resources-\(UUID().uuidString)")
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleHasNoResources() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-launch-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesKeepInheritedEnvironmentWhenBundleLacksThemes() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-launch-incomplete-resource-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLaunchGhosttyResourcesUseIncompleteBundleOnlyAsLastFallback() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-launch-incomplete-resource-last-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testThemeSearchPathsIncludeCmuxUserThemesDirectory() {
        ...
        XCTAssertTrue(
            paths.contains(
                "\(NSHomeDirectory())/Library/Application Support/com.kernelalex.zerocmux/themes/Zag Light"
            )
        )
    }

    func testThemeSearchPathsIncludeCmuxUserThemesDirectoryFromFixedHome() {
        let fixedHome = "/tmp/zerocmux-fixed-home-\(UUID().uuidString)"
        let paths = GhosttyConfig.themeSearchPaths(
            forThemeName: "Zag Light",
        ...
        XCTAssertTrue(
            paths.contains(
                "\(fixedHome)/Library/Application Support/com.kernelalex.zerocmux/themes/Zag Light"
            )
        )
    }
    ...
    func testThemesListIncludesCmuxUserThemesDirectory() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-user-theme-list-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }

        let themesDirectory = root
            .appendingPathComponent("Library/Application Support/com.kernelalex.zerocmux/themes", isDirectory: true)
    ...
    func testLoadReadsBackgroundFromRecursiveConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-config-recursive-background-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadDoesNotReparseTopLevelConfigReferencedByConfigFile() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-config-top-level-cycle-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadAllowsRecursiveConfigFileToReloadTopLevelConfigAsFinalOverride() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-config-top-level-reload-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadReadsOptionalQuotedConfigFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-config-optional-quoted-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadIgnoresLegacyAppSupportConfigWhenConfigGhosttyIsNonEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-app-support-legacy-skip-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadUsesLegacyAppSupportConfigWhenConfigGhosttyIsEmpty() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-app-support-legacy-fallback-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testLoadThemeReadsAbsoluteThemeFilePath() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-ghostty-absolute-theme-\(UUID().uuidString)")
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testUnparsedAppearanceFallbackIgnoresNativeLegacyBaselineWhenCurrentConfigExists() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-test-native-legacy-baseline-\(UUID().uuidString)")
    ...
    func testUnparsedAppearanceFallbackKeepsNativeLegacyBaselineWhenCurrentConfigIsMissingOrEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-test-native-legacy-baseline-empty-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/.local/state/zerocmux/remote-daemons/0.62.0/linux-arm64/"))
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        let sourceBrowser = try XCTUnwrap(source.newBrowserSurface(
            inPane: sourcePaneId,
            focus: false,
            preferredProfileID: defaultProfileID
        ))
        ...
        let destinationBrowser = try XCTUnwrap(destination.newBrowserSurface(
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        let sourcePaneId = try XCTUnwrap(source.bonsplitController.allPaneIds.first)
        let movedBrowser = try XCTUnwrap(source.newBrowserSurface(
            inPane: sourcePaneId,
            focus: false,
            preferredProfileID: defaultProfileID
        ))
        let remainingRemoteBrowser = try XCTUnwrap(source.newBrowserSurface(
            inPane: sourcePaneId,
            focus: false,
            preferredProfileID: defaultProfileID
        ))
...
final class WorkspaceRemoteConfigurationTransportKeyTests: XCTestCase {
    ...
    func testProxyBrokerTransportKeyIgnoresEphemeralAgentSocketPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/zerocmux-a.sock",
            terminalStartupCommand: "ssh cmux-macmini",
            agentSocketPath: "/tmp/zerocmux-agent-a.sock"
        )
        let second = WorkspaceRemoteConfiguration(
            destination: "cmux-macmini",
            port: 22,
            ...
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/zerocmux-b.sock",
            terminalStartupCommand: "ssh cmux-macmini",
            agentSocketPath: "/tmp/zerocmux-agent-b.sock"
        )
...
final class SocketControlSettingsTests: XCTestCase {
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "CMUX_TAG": "stray-tag",
                "CMUX_SOCKET_PATH": "/tmp/zerocmux-debug-issue-153-tmux-compat.sock",
            ],
    ...
    func testTaggedDebugLaunchUsesTagDefaultWhenNoOverrideIsProvided() {
        ...
        XCTAssertEqual(path, "/tmp/zerocmux-debug-my-tag.sock")
    }
    ...
    func testTaggedDebugLaunchStillHonorsSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "CMUX_TAG": "my-tag",
                "CMUX_SOCKET_PATH": "/tmp/zerocmux-debug-forced.sock",
            ],
            bundleIdentifier: "com.kernelalex.zerocmux.debug",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/zerocmux-debug-forced.sock")
    }
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        ...
        XCTAssertEqual(path, "/tmp/zerocmux-nightly.sock")
    }

    func testTaggedDebugBundleKeepsMatchingSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "CMUX_SOCKET_PATH": "/tmp/zerocmux-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.kernelalex.zerocmux.debug.my-tag",
            isDebugBuild: false
        )
    ...
    func testTaggedDebugBundleIgnoresSocketOverrideInheritedFromDifferentCmuxBundle() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "CMUX_BUNDLE_ID": "com.kernelalex.zerocmux.nightly",
                "CMUX_SOCKET_PATH": "/tmp/zerocmux-nightly.sock",
            ],
            bundleIdentifier: "com.kernelalex.zerocmux.debug.issue.4355.cmux.themes.set.state.dependent",
            isDebugBuild: true
        )

        XCTAssertEqual(path, "/tmp/zerocmux-debug-issue-4355-zerocmux-themes-set-state-dependent.sock")
    }
    ...
    func testTaggedDebugBundleIgnoresMismatchedInheritedSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.kernelalex.zerocmux.debug.fix-grok-notifications",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/zerocmux-debug-fix-grok-notifications.sock")
    }
    ...
    func testTaggedDebugBundleRefusesStableSocketOverrideEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.kernelalex.zerocmux.debug.sockguard",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/zerocmux-debug-sockguard.sock")
    }
    ...
    func testTaggedDebugBundleRefusesUserScopedStableSocketOverrideEvenWithOptInFlag() {
        ...
        for alias in aliases {
            ...
            XCTAssertEqual(path, "/tmp/zerocmux-debug-sockguard.sock", alias)
        }
    }
    ...
    func testTaggedDebugBundleRefusesCanonicalLegacyStableSocketAliasEvenWithOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "CMUX_SOCKET_PATH": "/private/tmp/zerocmux.sock",
                "CMUX_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.kernelalex.zerocmux.debug.sockguard",
            isDebugBuild: false
    ...
    func testTaggedDebugBundleRefusesCaseVariantStableSocketAliasesEvenWithOptInFlag() {
        let aliases = [
            ...
            SocketControlSettings.userScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "zerocmux-501.sock", with: "CMUX-501.sock"),
            SocketControlSettings.legacyUserScopedStableSocketPath(currentUserID: 501)
                .replacingOccurrences(of: "zerocmux-501.sock", with: "CMUX-501.sock"),
        ]
    ...
    func testTaggedDebugBundleRefusesLeafSymlinkToStableSocketEvenWithOptInFlag() throws {
        let alias = "/tmp/zerocmux-stable-alias-\(UUID().uuidString).sock"
    ...
    func testTaggedDebugBundleRefusesExcessiveSymlinkChainEvenWithOptInFlag() throws {
        let root = "/tmp/zerocmux-stable-chain-\(UUID().uuidString)"
    ...
    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "CMUX_SOCKET_PATH": "/tmp/zerocmux-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.kernelalex.zerocmux.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/zerocmux-staging-my-tag.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.kernelalex.zerocmux.nightly.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/zerocmux-nightly-tag.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.kernelalex.zerocmux.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/zerocmux-staging-tag.sock"
        )
    }
    ...
    func testInitialStableLaunchTreatsPrivateTmpLegacyStableAliasAsStablePath() {
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
            preferredPath: "/private/tmp/zerocmux.sock",
            bundleIdentifier: "com.kernelalex.zerocmux",
            isDebugBuild: false,
            currentUserID: 501,
    ...
    func testLegacyCmuxBundleIdentifiersKeepLegacySocketNamespace() {
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
    ...
    func testInitialSocketPathDoesNotProbeForTaggedDebugBuild() {
        let debugPath = "/tmp/zerocmux-debug-tag.sock"
        let path = SocketControlSettings.initialSocketPathBeforeListenerStart(
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.kernelalex.zerocmux.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
...
final class InitialMainWindowBootstrapLaunchTests: XCTestCase {
    func testUnitTestHostLaunchDoesNotBootstrapTerminalWindow() {
        XCTAssertFalse(
            AppDelegate.shouldBootstrapInitialMainWindowOnLaunch(
                environment: ["XCTestBundlePath": "Contents/PlugIns/cmuxTests.xctest"]
            )
        )
    }

    func testUITestLaunchStillBootstrapsTerminalWindow() {
        XCTAssertTrue(
            AppDelegate.shouldBootstrapInitialMainWindowOnLaunch(
                environment: ["CMUX_UI_TEST_MODE": "1"]
            )
    ...
    func testRegularLaunchBootstrapsTerminalWindow() {
        XCTAssertTrue(AppDelegate.shouldBootstrapInitialMainWindowOnLaunch(environment: [:]))
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    func testLoadedGhosttyConfigScanPathsSkipsNativeLegacyConfigWhenCurrentConfigIsNonEmpty() throws {
        let appSupport = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-test-appearance-app-support-current-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: appSupport, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: appSupport) }
    ...
    // Regression: https://github.com/kernelalex/zerocmux/issues/3459
    // `zerocmux themes set` always encodes the selection with conditional
    ...
    // stay at the white defaults), so zerocmux must inject the resolved plain theme
    ...
    // Regression: https://github.com/kernelalex/zerocmux/issues/3459
    // `zerocmux themes set --light X` encodes `theme = light:X`, which ghostty treats
    // as conditional config. zerocmux injects the resolved plain theme for the
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    ...
    func testBashNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-bash-no-git-watch-\(UUID().uuidString)")
        ...
        let socketPath = root.appendingPathComponent("zerocmux-test.sock", isDirectory: false)
    ...
    func testZshNoGitWatchSkipsHeadTrackingAndPRClear() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-zsh-no-git-watch-\(UUID().uuidString)")
        let repoA = root.appendingPathComponent("repo-a", isDirectory: true)
        let repoB = root.appendingPathComponent("repo-b", isDirectory: true)
        let logPath = root.appendingPathComponent("send.log", isDirectory: false)
        let socketPath = root.appendingPathComponent("zerocmux-test.sock", isDirectory: false)
    ...
    func testZshNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-zsh-no-pr-watch-\(UUID().uuidString)")
        ...
        try writeExecutableScript(
            ...
            printf '2746\\tOPEN\\thttps://github.com/kernelalex/zerocmux/pull/2746\\n'
            """
        )
        let socketFD = try bindUnixSocket(at: socketPath.path)
        defer {
    ...
    func testBashNoPullRequestWatchSkipsLegacyGhPRProbe() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-bash-no-pr-watch-\(UUID().uuidString)")
        let repoURL = root.appendingPathComponent("repo", isDirectory: true)
        let fakeBinURL = root.appendingPathComponent("fake-bin", isDirectory: true)
        let markerURL = root.appendingPathComponent("gh-pr-invoked", isDirectory: false)
        let socketPath = root.appendingPathComponent("zerocmux-test.sock", isDirectory: false)
