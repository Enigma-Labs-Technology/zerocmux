...
/// action. The opener must route the zerocmux config file through the same
...
/// routes to the OS default `.json` handler and ignores the zerocmux setting.
///
...
@Suite struct SidebarWorkspaceGroupConfigOpenerTests {
    @Test func routesConfigFileThroughInjectedOpener() throws {
        let home = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-group-config-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(url.deletingLastPathComponent().lastPathComponent == "zerocmux")
