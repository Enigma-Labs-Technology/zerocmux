...
@Suite struct ClaudeWrapperResumeEnvironmentTests {
    @Test func bundledClaudeWrapperScrubsNestedSessionEnvironmentOnResume() throws {
        ...
        let wrapperURL = repoRoot.appendingPathComponent("Resources/bin/zerocmux-claude-wrapper", isDirectory: false)
        #expect(
            fileManager.isExecutableFile(atPath: wrapperURL.path),
            "Bundled zerocmux-claude-wrapper must exist and be executable for resume environment coverage"
        )
        ...
        let sandbox = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("zerocmux-resume-\(String(UUID().uuidString.prefix(8)))", isDirectory: true)
        ...
        let socketURL = sandbox.appendingPathComponent("zerocmux.sock", isDirectory: false)
        ...
        let fakeCmuxURL = binDir.appendingPathComponent("zerocmux", isDirectory: false)
        try writeExecutable(
            fakeCmuxURL,
            """
            #!/usr/bin/env bash
        ...
        try runWithBoundedWait(process, shellDescription: "zerocmux-claude-wrapper --resume")
