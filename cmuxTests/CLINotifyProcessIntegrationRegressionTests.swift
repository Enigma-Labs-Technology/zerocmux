...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    // https://github.com/kernelalex/zerocmux/issues/6606
    //
    ...
    func testCodexPromptSubmitDoesNotRefreshTerminalLastTurnDiffBaseline() throws {
        ...
        _ = try runGit(["config", "user.name", "zerocmux tests"])
        _ = try runGit(["config", "user.email", "zerocmux@example.invalid"])
    ...
    // MARK: - Forked conversation restore (https://github.com/kernelalex/zerocmux/issues/5908)
    //
    ...
    func testClaudeForkedSessionPromptSubmitRecordsWhileParentTurnActive() throws {
        ...
        XCTAssertEqual(
            ...
            true,
            "The forked session must become restorable so a zerocmux restart resumes the fork, not the parent"
        )
    ...
    func testRightSidebarInvalidCommandValidatesBeforeTargetResolution() throws {
        let cliPath = try bundledCLIPath()
        let missingSocketPath = "/tmp/zerocmux-test-missing-\(UUID().uuidString).sock"
        var environment = ProcessInfo.processInfo.environment
    ...
    func testSSHForwardAgentRepeatedOptionUsesLastValue() throws {
        let run = try runMockedSSH(
            ...
            environmentOverrides: [
                "SSH_AUTH_SOCK": "/tmp/zerocmux-test-agent-\(UUID().uuidString).sock",
            ]
        )
        let createParams = try XCTUnwrap(params(for: "workspace.create", in: run.requests))
        let configureParams = try XCTUnwrap(params(for: "workspace.remote.configure", in: run.requests))
    ...
    func testSSHForwardAgentTildeSocketPathExpandsSocketPath() throws {
        ...
        let tildeSocketPath = "~/.ssh/zerocmux-test-agent.sock"
        let expandedSocketURL = homeURL.appendingPathComponent(".ssh/zerocmux-test-agent.sock")
    ...
    func testSSHPersistentPTYFallsBackWhenForegroundAuthCannotBeReused() throws {
        let cases: [(name: String, arguments: [String])] = [
            ...
            ("local-command", ["--ssh-option", "LocalCommand=echo zerocmux-test"]),
    ...
    func testSSHPTYAttachBridgeEOFWhileSessionRunsPreservesLifecycleForRetry() throws {
        let cliPath = try bundledCLIPath()
    ...
    func testCodexPromptSubmitRebindsRestoredSessionToCurrentCallerSurface() throws {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-rebind-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCodexPromptSubmitWithForeignCmuxEnvDoesNotFallbackToSelectedWorkspace() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-foreign-env-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertFalse(
            ...
            "Foreign zerocmux env must not mutate the selected workspace, saw \(state.commands)"
        )
    }
    ...
    func testCodexPromptSubmitWithForeignCmuxEnvIgnoresStaleMappedSession() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-foreign-mapped-\(UUID().uuidString)", isDirectory: true)
        let foreignWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let foreignSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
        let selectedSurfaceId = "44444444-4444-4444-4444-444444444444"
        ...
        XCTAssertFalse(
            state.commands.contains { $0.contains("set_status codex Running") || $0.contains("notify_target_async") },
            "Foreign zerocmux env must not reuse stale mapped sessions, saw \(state.commands)"
        )
    }
    ...
    func testCodexPromptSubmitWithInvalidSurfaceDoesNotFallbackToFocusedSurface() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-invalid-surface-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
    ...
    func testCodexPromptSubmitWithInvalidMappedWorkspaceDoesNotFallbackToSelectedWorkspace() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-invalid-mapped-\(UUID().uuidString)", isDirectory: true)
        let staleWorkspaceId = "11111111-1111-1111-1111-111111111111"
        let staleSurfaceId = "22222222-2222-2222-2222-222222222222"
        let selectedWorkspaceId = "33333333-3333-3333-3333-333333333333"
    ...
    func testCodexTeamsForkPromptPublishesResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-codex-teams-resume-\(UUID().uuidString)", isDirectory: true)
        let workspaceId = "33333333-3333-3333-3333-333333333333"
        ...
        environment["CMUX_AGENT_LAUNCH_EXECUTABLE"] = "/usr/local/bin/zerocmux"
        ...
        XCTAssertEqual(
            ...
            "cd -- '\(root.path)' 2>/dev/null || [ ! -d '\(root.path)' ] && '/usr/local/bin/zerocmux' 'codex-teams' 'resume' '\(sessionId)' '-c' 'check_for_update_on_startup=false' '--model' 'gpt-5.4' '--sandbox' 'danger-full-access'"
        )
    }
    ...
    func testAgentPromptClearsSurfaceResumeBindingWhenResumeCommandUnavailable() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-agent-resume-unavailable-\(UUID().uuidString)", isDirectory: true)
            ...
            "sessions": [
                sessionId: [
                    ...
                    "launchCommand": [
                        "launcher": "omx",
                        "executablePath": "/usr/local/bin/zerocmux",
                        "arguments": ["/usr/local/bin/zerocmux", "omx", "hud"],
                        "workingDirectory": root.path,
                        "capturedAt": now,
                        "source": "test",
                    ],
    ...
    func testGenericAgentSessionEndClearsMatchingSurfaceResumeBinding() throws {
        ...
        let state = MockSocketServerState()
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-agent-resume-clear-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer {
            Darwin.close(listenerFD)
            unlink(socketPath)
    ...
    private func makeClaudeHookContext(name: String) throws -> ClaudeHookContext {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-\(name)-\(UUID().uuidString)", isDirectory: true)
    ...
    private func makeExistingAgentSocketPath() throws -> String {
        let directory = try makeTemporaryDirectory(prefix: "zerocmux-agent")
...
extension CLINotifyProcessIntegrationRegressionTests {
    ...
    func testTmuxCompatEnvStampsLaunchSurfaceNotFocusedPane() throws {
        ...
        let tmpDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-spawn-id-e2e-\(UUID().uuidString)", isDirectory: true)
