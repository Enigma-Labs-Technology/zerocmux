...
final class CLINotifyProcessIntegrationRegressionTests: XCTestCase {
    ...
    func testNotificationCLIActionsMutateSocketStateAndListExtendedFields() async throws {
        ...
        let windowId = UUID()
        var window: NSWindow?
        var didRegisterWindowContext = false

        defer {
            TerminalController.shared.stop()
            if didRegisterWindowContext {
        ...
        XCTAssertTrue(waitForSocketPing(at: socketPath), "Socket did not answer ping at \(socketPath)")

        var environment = ProcessInfo.processInfo.environment
        environment["CMUX_SOCKET_PATH"] = socketPath

        func waitForSocketPing(at path: String, timeout: TimeInterval = 5.0) -> Bool {
        ...
        func run(_ arguments: [String], timeout: TimeInterval = 60) async -> ProcessRunResult {
        ...
        func registerWindowContextForNotificationOpen() {
            ...
            notificationWindow.identifier = NSUserInterfaceItemIdentifier("cmux.main.\(windowId.uuidString)")
            ...
            didRegisterWindowContext = true
            window = notificationWindow
        }

        func waitForNotificationRead(_ id: UUID, timeout: TimeInterval = 2.0) -> Bool {
            let deadline = Date().addingTimeInterval(timeout)
            while Date() < deadline {
                if store.notifications.first(where: { $0.id == id })?.isRead == true {
                    return true
                }
                RunLoop.current.run(until: Date().addingTimeInterval(0.01))
            }
            return store.notifications.first(where: { $0.id == id })?.isRead == true
        }
        ...
        registerWindowContextForNotificationOpen()
        ...
        XCTAssertTrue(waitForNotificationRead(openNotification.id))
        rows = try notificationRows(from: await run(["list-notifications", "--json", "--id-format", "uuids"]).stdout)
        ...
        XCTAssertTrue(waitForNotificationRead(jumpNotification.id))
        rows = try notificationRows(from: await run(["list-notifications", "--json", "--id-format", "uuids"]).stdout)
