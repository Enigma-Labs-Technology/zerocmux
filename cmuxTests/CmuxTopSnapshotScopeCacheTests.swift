...
// Regression coverage for the zerocmux-scope attribution cache used by system.top.
// See https://github.com/kernelalex/zerocmux/issues/5756.
//
// `.serialized` because these tests share the process-global zerocmux-top scope
...
struct CmuxTopSnapshotScopeCacheTests {
    ...
    // Before the fix, a process with no zerocmux scope was a permanent cache miss, so
    // every system.top poll re-ran the 3-sysctl probe for every non-zerocmux process
    ...
    @Test func negativeScopeResultIsCachedWithinTTL() {
        ...
        #expect(first == nil)
        #expect(second == nil)
        #expect(probeCount == 1, "non-zerocmux process should be probed once per TTL, not on every poll")
    }
    ...
    // attribute a live zerocmux process.
    ...
    // into a zerocmux-scoped command later, must be re-probed once the negative TTL
    // elapses so it is eventually attributed (https://github.com/kernelalex/zerocmux/issues/5756).
    @Test func negativeScopeIsReprobedAfterTTLExpiry() {
        ...
        // Process has since execed into a zerocmux-scoped command; after the TTL the
