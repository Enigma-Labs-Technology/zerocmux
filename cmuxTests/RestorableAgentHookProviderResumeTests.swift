...
extension SocketListenerAcceptPolicyTests {
    ...
    func testAdditionalHookAgentResumeCommandsUseVerifiedCLIResumeFlags() {
        ...
        let copilot = SessionRestorableAgentSnapshot(
            ...
            launchCommand: AgentLaunchCommandSnapshot(
                launcher: "copilot",
                executablePath: "/tmp/zerocmux-agent-upstreams/copilot-install/bin/copilot",
                arguments: [
                    "/tmp/zerocmux-agent-upstreams/copilot-install/bin/copilot",
                    "--model",
                    "gpt-5.4",
                    "--resume=old-session",
                    "--allow-all-tools",
        ...
        XCTAssertEqual(
            ...
            "cd -- '/tmp/copilot repo' 2>/dev/null || [ ! -d '/tmp/copilot repo' ] && 'env' 'COPILOT_HOME=/tmp/copilot home' '/tmp/zerocmux-agent-upstreams/copilot-install/bin/copilot' '--resume' 'copilot-session-123' '--model' 'gpt-5.4' '--allow-all-tools'"
        )
        XCTAssertEqual(
        ...
        XCTAssertEqual(
            kiro.resumeCommand,
            "cd -- '/tmp/kiro repo' 2>/dev/null || [ ! -d '/tmp/kiro repo' ] && 'env' 'KIRO_HOME=/tmp/kiro home' '/Users/example/.cargo/bin/kiro-cli' 'chat' '--resume-id' 'kiro-session-123' '--agent' 'zerocmux' '--trust-tools' 'fs_read,fs_write'"
        )
        XCTAssertEqual(
            grok.resumeCommand,
