...
final class RestorableAgentSessionIndexTests: XCTestCase {
    ...
    func testPanelFallbackUsesLatestHookRecord() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-claude-panel-fallback-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeForkResolvesDriftedCwdViaTranscriptPath() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-claude-fork-drift-path-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeForkResolvesDriftedCwdViaConfigScanWhenTranscriptPathMissing() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-claude-fork-drift-scan-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeResumePrefersLaunchCwdWhenTranscriptUnverifiable() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-claude-fallback-prefers-launch-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testDirectoryNamespacedNonClaudeAgentResolvesDriftToLaunchCwd() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-gemini-drift-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testPiDetectedLatestSessionDoesNotCollapseExactHookRecordsAcrossPanels() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-pi-restore-collapse-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testPiInferredLatestFallbackUsesSameKindPanelHookWhenAnotherKindIsNewer() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-pi-restore-kind-fallback-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testClaudeWorkflowDirectorySessionUsesSiblingJsonlSessionForResume() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-claude-workflow-directory-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testCustomVaultAgentPreservesRuntimeCwdOnRestore() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-custom-cwd-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let launchCwd = root.appendingPathComponent("repo.main", isDirectory: true)
    ...
    func testForkCommandUsesPerAgentVerbAndSessionCwd() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-fork-agents-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
    ...
    func testNonForkAgentsProduceNoForkCommand() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-nofork-agents-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testReplacementRestoresNewestSessionForSurface() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-newest-wins-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testRestoreIsIdempotentAcrossReloads() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-idempotent-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testKilledSessionWithDeadProcessDoesNotRestore() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-killed-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testCrossAgentLaunchCaptureIsDiscardedForResumeAndFork() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-cross-agent-capture-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        // wrapped in `/bin/sh -c '…'` so the resumed session keeps zerocmux hooks (issue #5639).
    ...
    func testShellWrapperArgvCaptureIsDiscardedForResumeAndFork() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-shell-argv-capture-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        // (issue #5639) so the resumed session keeps zerocmux hooks.
        XCTAssertTrue(resume.contains("CMUX_CODEX_WRAPPER_SHIM"), "codex resume must route through the codex wrapper shim; got: \(resume)")
        XCTAssertTrue(resume.contains("resume") && resume.contains(sid), "codex resume must use the resume verb and session id; got: \(resume)")
        let fork = try XCTUnwrap(snapshot.forkCommand)
    ...
    func testWrapperLauncherCaptureStaysTrusted() throws {
        let fm = FileManager.default
        let root = fm.temporaryDirectory
            .appendingPathComponent("zerocmux-wrapper-launcher-\(UUID().uuidString)", isDirectory: true)
        defer { try? fm.removeItem(at: root) }
        let dir = root.appendingPathComponent("repo", isDirectory: true)
        try fm.createDirectory(at: dir, withIntermediateDirectories: true)
        ...
        record["launchCommand"] = [
            ...
            "arguments": ["/usr/local/bin/zerocmux", "codex-teams"],
            "workingDirectory": dir.path,
            "capturedAt": 10,
            "source": "environment",
        ]
        ...
        XCTAssertTrue(
            ...
            "codexTeams capture must keep routing fork through the zerocmux wrapper; got: \(fork)"
        )
    }
