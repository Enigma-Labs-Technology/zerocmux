...
final class MarkdownPanelTests: XCTestCase {
    ...
    func testMarkdownPanelZoomStepsClampAndReset() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-zoom-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownPanelTypographyResetsToConfiguredDefaults() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-typography-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        let fileURL = directoryURL.appendingPathComponent("README.md")
        try "# hello".write(to: fileURL, atomically: true, encoding: .utf8)
        defer { try? fileManager.removeItem(at: directoryURL) }
    ...
    func testExternalFileOpenRoutesMarkdownFilesToPreviewMarkdownPanel() throws {
        let fileManager = FileManager.default
        let directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-external-open-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directoryURL, withIntermediateDirectories: true)
        defer {
            try? fileManager.removeItem(at: directoryURL)
        }
    ...
    func testMarkdownRenderHandlesLocalImageSources() async throws {
        ...
        let rootURL = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-image-\(UUID().uuidString)", isDirectory: true)
        ...
        XCTAssertTrue((localImage["currentSrc"] as? String ?? "").hasPrefix("zerocmux-local-image://"))
        ...
        XCTAssertTrue((traversalImage["currentSrc"] as? String ?? "").hasPrefix("zerocmux-local-image://"))
    ...
    func testMarkdownRenderDeniesLocalImageWhenMarkdownPathIsMissing() async throws {
        let fileManager = FileManager.default
        let rootURL = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-missing-path-\(UUID().uuidString)", isDirectory: true)
    ...
    func testMarkdownRenderLoadsSafeDataImage() async throws {
        let markdownURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-data-image-\(UUID().uuidString).md")
    ...
    func testMarkdownRenderBlocksRemoteImagesUntilUserAction() async throws {
        let markdownURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-markdown-remote-image-\(UUID().uuidString).md")
        ...
        let expectedConsentMessage = String(
            ...
            defaultValue: "zerocmux will not contact this image URL until you load this image."
        )
        ...
        XCTAssertTrue((loadingLinkedImage["src"] as? String ?? "").hasPrefix("zerocmux-remote-image://"))
        XCTAssertEqual(loadingLinkedImage["hidden"] as? Bool, true)
        XCTAssertTrue((loadingDuplicateImage["src"] as? String ?? "").hasPrefix("zerocmux-remote-image://"))
        ...
        _ = try await webView.evaluateJavaScript(
            """
            (function() {
              Array.prototype.slice.call(document.querySelectorAll('img[src^="zerocmux-remote-image://"]')).forEach(function(img) {
        ...
        XCTAssertTrue((linkedImage["src"] as? String ?? "").hasPrefix("zerocmux-remote-image://"))
        XCTAssertEqual(linkedImage["hidden"] as? Bool, false)
        XCTAssertTrue((duplicateImage["src"] as? String ?? "").hasPrefix("zerocmux-remote-image://"))
        ...
        XCTAssertTrue((autoLoadingImage["src"] as? String ?? "").hasPrefix("zerocmux-remote-image://"))
