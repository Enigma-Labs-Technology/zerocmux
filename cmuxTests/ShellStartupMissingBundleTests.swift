...
struct ShellStartupMissingBundleTests {
    ...
    func shellIntegrationDirectoryExistsRequiresARealDirectory() throws {
        ...
        let missingDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-tests-deleted-bundle-\(UUID().uuidString)")
    ...
    // zshrc": spawn a real interactive zsh with the environment zerocmux produces
    ...
    func zshStillLoadsUserZshrcWhenBundledIntegrationDirIsDeleted() throws {
        ...
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-zshrc-deleted-bundle-\(UUID().uuidString)")
    ...
    func zshReturnShellLoadsUserZshrcWhenBundledIntegrationDirIsDeleted() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-return-zshrc-deleted-bundle-\(UUID().uuidString)")
        let home = root.appendingPathComponent("home")
        try fileManager.createDirectory(at: home, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: root) }
    ...
    private func makeBundledIntegrationDir(
        ...
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("zerocmux-shell-missing-bundle-\(UUID().uuidString)")
