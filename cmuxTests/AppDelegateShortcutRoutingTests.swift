...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    func testWelcomeWindowSidebarShortcutsUseSharedToggleCommands() {
        ...
        waitUntil(timeout: 5.0, description: "Cmd+Option+B to toggle the Welcome window right sidebar") {
    ...
    func testCmdCtrlWClosesWindowAfterConfirmation() {
        ...
        waitUntil(timeout: 5.0, description: "Cmd+Ctrl+W confirmed close to finish") {
            let closedWindow = self.window(withId: windowId)
            let context = closedWindow.flatMap { appDelegate.contextForMainTerminalWindow($0, reindex: false) }
            return context == nil && !(closedWindow?.isVisible ?? false)
        }
    ...
    func testRemappedCloseTabDoesNotLetCmdWReachGhosttyCloseSurfaceFallback() throws {
        ...
        defer {
            ...
            probeWindow.makeFirstResponder(nil)
    ...
    func testFindShortcutFromTerminalOpensTerminalFind() {
        ...
        waitUntil(timeout: 5.0, description: "terminal search state to open from Cmd+F") {
    ...
    func testWindowSendEventRepairsFocusedTerminalSearchTypingAfterResponderDrift() {
        ...
        waitUntil(timeout: 10.0, description: "terminal search overlay to mount") {
        ...
        window.sendEvent(keyDown)
        waitUntil(timeout: 10.0, description: "typing repair to preserve first terminal search key") {
    ...
    private func withTemporaryShortcut(
        ...
        KeyboardShortcutSettings.setShortcutOverrideForTesting(shortcut ?? action.defaultShortcut, for: action)
        body()
        #else
