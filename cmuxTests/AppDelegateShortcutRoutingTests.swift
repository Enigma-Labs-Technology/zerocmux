...
final class AppDelegateShortcutRoutingTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        originalSettingsFileStore = KeyboardShortcutSettings.installIsolatedTestFileStore(prefix: "zerocmux-shortcut-routing")
    ...
    func testCmdDPropagatesWhenSplitRightShortcutIsCleared() {
        ...
        withTemporaryShortcut(action: .splitRight, shortcut: .unbound) {
            ...
            XCTAssertFalse(
                appDelegate.debugHandleCustomShortcut(event: event),
                "Cleared Cmd+D split shortcut should not be consumed by zerocmux"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
    ...
    func testRemappedCloseTabDoesNotLetCmdWReachGhosttyCloseSurfaceFallback() throws {
        ...
        withTemporaryShortcut(action: .closeTab, shortcut: remappedCloseTab) {
            ...
            XCTAssertFalse(
                ...
                "After Close Tab is remapped, Cmd+W must not match the zerocmux Close Tab action"
            )
            ...
            XCTAssertEqual(
                ...
                1,
                "Remapped-away Cmd+W should reach the terminal as input instead of closing through zerocmux"
            )
            ...
            XCTAssertTrue(
                ...
                "The remapped Cmd+Option+W shortcut should match the zerocmux Close Tab action"
            )
#if DEBUG
            XCTAssertTrue(
                appDelegate.debugHandleShortcutMonitorEvent(event: remappedCmdOptionW),
                "The remapped Cmd+Option+W shortcut should trigger the zerocmux Close Tab action"
            )
#else
            XCTFail("debugHandleShortcutMonitorEvent is only available in DEBUG")
#endif
    ...
    func testGhosttyConfigDoesNotRetainNumberedGotoTabFallback() throws {
        // Regression for https://github.com/kernelalex/zerocmux/issues/5189.
        // zerocmux owns "Select Workspace 1…9" (default ⌘1–9) through KeyboardShortcutSettings.
        // Ghostty's built-in super+1…8 = goto_tab and super+9 = last_tab fallbacks must be
        // unbound — exactly like zerocmux already unbinds super+d / super+w — so the numbered
    ...
    func testRebindingSelectWorkspaceByNumberHonorsNewModifierAndDropsDefault() {
        ...
        // the zerocmux routing layer must drive the numbered shortcut from the configured
        // value, so a rebound modifier selects the workspace and the old ⌘ default no
        // longer routes through zerocmux.
        guard let appDelegate = AppDelegate.shared else {
            XCTFail("Expected AppDelegate.shared")
            return
        }
        ...
        withTemporaryShortcut(action: .selectWorkspaceByNumber, shortcut: rebound) {
            ...
            XCTAssertFalse(
                ...
                "After rebinding Select Workspace 1…9, the old ⌘1 must not be routed by zerocmux"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
            ...
            XCTAssertTrue(
                ...
                "The rebound Ctrl+Option+1 shortcut should be routed by zerocmux"
            )
#else
            XCTFail("debugHandleCustomShortcut is only available in DEBUG")
#endif
    ...
    func testMinimalModeTitlebarPaddingOnlyCancelsHostingSafeArea() {
        XCTAssertEqual(
            ...
            accuracy: 0.5,
            "Standard mode should align terminal content with zerocmux's visual titlebar height even when AppKit reports a taller native titlebar zone"
        )
    ...
    func testBrowserFirstFindShortcutRoutingExcludesAppOwnedFindCommands() {
        ...
        for testCase in cases {
            ...
            XCTAssertFalse(
                shouldRouteBrowserFindCommandEquivalentThroughWebContentFirst(event),
                "\(testCase.name) belongs to zerocmux find routing, not browser-first routing"
            )
        }
    }
    ...
    func testInlineVSCodeCommandPaletteShortcutDoesNotRouteForUntrackedLocalhostPage() {
        ...
        XCTAssertFalse(
            ...
            "A localhost page with a folder query must not steal zerocmux's command palette shortcut unless it is the tracked VS Code serve-web origin"
        )
    }
    ...
    func testInlineVSCodeCommandPaletteShortcutDoesNotRouteUnrelatedShortcut() {
        ...
        XCTAssertFalse(
            ...
            "Only the configured command palette shortcut should bypass zerocmux for inline VS Code"
        )
    }
    ...
    func testWindowPerformKeyEquivalentSuppressesRemappedCmdDStaleMenuShortcut() {
        ...
        withTemporaryShortcut(action: .splitRight, shortcut: remappedSplitRight) {
            XCTAssertFalse(
                probeWindow.performKeyEquivalent(with: event),
                "Remapped Cmd+D should not be consumed by stale zerocmux menu equivalents"
            )
        }
    ...
    func testTextBoxMentionFileSuggestionsUseCommandPaletteSearchIndex() async throws {
        ...
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "zerocmux-textbox-mentions-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testTextBoxMentionFileSuggestionsRefreshCachedMisses() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "zerocmux-textbox-mentions-refresh-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testTextBoxMentionSkillSuggestionsUseTypedDollarTrigger() async throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory.appendingPathComponent(
            "zerocmux-textbox-skills-\(UUID().uuidString)",
            isDirectory: true
        )
        defer { try? fileManager.removeItem(at: root) }
    ...
    func testTextBoxSubmitUsesLocalPreviewPathForClaudeRemoteImage() throws {
        ...
        let remotePath = "/tmp/zerocmux-upload/moon.png"
        let attachment = TextBoxAttachment(
            localURL: previewURL,
    ...
    private func makeTemporaryPNGFile(named name: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-textbox-attachment-\(UUID().uuidString)", isDirectory: true)
