...
extension CLINotifyProcessIntegrationRegressionTests {
    func testVMSSHOpensManagedWorkspaceThroughSharedSSHPath() throws {
        ...
        XCTAssertEqual(result.stdout, "OK workspace=\(workspaceRef) target=zerocmux@gateway.freestyle.sh state=connecting\n")
        ...
        XCTAssertEqual(configureParams["destination"] as? String, "zerocmux@gateway.freestyle.sh")
    ...
    func testVMSSHInfoRemainsPrintOnly() throws {
        ...
        XCTAssertFalse(result.timedOut, result.stderr)
        XCTAssertEqual(result.status, 0, result.stderr)
        XCTAssertTrue(result.stdout.contains("ssh zerocmux@gateway.freestyle.sh -p 2222"), result.stdout)
