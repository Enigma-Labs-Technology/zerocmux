...
"""
    ...
    for label, source in (
        ...
        command_failure = next(
            (payload for payload in diagnostics if payload.get("message") == "zerocmux hook command failed"),
            None,
        )
...
"""
    ...
    if any(payload.get("message") == "zerocmux hook command failed" for payload in ordered_diagnostics):
...
"""
    ...
    dropped = next(
        (payload for payload in deadline_diagnostics if payload.get("message") == "zerocmux feed delivery dropped"),
        None,
    )
