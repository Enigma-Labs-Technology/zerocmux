...
# The control socket, markers, and password live in the non-TCC zerocmux state
# directory (~/.local/state/zerocmux), not Application Support, so the separately
# signed zerocmux CLI can touch them without the macOS "access data from other apps"
# prompt (https://github.com/kernelalex/zerocmux/issues/5146).
_STATE_DIR = os.path.expanduser("~/.local/state/zerocmux")
_STABLE_SOCKET_PATH = os.path.join(_STATE_DIR, "zerocmux.sock")
_LEGACY_STABLE_SOCKET_PATH = "/tmp/zerocmux.sock"
_STABLE_BUNDLE_ID = "com.kernelalex.zerocmux"
_NIGHTLY_BUNDLE_ID = "com.kernelalex.zerocmux.nightly"
_STAGING_BUNDLE_ID = "com.kernelalex.zerocmux.staging"
...
def _last_socket_path_files() -> List[str]:
    ...
    if variant == "nightly":
        ...
        tmp_marker = f"/tmp/zerocmux-nightly-{slug}-last-socket-path" if slug else "/tmp/zerocmux-nightly-last-socket-path"
    elif variant == "staging":
        marker = f"staging-{slug}-last-socket-path" if slug else "staging-last-socket-path"
        tmp_marker = f"/tmp/zerocmux-staging-{slug}-last-socket-path" if slug else "/tmp/zerocmux-staging-last-socket-path"
    elif variant == "dev":
        marker = f"dev-{slug}-last-socket-path" if slug else "dev-last-socket-path"
        tmp_marker = f"/tmp/zerocmux-dev-{slug}-last-socket-path" if slug else "/tmp/zerocmux-dev-last-socket-path"
...
def _variant_socket_candidates() -> List[str]:
    variant, slug = _socket_variant()
    if variant == "nightly":
        return [f"/tmp/zerocmux-nightly-{slug}.sock"] if slug else ["/tmp/zerocmux-nightly.sock"]
    if variant == "staging":
        return [f"/tmp/zerocmux-staging-{slug}.sock"] if slug else ["/tmp/zerocmux-staging.sock"]
    if variant == "dev":
        if slug:
            return [f"/tmp/zerocmux-debug-{slug}.sock"]
...
def _default_socket_path() -> str:
    ...
    if bundle_id == _DEFAULT_DEBUG_BUNDLE_ID:
        ...
        tagged.extend(glob.glob(os.path.join(_STATE_DIR, "zerocmux*.sock")))
        tagged = [
            p for p in tagged
            if os.path.exists(p)
            and os.path.basename(p).startswith("zerocmux-debug-")
        ]
        if tagged:
