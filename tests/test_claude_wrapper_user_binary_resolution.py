...
def test_wrapper_skips_cmux_shims_and_bundled_claude(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-wrapper-resolution-") as td:
        root = Path(td)
        bundle_bin = root / "zerocmux.app" / "Contents" / "Resources" / "bin"
        shim_bin = root / "shim-bin"
        real_bin = root / "real-bin"
        for directory in (bundle_bin, shim_bin, real_bin):
            directory.mkdir(parents=True, exist_ok=True)

        wrapper = bundle_bin / "zerocmux-claude-wrapper"
...
def test_wrapper_skips_inherited_cmux_cli_shim_roots(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-wrapper-inherited-shim-") as td:
        root = Path(td)
        wrapper_bin = root / "wrapper-bin"
        current_shim_root = root / "tmp" / "zerocmux-cli-shims" / "current-surface"
        inherited_shim_root = root / "tmp" / "zerocmux-cli-shims" / "old-surface"
        real_bin = root / "real-bin"
        for directory in (wrapper_bin, current_shim_root, inherited_shim_root, real_bin):
            directory.mkdir(parents=True, exist_ok=True)

        wrapper = wrapper_bin / "zerocmux-claude-wrapper"
        wrapper.write_bytes(WRAPPER.read_bytes())
        wrapper.chmod(0o755)
...
""",
        ...
        if output != "real-claude --version":
            failures.append(f"expected inherited zerocmux shim roots to be skipped, got {output!r}")


def test_shell_integration_does_not_shim_grok(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-grok-wrapper-resolution-") as td:
        root = Path(td)
        real_bin = root / "real-bin"
...
""",
        ...
        shell_commands = [
            [
                ...
                "--norc",
                "-c",
                'source "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-bash-integration.bash"; grok --version',
            ],
            [
                "/bin/zsh",
                "-f",
                "-c",
                'source "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-zsh-integration.zsh"; grok --version',
            ],
        ]
...
def test_shell_integration_preserves_empty_path_components(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-shell-path-components-") as td:
        root = Path(td)
        tmpdir = root / "tmp"
        ...
        shim_root = tmpdir / "zerocmux-cli-shims" / surface_id
        ...
        shell_commands = [
            [
                ...
                'source "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-bash-integration.bash"; '
                'printf "%s\\n" "$PATH"',
            ],
            [
                "/bin/zsh",
                ...
                "-c",
                'PATH="$CMUX_TEST_INPUT_PATH"; '
                'source "$CMUX_SHELL_INTEGRATION_DIR/zerocmux-zsh-integration.zsh"; '
                'printf "%s\\n" "$PATH"',
            ],
        ]
        for argv in shell_commands:
