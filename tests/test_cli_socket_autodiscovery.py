...
def write_marker(home: str, marker_name: str, socket_path: str) -> None:
    app_support = os.path.join(home, ".local", "state", "zerocmux")
...
def temporary_socket_home(prefix: str) -> tempfile.TemporaryDirectory:
    ...
    # roots short because stable sockets live under ~/.local/state/zerocmux.
...
def bundled_cli_for_variant(cli_path: str, root: str, app_name: str, bundle_id: str) -> str:
    ...
    bundled_cli = os.path.join(bin_dir, "zerocmux")
...
def expect_ping_uses_socket(cli_path: str, home: str, socket_path: str, label: str) -> bool:
    ...
    try:
        ...
        print(f"FAIL: invoking {label} zerocmux ping failed: {exc}")
        return False
    finally:
    ...
    if proc.returncode != 0:
        print(f"FAIL: {label} zerocmux ping returned non-zero status")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False

    if proc.stdout.strip() != "PONG":
        print(f"FAIL: {label} zerocmux ping did not use the expected socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def expect_ping_ignores_dev_tag(
    ...
    if proc.stdout.strip() != "PONG":
        print(f"FAIL: {label} zerocmux ping followed CMUX_TAG to the rogue dev socket")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def expect_ping_does_not_use_socket(
    ...
    try:
        ...
        print(f"FAIL: invoking {label} zerocmux ping failed unexpectedly: {exc}")
        return False
    finally:
        server.join(timeout=2.0)
        try:
    ...
    if proc.stdout.strip() == "WRONG":
        print(f"FAIL: {label} zerocmux ping used the stable socket fallback")
        print(f"stdout={proc.stdout!r}")
        print(f"stderr={proc.stderr!r}")
        return False
...
def test_python_client_ignores_unknown_bundle_env() -> bool:
    expected_tagged_debug = "com.kernelalex.zerocmux.debug.variant.test.tag"
    ...
    if actual != "com.kernelalex.zerocmux":
...
def test_python_client_treats_stable_override_as_implicit() -> bool:
    ...
    expected_socket = f"/tmp/zerocmux-debug-{tag}.sock"

    with temporary_socket_home("zerocmux-py-") as home:
        app_support = os.path.join(home, ".local", "state", "zerocmux")
        os.makedirs(app_support, exist_ok=True)
        stable_socket = os.path.join(app_support, "zerocmux.sock")
...
def test_variant_last_socket_markers(cli_path: str) -> bool:
    ...
    rogue_stable_socket = f"/tmp/zerocmux-debug-rogue-stable-{pid}.sock"
    rogue_stable_tag = f"rogue-stable-{pid}"
    rogue_nightly_socket = f"/tmp/zerocmux-debug-rogue-nightly-{pid}.sock"
    rogue_nightly_tag = f"rogue-nightly-{pid}"
    rogue_dev_agent_socket = f"/tmp/zerocmux-debug-rogue-dev-agent-{pid}.sock"
    rogue_dev_agent_tag = f"rogue-dev-agent-{pid}"

    with temporary_socket_home("zerocmux-home-") as home, \
            tempfile.TemporaryDirectory(prefix="zerocmux-cli-variant-apps-") as apps:
