#!/usr/bin/env python3
"""
Regression tests for OMO subagent panes through zerocmux's tmux compatibility shim.
"""
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="zerocmux-omo-respawn-") as td:
            tmp = Path(td)
            socket_path = tmp / "fake-zerocmux.sock"
            state = FakeCmuxState()
