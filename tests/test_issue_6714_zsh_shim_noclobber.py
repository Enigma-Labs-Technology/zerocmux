#!/usr/bin/env python3
"""
Regression coverage for https://github.com/kernelalex/zerocmux/issues/6714.

When a user enables ``setopt noclobber`` in their interactive zsh, zerocmux's shell
integration prints a spurious error on startup:

    _cmux_install_cli_command_shim:13: file exists: \
        /var/folders/.../T//zerocmux-cli-shims/<surface-id>/claude

Root cause: ``Resources/shell-integration/zerocmux-zsh-integration.zsh`` writes a
...
The fix is zsh's explicit clobber redirection (``>|``) for this zerocmux-owned
...
INTEGRATION = REPO_ROOT / "Resources/shell-integration/zerocmux-zsh-integration.zsh"

# Drive the real shim writer twice under noclobber. The first call creates the
# shim; the second must overwrite the existing file (zerocmux owns it) without
...
def test_zsh_shim_refresh_is_silent_and_refreshes_under_noclobber() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-6714-") as td:
        tmp = Path(td)
        ...
        assert "file exists" not in proc.stderr.lower(), (
            "zerocmux printed a noclobber 'file exists' error while refreshing its own "
        ...
        shim_path = tmp / "zerocmux-cli-shims" / "issue-6714-shim" / "claude"
        ...
        assert f'cmux_wrapper="{wrapper_b}"' in contents, (
            "zerocmux did not refresh its generated shim on the second write under "
...
if __name__ == "__main__":
    ...
    print("PASS: zerocmux refreshes its zsh CLI shim silently under noclobber")
