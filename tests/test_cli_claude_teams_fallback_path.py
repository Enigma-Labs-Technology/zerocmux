#!/usr/bin/env python3
"""
Regression test: `zerocmux claude-teams` preserves fallback provider dirs in PATH.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-teams-fallback-path-") as td:
        tmp = Path(td)
        home = tmp / "home"
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `zerocmux claude-teams --version` failed with Claude in a fallback dir")
    ...
    print("PASS: zerocmux claude-teams preserves fallback provider dirs in PATH")
    return 0
