...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-omp-extension-") as td:
        ...
        shared_pi_extension = shared_agent_dir / "extensions" / "zerocmux-session.ts"
        shared_pi_extension.parent.mkdir(parents=True)
        shared_pi_extension.write_text("// zerocmux-pi-session-extension-marker v1\n", encoding="utf-8")
        ...
        # Pi's different zerocmux-session.ts filename in the same extensions folder.
        ...
        extension_path = shared_agent_dir / "extensions" / "zerocmux-omp-session.ts"
...
"""
        ...
        config_extension_path = config_override / "agent" / "extensions" / "zerocmux-omp-session.ts"
