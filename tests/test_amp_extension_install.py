#!/usr/bin/env python3
"""
Regression test: the generated Amp plugin is importable and emits zerocmux hook calls.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-amp-extension-") as td:
        root = Path(td)
        ...
        extension_path = root / ".config" / "amp" / "plugins" / "zerocmux-session.ts"
        if not extension_path.exists():
            print(f"FAIL: expected plugin at {extension_path}")
            return 1
        extension_text = extension_path.read_text(encoding="utf-8")
        if "zerocmux-amp-session-extension-marker" not in extension_text:
...
"""
    ...
    print("PASS: generated Amp plugin installs and emits zerocmux hooks")
    return 0
