#!/usr/bin/env python3
"""
Regression coverage for https://github.com/kernelalex/zerocmux/issues/5164.

Using Starship as the bash prompt inside a zerocmux session breaks: Starship's
status line stops updating after the first command. The root cause is the local
macOS bash bootstrap that zerocmux injects as ``PROMPT_COMMAND`` (see
``Resources/shell-integration/zerocmux-bash-bootstrap.bash``). The user's startup
...
BOOTSTRAP = REPO_ROOT / "Resources/shell-integration/zerocmux-bash-bootstrap.bash"
...
# zerocmux does via the environment), let the user's rc append starship_precmd, then
# render three prompts (with a cd in between) by evaluating PROMPT_COMMAND the
# way bash itself does.
DRIVER = r"""
set +e

# bash inherits the zerocmux bootstrap as PROMPT_COMMAND from the environment. The
...
def test_starship_precmd_survives_under_cmux_bash_bootstrap() -> None:
    ...
    # zerocmux must compose, not clobber: after the one-shot bootstrap runs, the
    ...
    for i in (1, 2, 3):
        ...
        assert "starship_precmd" in pc, (
            ...
            f"zerocmux took exclusive ownership instead of composing. PROMPT_COMMAND=<{pc}>"
            + debug
        )
        assert "_cmux_prompt_command" in pc, (
            f"zerocmux's own prompt hook missing from PROMPT_COMMAND at prompt {i}: <{pc}>"
            + debug
        )
...
def test_plain_bash_bootstrap_installs_cmux_prompt_command() -> None:
    ...
    for i in (1, 2, 3):
        ...
        # stays green if zerocmux-bash-integration.bash ever tweaks PROMPT_COMMAND.
        assert "_cmux_prompt_command" in pc, (
...
if __name__ == "__main__":
    ...
    print("PASS: zerocmux bash bootstrap composes with (and without) user prompt hooks")
