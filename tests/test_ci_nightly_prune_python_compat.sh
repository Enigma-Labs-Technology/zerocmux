...
release = module.load_release("kernelalex/zerocmux", "nightly")
assert release == {"assets": []}
module.delete_assets("kernelalex/zerocmux", [module.ReleaseAsset(asset_id=123, name="old.dmg", build=1)])
assert requests == [
    ("GET", "https://api.example.test/repos/kernelalex/zerocmux/releases/tags/nightly"),
    ("DELETE", "https://api.example.test/repos/kernelalex/zerocmux/releases/assets/123"),
]
...
module.delete_assets("kernelalex/zerocmux", [module.ReleaseAsset(asset_id=456, name="older.dmg", build=1)])
assert gh_calls == [
    ["gh", "api", "-X", "DELETE", "repos/kernelalex/zerocmux/releases/assets/456"],
]
PY
