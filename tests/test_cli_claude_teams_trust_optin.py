#!/usr/bin/env python3
"""
Regression test (#6447): `zerocmux claude-teams` grants the Claude trust-gate bypass
...
def run(cli_path: str, args: list[str], extra_env: dict[str, str]) -> tuple[str, str]:
    with tempfile.TemporaryDirectory(prefix="zerocmux-teams-trust-") as td:
        tmp = Path(td)
        real_bin = tmp / "bin"
        real_bin.mkdir()
...
def main() -> int:
    ...
    # Opted in: both the Claude env var and the zerocmux propagation marker are set.
    ...
    print("PASS: zerocmux claude-teams gates the trust bypass on the current opt-in and clears ambient markers")
    return 0
