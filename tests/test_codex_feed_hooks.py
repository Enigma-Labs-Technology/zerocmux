...
def test_codex_stop_reaps_transcript_monitor(cli_path: str, root: Path) -> None:
    socket_path = root / "zerocmux-monitor.sock"
...
def test_codex_stop_without_turn_keeps_session_wide_monitor(cli_path: str, root: Path) -> None:
    socket_path = root / "zerocmux-monitor-session-wide.sock"
...
def test_codex_prompt_submit_starts_monitor_when_lease_write_fails(cli_path: str, root: Path) -> None:
    socket_path = root / "zerocmux-monitor-lease-failure.sock"
...
def test_codex_monitor_exits_when_workspace_has_no_surfaces(cli_path: str, root: Path) -> None:
    socket_path = root / "zerocmux-monitor-empty-surfaces.sock"
...
def test_codex_monitor_survives_transient_owner_rpc_timeout(cli_path: str, root: Path) -> None:
    socket_path = root / "zerocmux-monitor-timeout.sock"
...
def cmux_codex_hook_command(subcommand: str) -> str:
    ...
    return (
        ...
        'then cmux_cli="$(command -v zerocmux 2>/dev/null || true)"; fi; if [ -n "$CMUX_SURFACE_ID" ] '
...
def test_install_adds_codex_permission_request_hook(cli_path: str, root: Path) -> None:
    ...
    if not expected_trust:
        raise AssertionError(f"expected zerocmux Codex trust entries, got {expected_trust!r}")
...
def test_install_preserves_codex_hook_position_with_third_party_hooks(cli_path: str, root: Path) -> None:
    ...
    if first_command != cmux_pre_tool:
        raise AssertionError(f"zerocmux hook did not keep its existing position: {groups!r}")
    if second_command != orca_hook:
        raise AssertionError(f"third-party hook was not preserved after zerocmux hook: {groups!r}")
...
def test_install_deduplicates_interleaved_codex_hook_positions(
    ...
    if commands != expected:
        raise AssertionError(f"interleaved zerocmux hook dedupe changed: {commands!r}")
...
def test_install_collapses_consecutive_codex_hook_positions(cli_path: str, root: Path) -> None:
    ...
    if commands != expected:
        raise AssertionError(f"consecutive zerocmux hooks were not collapsed: {commands!r}")
...
def test_install_replaces_legacy_codex_hook_commands(cli_path: str, root: Path) -> None:
        ...
        json.dumps(
            {
                "hooks": {
                    "Stop": [
                        {"hooks": [{"type": "command", "command": "zerocmux codex-hook stop"}]},
                    ],
                    "PreToolUse": [
                        {
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": "zerocmux feed-hook --source codex --event PreToolUse",
                                }
                            ]
                        },
                    ],
    ...
    if any("zerocmux codex-hook" in command or "zerocmux feed-hook --source" in command for command in commands):
        raise AssertionError(f"legacy zerocmux hook commands were not removed: {commands!r}")
...
def test_install_codex_hooks_only_edits_real_features_table(cli_path: str, root: Path) -> None:
    ...
    if lines[features_index + 1] != "# zerocmux-codex-hooks-feature-78f1e4ba-66df-4d35-93c1-67fdf1cbb7df begin":
        raise AssertionError(f"zerocmux marker should be inserted into [features]: {config_toml!r}")
...
def test_uninstall_restores_disabled_codex_hooks_feature(cli_path: str, root: Path) -> None:
    ...
    if _toml_has_line(config_toml, "hooks = true"):
        raise AssertionError(f"zerocmux-owned hooks feature was not removed: {config_toml!r}")
    if "apps = true" not in config_toml:
        raise AssertionError(f"existing feature setting was not preserved: {config_toml!r}")
...
def test_uninstall_restores_disabled_dotted_codex_hooks_feature(cli_path: str, root: Path) -> None:
    ...
    if _toml_has_line(config_toml, "features.hooks = true"):
        raise AssertionError(f"zerocmux-owned dotted hooks feature was not removed: {config_toml!r}")
    if "features.apps = true" not in config_toml:
        raise AssertionError(f"existing dotted feature setting was not preserved: {config_toml!r}")
...
def test_uninstall_removes_cmux_owned_codex_hooks_feature(cli_path: str, root: Path) -> None:
    ...
    if "hooks.state" in config_toml or "trusted_hash" in config_toml:
        raise AssertionError(f"zerocmux-owned hook trust was not removed: {config_toml!r}")
...
def test_uninstall_preserves_unowned_hook_trust_when_cmux_marker_is_unclosed(
    ...
    (codex_home / "config.toml").write_text(
        "[features]\n"
        "hooks = true\n"
        "# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 begin\n"
        "[hooks.state.\"/tmp/zerocmux/hooks.json:pre_tool_use:0:0\"]\n"
        'trusted_hash = "sha256:zerocmux"\n'
    ...
    config_toml = (codex_home / "config.toml").read_text(encoding="utf-8")
    if "# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 begin" in config_toml:
        raise AssertionError(f"orphaned zerocmux hook trust marker was preserved: {config_toml!r}")
...
def test_install_recovers_hook_trust_when_cmux_marker_is_unclosed(
    ...
    config_toml = (codex_home / "config.toml").read_text(encoding="utf-8")
    if "approved zerocmux hooks" not in result.stdout:
        raise AssertionError(f"install did not report recovered hook trust approval: {result.stdout!r}")
    if config_toml.count("# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 begin") != 1:
        raise AssertionError(f"install did not write one fresh zerocmux hook trust marker: {config_toml!r}")
    if config_toml.count("# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 end") != 1:
    ...
    if 'trusted_hash = "sha256:stale"' in config_toml:
        raise AssertionError(f"install preserved stale zerocmux hook trust: {config_toml!r}")
    hooks = json.loads((codex_home / "hooks.json").read_text(encoding="utf-8"))
    state = codex_hook_trust_state(config_toml)
    expected_trust = expected_cmux_codex_hook_trust(hooks, codex_home / "hooks.json")
    for key, trusted_hash in expected_trust.items():
...
def test_install_preserves_plugin_tables_inside_stale_cmux_hook_trust_marker(
    ...
    config_path.write_text(
        ...
        f'[ hooks . state . "{stale_key}" ] # stale zerocmux trust\n'
        'trusted_hash = "sha256:stale"\n'
        "\n"
        ...
        "enabled = true\n"
        "# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 end\n",
        encoding="utf-8",
    )
    env = os.environ.copy()
    env["CODEX_HOME"] = str(codex_home)
    ...
    if 'trusted_hash = "sha256:stale"' in config_toml:
        raise AssertionError(f"stale zerocmux hook trust was preserved: {config_toml!r}")
    if stale_old_cmux_key in config_toml:
        raise AssertionError(f"old zerocmux hook trust was preserved: {config_toml!r}")
    if stale_legacy_key in config_toml or stale_legacy_hash in config_toml:
        raise AssertionError(f"legacy zerocmux hook trust was preserved: {config_toml!r}")
    trust_begin = "# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 begin"
    trust_end = "# zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738 end"
    if config_toml.count(trust_begin) != 1:
        raise AssertionError(f"install did not write one fresh zerocmux hook trust marker: {config_toml!r}")
    if config_toml.count(trust_end) != 1:
        raise AssertionError(f"install did not close the fresh zerocmux trust block: {config_toml!r}")
    ...
    for plugin_header in (
        ...
        if trust_begin_index < plugin_index < trust_end_index:
            raise AssertionError(f"plugin table remained inside fresh zerocmux trust block: {config_toml!r}")
...
def test_uninstall_preserves_third_party_hook_trust_inside_cmux_marker(
    ...
    config_toml = config_path.read_text(encoding="utf-8")
    if "zerocmux-codex-hook-trust-f5cc24da-7a09-4b20-a756-89e7786f6738" in config_toml:
        raise AssertionError(f"zerocmux hook trust marker was preserved: {config_toml!r}")
    state = codex_hook_trust_state(config_toml)
    for key in state:
        if key.startswith(f"{(codex_home / 'hooks.json').resolve()}:") and key != same_file_user_key:
            raise AssertionError(f"zerocmux hook trust was preserved: {config_toml!r}")
    if 'trusted_hash = "sha256:same-file-user"' not in config_toml:
        raise AssertionError(f"same-file user hook trust was removed: {config_toml!r}")
    if 'trusted_hash = "sha256:third-party"' not in config_toml:
        raise AssertionError(f"third-party hook trust was removed: {config_toml!r}")
...
def test_uninstall_retry_preserves_user_hook_trust_at_default_cmux_key(
    ...
    if 'trusted_hash = "sha256:user-default-index"' not in config_toml:
        raise AssertionError(f"user hook trust at default zerocmux key was removed: {config_toml!r}")
    for key, trusted_hash in expected_trust.items():
...
def test_uninstall_removes_legacy_codex_hook_trust(cli_path: str, root: Path) -> None:
    ...
    hooks_path = codex_home / "hooks.json"
    config_path = codex_home / "config.toml"
    legacy_command = "zerocmux feed-hook --source codex --event PreToolUse"
    ...
    if legacy_command in json.dumps(hooks):
        raise AssertionError(f"legacy zerocmux hook command was preserved: {hooks!r}")
    config_toml = config_path.read_text(encoding="utf-8")
    if legacy_key in config_toml or legacy_hash in config_toml:
        raise AssertionError(f"legacy zerocmux hook trust was preserved: {config_toml!r}")
    ...
    config_path.write_text(
        "\n".join(
            [
                "[features]",
                "apps = true",
                "# zerocmux-codex-hooks-feature-78f1e4ba-66df-4d35-93c1-67fdf1cbb7df begin",
                "# zerocmux-codex-hooks-feature-78f1e4ba-66df-4d35-93c1-67fdf1cbb7df previous line: hooks = false",
                "hooks = true",
                "# zerocmux-codex-hooks-feature-78f1e4ba-66df-4d35-93c1-67fdf1cbb7df end",
                "# zerocmux hooks codex feature begin",
                "# zerocmux hooks codex feature previous line: features.hooks = false",
                "features.hooks = true",
                "# zerocmux hooks codex feature end",
                "",
            ]
        ),
        encoding="utf-8",
...
def test_codex_permission_request_is_nonblocking_telemetry(cli_path: str, root: Path) -> None:
    socket_path = root / "zerocmux.sock"
    payload = {
...
def test_codex_lifecycle_feed_events_stay_telemetry_and_distinct(cli_path: str, root: Path) -> None:
    ...
    for event_name, payload in event_payloads.items():
        stdout, frame = run_feed_hook(
            cli_path,
            root / f"zerocmux-codex-{event_name}.sock",
            payload,
            None,
        )
        if stdout != {}:
...
def test_codex_post_tool_use_redacts_tool_output(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook(
        cli_path,
        root / "zerocmux-codex-large-posttool.sock",
        payload,
        None,
    )
    if stdout != {}:
...
def test_codex_post_tool_use_accepts_native_event_label(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook(
        cli_path,
        root / "zerocmux-codex-native-posttool.sock",
        payload,
        None,
    )
    if stdout != {}:
...
def test_codex_post_tool_use_oversize_payload_is_dropped_before_decode(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook_optional_frame(
        cli_path,
        root / "zerocmux-codex-oversize-posttool.sock",
        payload,
        None,
    )
    if stdout != {}:
...
def test_codex_lifecycle_oversize_payload_is_dropped_before_decode(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook_optional_frame(
        cli_path,
        root / "zerocmux-codex-oversize-precompact.sock",
        payload,
        None,
    )
    if stdout != {}:
...
def test_codex_post_tool_use_keeps_cwd_from_tool_input(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook(
        cli_path,
        root / "zerocmux-codex-posttool-cwd.sock",
        payload,
        None,
    )
    if stdout != {}:
...
def test_codex_post_tool_use_without_response_keeps_request_input(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook(
        cli_path,
        root / "zerocmux-codex-posttool-request-only.sock",
        payload,
        None,
    )
    if stdout != {}:
...
def test_non_codex_post_tool_use_keeps_request_input(cli_path: str, root: Path) -> None:
    ...
    stdout, frame = run_feed_hook(
        cli_path,
        root / "zerocmux-antigravity-posttool.sock",
        payload,
        None,
        source="antigravity",
    )
...
def test_claude_subagent_stop_stays_distinct_feed_telemetry(cli_path: str, root: Path) -> None:
    stdout, frame = run_feed_hook(
        cli_path,
        root / "zerocmux-claude-subagent-stop.sock",
        {
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-codex-feed-hooks-", dir="/tmp") as td:
        root = Path(td)
        try:
