...
WRAPPER = ROOT / "Resources" / "bin" / "zerocmux-claude-wrapper"
...
def build_mutual_shim_tree(root: Path) -> tuple[Path, dict[str, str]]:
    cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-loop"
...
def test_wrapper_stops_mutual_foreign_shim_loop(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-mutual-shim-loop-") as td:
...
def test_wrapper_stops_node_based_foreign_shim_loop(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-node-mutual-shim-loop-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-node-loop"
...
def test_wrapper_stops_indirect_shell_foreign_shim_loop(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-indirect-mutual-shim-loop-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-indirect-loop"
...
def test_wrapper_guard_allows_child_claude_process(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-child-reentry-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-child"
        real_dir = root / "real-bin"
...
def test_wrapper_allows_finite_layered_foreign_shims(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-finite-shim-chain-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-finite"
...
def test_passthrough_real_node_claude_does_not_receive_guard_env(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-passthrough-guard-env-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-passthrough"
...
def test_custom_shim_path_with_colon_is_tracked_as_one_target(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-custom-shim-target-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-custom-colon"
...
def test_real_shell_claude_launcher_allows_child_claude_process(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-real-shell-launcher-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-shell-launcher"
        real_dir = root / "real-bin"
...
def test_custom_shell_wrapper_execing_real_claude_allows_child_claude(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-custom-real-wrapper-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-custom-real"
...
def test_interactive_finite_shim_chain_does_not_duplicate_hooks(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-interactive-finite-shim-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-interactive"
        foreign_shim_dir = root / "foreign-shim"
        real_dir = root / "real-bin"
        for directory in (cmux_shim_dir, foreign_shim_dir, real_dir):
            directory.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        if len(session_start) != 1 or len(stop) != 3:
            failures.append(
                "expected one zerocmux hook injection after finite shim chain, "
...
def test_interactive_spawning_shim_chain_refreshes_claude_pid(failures: list[str]) -> None:
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-interactive-spawn-shim-") as td:
        root = Path(td)
        cmux_shim_dir = root / "tmp" / "zerocmux-cli-shims" / "surface-spawn"
...
""",
        ...
        if "--require=" not in node_options or "--max-old-space-size=4096" not in node_options:
            failures.append(f"expected reentry to reinstall zerocmux NODE_OPTIONS, got: {combined_output!r}")
            return
