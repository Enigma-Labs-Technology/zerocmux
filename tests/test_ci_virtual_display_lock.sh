...
TMP_DIR="$(mktemp -d)"
LOCK_DIR="$TMP_DIR/zerocmux-test-virtual-display.lock"
...
if RUNNER_TEMP="$TMP_DIR" \
  ...
  CMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire >/tmp/zerocmux-vdisplay-second-acquire.out 2>/tmp/zerocmux-vdisplay-second-acquire.err; then
  cat /tmp/zerocmux-vdisplay-second-acquire.out
  cat /tmp/zerocmux-vdisplay-second-acquire.err >&2
...
if RUNNER_TEMP="$TMP_DIR" \
  ...
  CMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire >/tmp/zerocmux-vdisplay-live-owner-acquire.out 2>/tmp/zerocmux-vdisplay-live-owner-acquire.err; then
  cat /tmp/zerocmux-vdisplay-live-owner-acquire.out
  cat /tmp/zerocmux-vdisplay-live-owner-acquire.err >&2
...
if ps -p 1 >/dev/null 2>&1; then
  ...
  if RUNNER_TEMP="$TMP_DIR" \
    ...
    CMUX_VDISPLAY_LOCK_STALE_SECONDS=1 \
    CMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
    "$SCRIPT" acquire >/tmp/zerocmux-vdisplay-foreign-owner-acquire.out 2>/tmp/zerocmux-vdisplay-foreign-owner-acquire.err; then
    cat /tmp/zerocmux-vdisplay-foreign-owner-acquire.out
    cat /tmp/zerocmux-vdisplay-foreign-owner-acquire.err >&2
...
CMUX_VDISPLAY_LOCK_TOKEN="wrong-token" \
  "$SCRIPT" release 2>/tmp/zerocmux-vdisplay-wrong-token-release.err
...
OWNERLESS_LOCK_ENV="$(
  ...
  CMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire 2>/tmp/zerocmux-vdisplay-ownerless-acquire.err
)"
...
DEAD_OWNER_LOCK_ENV="$(
  ...
  CMUX_VDISPLAY_LOCK_STALE_SECONDS=1800 \
  CMUX_VDISPLAY_LOCK_POLL_SECONDS=1 \
  "$SCRIPT" acquire 2>/tmp/zerocmux-vdisplay-dead-owner-acquire.err
)"
...
REAP_LOCK_DIR="$TMP_DIR/zerocmux-test-reap.lock"
REAP_ENV="$(
  RUNNER_TEMP="$TMP_DIR" \
