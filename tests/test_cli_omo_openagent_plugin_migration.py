#!/usr/bin/env python3
"""
Regression test: zerocmux omo must not re-add the legacy oh-my-opencode plugin
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-omo-openagent-") as td:
        root = Path(td)
        fake_bin = root / "bin"
        fake_bin.mkdir()
...
""",
        ...
        if "oh-my-opencode" in package_names:
            print(f"FAIL: zerocmux omo re-added legacy plugin name: {plugins!r}")
            return 1
        ...
        if openagent_specs != ["oh-my-openagent@3.17.5"]:
            print(
                "FAIL: zerocmux omo did not prefer the existing renamed plugin entry; "
                f"got {plugins!r}"
            )
            return 1
        ...
        if run.returncode != 0:
            print("FAIL: zerocmux omo reached fake opencode but returned non-zero")
            print(f"exit={run.returncode}")
    ...
    print("PASS: zerocmux omo preserves oh-my-openagent without legacy plugin loop")
    return 0
