...
5) Clean quit, then corrupt the primary session snapshot -> relaunch: zerocmux
   ...
   `zerocmux restore-session` keeps working.
"""
...
# because the zerocmux claude wrapper inserts its own arguments around --resume.
SESSION_SPECS = [
...
def _connect(socket_path: Path) -> zerocmux:
    client = zerocmux(socket_path=str(socket_path))
    client.connect()
    if not client.ping():
...
def _read_scrollback(client: zerocmux) -> str:
...
def _collect_all_scrollbacks(client: zerocmux) -> str:
    chunks: list[str] = []
...
def main() -> int:
    ...
    socket_path = Path(f"/tmp/zerocmux-restore-stress-{bundle_id.replace('.', '-')}.sock")
    snapshot = _snapshot_path(bundle_id)
    previous_snapshot = _snapshot_path(bundle_id, suffix="-previous")

    failures: list[str] = []

    with tempfile.TemporaryDirectory(prefix="zerocmux-restore-stress-") as td:
        ...
        app_env = {
            ...
            # Claude resume routes through the zerocmux claude wrapper, which
        ...
        try:
            ...
            try:
                ...
                if not previous_snapshot.exists():
                    ...
                    # The manual `zerocmux restore-session` recovery entrypoint must
