#!/usr/bin/env python3
"""
Regression test: `zerocmux omo` preserves fallback OpenCode dirs in PATH.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-omo-fallback-path-") as td:
        root = Path(td)
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `zerocmux omo` failed with OpenCode in a fallback dir")
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-omo-fallback-install-path-") as td:
        root = Path(td)
        fallback_bin = root / ".bun" / "bin"
        fallback_bin.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `zerocmux omo` failed to install OMO with bun in a fallback dir")
            print(f"exit={proc.returncode}")
            print(f"stdout={proc.stdout.strip()}")
            print(f"stderr={proc.stderr.strip()}")
            return 1
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-omo-skip-app-bundle-path-") as td:
        root = Path(td)
        fallback_bin = root / ".bun" / "bin"
        stale_app_bin = root / "Older zerocmux.app" / "Contents" / "Resources" / "bin"
        fallback_bin.mkdir(parents=True, exist_ok=True)
...
""",
        ...
        if proc.returncode != 0:
            print("FAIL: `zerocmux omo` failed when stale app-bundled OpenCode was first on PATH")
            print(f"exit={proc.returncode}")
            print(f"stdout={proc.stdout.strip()}")
            print(f"stderr={proc.stderr.strip()}")
            return 1
    ...
    print("PASS: zerocmux omo preserves fallback OpenCode dirs in PATH")
    return 0
