...
SOURCE_WRAPPER = ROOT / "Resources" / "bin" / "zerocmux-claude-wrapper"
...
def run_wrapper(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="cmux-claude-wrapper-test-") as td:
        ...
        wrapper = wrapper_dir / "zerocmux-claude-wrapper"
...
def test_live_socket_injects_supported_hooks_without_unlocking_bypass(failures: list[str]) -> None:
    ...
    expect(
        any(
            h.get("command") == '"${CMUX_CLAUDE_HOOK_CMUX_BIN:-zerocmux}" hooks feed --source claude'
...
def test_live_socket_merges_user_settings_into_hooks(failures: list[str]) -> None:
    ...
    expect(
        settings.get("preferredNotifChannel") == "notifications_disabled",
        f"merge user settings: zerocmux hook settings lost, got {settings}",
        failures,
    )
    expected_hooks = {
    ...
    expect(
        ...
        f"merge user settings: zerocmux hooks missing after merge, got {settings.get('hooks', {}).keys()}",
        failures,
    )
    expect(
...
def test_live_socket_merges_inline_settings_form(failures: list[str]) -> None:
    ...
    expect(
        settings.get("preferredNotifChannel") == "notifications_disabled",
        f"inline settings: zerocmux hooks lost, got {settings}",
        failures,
    )
...
def test_live_socket_repeated_settings_user_value_wins_conflict(failures: list[str]) -> None:
    ...
    expect(
        settings.get("preferredNotifChannel") == "notifications_disabled",
        f"merged: zerocmux hook settings lost, got {settings}",
        failures,
    )
...
def test_live_socket_user_nonobject_hooks_does_not_drop_cmux_hooks(failures: list[str]) -> None:
    ...
    # zerocmux's own hook structure. If a user --settings sets `hooks` to a non-object
    # (here an array; `null` behaves the same), the zerocmux hook object must survive
    ...
    expect(
        ...
        f"nonobject-hooks: zerocmux hook object dropped by non-object user hooks, got {hooks!r}",
        failures,
    )
    expect(
        settings.get("preferredNotifChannel") == "notifications_disabled",
        f"nonobject-hooks: zerocmux preferredNotifChannel lost, got {settings}",
        failures,
    )
    expect(
...
def test_live_socket_merges_settings_file_form(failures: list[str]) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="zerocmux-claude-wrapper-settings-file-") as td:
    ...
    expect(
        settings.get("preferredNotifChannel") == "notifications_disabled",
        f"settings file: zerocmux hooks lost, got {settings}",
        failures,
    )
...
def test_live_socket_resume_self_heals_mismatched_claude_config_dir(failures: list[str]) -> None:
    # Regression for https://github.com/kernelalex/zerocmux/issues/6194: when zerocmux is
    ...
    def setup_env(tmp: Path) -> dict[str, str]:
        ...
        # this session (mirrors the zerocmux app inheriting an agent's CLAUDE_CONFIG_DIR).
...
def test_stale_socket_resume_self_heals_mismatched_claude_config_dir(failures: list[str]) -> None:
    # App restore can launch terminal startup commands before the zerocmux socket is
...
def test_stale_socket_resume_self_heals_after_value_option(failures: list[str]) -> None:
    ...
    # newer Claude options that are not in zerocmux's preserved-argument allowlists.
    ...
    code, auth_env, real_argv, stderr = run_wrapper_auth_env(
        argv=["--permission-prompt-tool", "/tmp/zerocmux-permission-tool", "--resume", session_id],
        inherited_env={},
        socket_state="stale",
        setup_env=setup_env,
    )
    ...
    expect(
        real_argv == ["--permission-prompt-tool", "/tmp/zerocmux-permission-tool", "--resume", session_id],
...
def test_plain_terminal_resume_does_not_self_heal_mismatched_claude_config_dir(failures: list[str]) -> None:
    # Outside zerocmux, the wrapper must be a passthrough and must not repoint the
...
def test_live_socket_resume_after_unlisted_value_option_does_not_inject_session_id(failures: list[str]) -> None:
    code, real_argv, _, stderr, _, _, _, _, _, _ = run_wrapper(
        socket_state="live",
        argv=["--permission-prompt-tool", "/tmp/zerocmux-permission-tool", "--resume", "some-session-id"],
    )
    ...
    expect(
        passthrough_argv == ["--permission-prompt-tool", "/tmp/zerocmux-permission-tool", "--resume", "some-session-id"],
...
def test_live_socket_resume_self_heal_ignores_prompt_text_after_double_dash(failures: list[str]) -> None:
    ...
    # trigger resume self-healing or suppress zerocmux's generated --session-id.
...
def test_live_socket_preserves_plain_anthropic_model_on_default_path(failures: list[str]) -> None:
    # Regression for https://github.com/kernelalex/zerocmux/issues/7047.
    # A user who pins `export ANTHROPIC_MODEL=claude-opus-4-8[1m]` to get the
    # Max-plan 1M context window must keep that selection inside zerocmux on the
    ...
    # The model pin must not block the normal zerocmux hook/session injection.
    expect(
        "--session-id" in real_argv,
