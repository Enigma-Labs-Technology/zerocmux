...
if ! awk '
  ...
  job == "app" && /tar -C "\$products" -czf "\$RUNNER_TEMP\/cmux-nightly-unsigned\.tar\.gz" zerocmux\.app/ { saw_app_only_archive=1 }
...
if ! grep -Fq 'bundle ID `com.kernelalex.zerocmux.nightly`' "$WORKFLOW_FILE"; then
...
if ! awk '
  ...
  in_upload && /zerocmux-nightly-macos\*\.dmg/ { saw_arm_artifacts=1 }
...
if ! awk '
  ...
  in_publish && /zerocmux-nightly-macos-arm64\.dmg/ { saw_arm=1 }
  in_publish && /zerocmux-nightly-macos-x86_64\.dmg/ { saw_intel=1 }
