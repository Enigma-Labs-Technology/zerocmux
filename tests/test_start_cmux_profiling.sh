...
submit_output="$("$ROOT_DIR/Resources/bin/submit-zerocmux-profile" --dry-run --profile "$timeout_out" --target-name "zerocmux DEV dog" --target-pid 303 --channel dev --bundle-id com.kernelalex.zerocmux.debug.dog --reply-to "user@example.com")"
if [[ "$submit_output" != *"Recipient: founders@manaflow.com"* ]] ||
   [[ "$submit_output" != *"Reply-to: user@example.com"* ]] ||
   [[ "$submit_output" != *"Subject: zerocmux profiling capture: zerocmux DEV dog"* ]]; then
...
"$ROOT_DIR/Resources/bin/submit-zerocmux-profile" --dry-run --profile "$timeout_out" --target-name "zerocmux DEV dog" >/dev/null
...
EOF
chmod +x "$sleep_ditto"
CMUX_PROFILE_DITTO="$sleep_ditto" "$ROOT_DIR/Resources/bin/submit-zerocmux-profile" \
  --profile "$timeout_out" \
  --package-only &
...
package_output="$(CMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-zerocmux-profile" --profile "$timeout_out" --package-only)"
...
CMUX_PROFILE_OSASCRIPT="$cancel_bin" CMUX_PROFILE_OPEN="$open_bin" CMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-zerocmux-profile" \
  --profile "$timeout_out" \
  --target-name "zerocmux DEV dog" \
  --target-pid 303 \
  --channel dev \
  --bundle-id com.kernelalex.zerocmux.debug.dog
...
CMUX_PROFILE_OSASCRIPT="$sleep_osascript" CMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-zerocmux-profile" \
  --profile "$timeout_out" \
  --target-name "zerocmux DEV dog" \
  --target-pid 303 \
  --channel dev \
...
if CMUX_PROFILE_OSASCRIPT="$cancel_bin" CMUX_PROFILE_OPEN="$open_bin" CMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-zerocmux-profile" \
  --profile "$timeout_out" \
  --target-name "zerocmux DEV dog" \
  --target-pid 303 \
  --channel dev \
  --bundle-id com.kernelalex.zerocmux.debug.dog \
  --send >/tmp/zerocmux-profile-send-cancel.log 2>&1; then
  echo "FAIL: submit helper send mode should fail when Mail send is canceled" >&2
  exit 1
fi
if ! grep -Fq "User canceled" /tmp/zerocmux-profile-send-cancel.log; then
  echo "FAIL: submit helper send cancellation did not preserve the AppleScript error" >&2
  cat /tmp/zerocmux-profile-send-cancel.log >&2
  exit 1
fi
...
HOME="$TMP_DIR" CMUX_PROFILE_FEEDBACK_EMAIL=wrong@example.com CMUX_PROFILE_LOCALE=ja_JP CMUX_PROFILE_OSASCRIPT="$capture_osascript" CMUX_PROFILE_DITTO="$ditto_bin" "$ROOT_DIR/Resources/bin/submit-zerocmux-profile" \
  --profile "$timeout_out" \
  --target-name "zerocmux DEV dog" \
  --target-pid 303 \
  --channel dev \
  ...
  --note-file "$note_file" \
  --send
if ! grep -Fq "zerocmuxプロファイルを送信" "$captured_args" ||
   ! grep -Fq "下書きを開く" "$captured_args" ||
   ! grep -Fq "zerocmuxプロファイリングキャプチャ" "$captured_args" ||
