...
This is an internal planning note for zerocmux skills and customization surfaces. Keep public end-user skills in the zerocmux repo when they teach repeatable user workflows. Keep release, debug, and company operations skills in `cmuxterm-hq`.

## Current Public Skills

- `zerocmux`: core CLI control for windows, workspaces, panes, surfaces, focus, and routing.
- `zerocmux-workspace`: current-workspace automation, sidebar metadata, input, and helper surfaces.
- `zerocmux-settings`: safe reads, writes, validation, and editor open for `~/.config/cmux/cmux.json`.
- `zerocmux-customization`: user-facing config across actions, plus button, tab bar buttons, workspace layouts, Dock controls, settings, notifications, browser routing, and Ghostty config boundaries.
- `zerocmux-diagnostics`: support-safe health checks for CLI, socket, hooks, session restore, settings, and agent binaries.
- `zerocmux-browser`: browser automation inside zerocmux webview surfaces.
- `zerocmux-markdown`: formatted markdown panels beside terminals.
...
- `.cmux/dock.json` and `~/.config/cmux/dock.json`: right-sidebar Dock controls for TUIs, logs, tests, queues, dev servers, and `zerocmux feed tui --opentui`.
- `zerocmux-settings` paths: appearance, sidebar behavior, app icon, menu-bar mode, notifications, browser routing, automation, shortcuts, and new-workspace placement.
- zerocmux CLI workspace metadata: workspace names, descriptions, colors, read/unread state, progress, status pills, and logs.
...
- `zerocmux-dock`: create `.cmux/dock.json` or global Dock controls after inspecting project scripts, logs, services, and TUIs. This should become a separate skill if Dock setup gets enough schema, trust, and validation detail to make `zerocmux-customization` too broad.
- `zerocmux-feed`: diagnose and configure Feed hooks, Feed TUI Dock controls, notification categories, and event stream checks. Keep it separate from diagnostics only if it gains repeatable setup/edit flows beyond read-only health checks.
- `zerocmux-sidebar`: manage sidebar metadata, workspace descriptions, colors, pinned state, read state, and project conventions. This is useful when sidebar metadata becomes a common integration target for agents and scripts.
- `zerocmux-ssh`: set up remote workspaces, SSH URL launches, remote browser routing, reconnect behavior, and remote agent notifications.
- `zerocmux-cloud-vm`: operate Cloud VM create, attach, exec, SSH endpoint, billing, provider, and smoke-test workflows.
- `zerocmux-vault`: manage vault-backed agent configuration, credential references, and restore behavior without leaking secrets into prompts.
...
`zerocmux-customization`.

## Promotion Rule

Create a new skill when the workflow has setup commands, validation, and safety rules that an agent would otherwise rediscover. Keep an idea in docs when it is just product positioning, a list of possible settings, or a compact `zerocmux-customization` example. Do not publish private debug windows, release automation, production operations, or company-specific workflows as end-user zerocmux skills.
