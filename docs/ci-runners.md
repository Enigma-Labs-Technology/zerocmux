...
gh variable set MACOS_RUNNER_26       --repo kernelalex/zerocmux -b blacksmith-6vcpu-macos-26
gh variable set MACOS_RUNNER_26_RELEASE --repo kernelalex/zerocmux -b blacksmith-6vcpu-macos-26
gh variable set MACOS_RUNNER_IOS      --repo kernelalex/zerocmux -b blacksmith-6vcpu-macos-26
```

Break-glass a type to WarpBuild only when Blacksmith is down or queuing for
minutes (as happened for macOS in https://github.com/kernelalex/zerocmux/pull/4926).
...
gh variable set MACOS_RUNNER_26 --repo kernelalex/zerocmux -b depot-macos-latest
```
...
We do not use the self-hosted mac-mini fleet (`zerocmux-mac-mini`, `studio1`,
`mac4-cmuxvnc*`, `zerocmux-austin-mini-*`) for any CI job. Those minis carry labels
