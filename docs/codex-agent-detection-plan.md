...
Done: `zerocmux-codex-wrapper` (PATH shim, Claude-parity per-invocation `[hooks]`
...
- zerocmux's Codex hooks (`~/.codex/hooks.json`) are NOT installed.
- Codex has a single legacy `notify` slot, and it is taken by Computer Use:
  `notify = [".../SkyComputerUseClient", "turn-ended"]`. zerocmux's notify-based
  events cannot fire.
...
Why Claude works, for contrast: Claude Code has a `SessionStart` hook. zerocmux's
`zerocmux-claude-wrapper` is a PATH shim that injects zerocmux's hooks per-invocation
...
zerocmux owns the terminal environment, so it can mediate any agent the user launches
in its terminal, per-invocation, without touching global config. Detection must
depend only on what zerocmux controls — the wrapper, the injected env, the pid it
...
A `zerocmux-codex-wrapper`, mirroring `zerocmux-claude-wrapper`:

1. zerocmux prepends a shim dir to `PATH` when it spawns its terminal shells (the
...
2. Before exec'ing the real codex, the wrapper emits the launch signal itself:
   `zerocmux hooks codex session-start` carrying `CMUX_SURFACE_ID`, the cwd, and its
   child pid. THIS is the reliable detection signal, and it needs nothing from
   Codex — the wrapper, which zerocmux controls, is the source. (More robust than the
...
4. Outside zerocmux (no `CMUX_SURFACE_ID` / socket), the wrapper no-ops and execs the
   real codex, so it is invisible to non-zerocmux usage.
...
- Chain: read the user's existing `notify` from `config.toml`; zerocmux's injected
...
Super reliable:
- Detection of any Codex that zerocmux launches OR the user types in a zerocmux terminal.
  Anchored to the wrapper-emitted session-start (surface + pid zerocmux owns).
...
- Deliberate bypass is not caught: absolute path (`/usr/local/bin/codex`), an
  ...
  a host zerocmux does not own. Not surfacing a truly-unmediated agent is correct
  behavior, not a bug.
...
1. Does a `zerocmux-claude-wrapper`-style PATH shim already exist, and where is the
...
4. Does zerocmux's existing Codex launch path (`codex-teams` /
...
2. Add `zerocmux-codex-wrapper` to the same shim dir + PATH injection Claude uses.
3. Wrapper emits `zerocmux hooks codex session-start` (surface / pid / cwd) before
   exec, and no-ops cleanly outside zerocmux.
...
8. Build + dogfood: type `codex` in a zerocmux terminal, confirm it appears in the
