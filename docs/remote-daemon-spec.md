...
- `DONE` `zerocmux ssh` creates remote-tagged workspaces and does not require `--name`.
- `DONE` scoped shell niceties are applied only for `zerocmux ssh` launches.
...
- `DONE` bootstrap installs `~/.cmux/bin/zerocmux` wrapper (also tries `/usr/local/bin/zerocmux`) so `cmux` is available in PATH on the remote.
- `DONE` normal `zerocmux ssh` launches `cmuxd-remote serve --stdio --persistent --slot <slot> --persistent-lease-port <port>`, where the stdio process proxies to a long-lived authenticated daemon with slot credentials under `~/.cmux/daemon/<version>/<slot>/`, a short per-user socket path under `/tmp/cmuxd-remote-<uid>/`, and an exact relay-slot lease path.
- `DONE` persistent daemon slots advertise `pty.session.persistent_daemon`; zerocmux requires that capability before preserving a saved remote PTY session ID across app relaunch.
...
- `DONE` `zerocmux remote-daemon-status` exposes the current manifest entry, local cache verification state, release download command, and GitHub attestation verification command.
...
|---|---|---|---|
| M-001 | `zerocmux ssh` workspace creation + metadata + optional `--name` | DONE | Covered by `tests_v2/test_ssh_remote_cli_metadata.py` |
...
| M-004b | CLI relay: run zerocmux commands from within SSH sessions | DONE | Reverse TCP forward + Go CLI relay + bootstrap wrapper |
...
| M-011 | Detachable persistent `zerocmux ssh` PTY sessions | IN PROGRESS | Persistent remote daemon slots keep PTY sessions alive across local surface close and app relaunch; coverage includes Go daemon auth/reattach tests, Swift restore tests, CLI contract tests, and `tests_v2/test_ssh_remote_detachable_pty.py` |
...
| ID | Scenario | Status |
|---|---|---|
| C-001 | `zerocmux ping` from remote session | DONE |
| C-002 | `zerocmux list-workspaces --json` from remote | DONE |
| C-003 | `zerocmux new-workspace` from remote | DONE |
| C-004 | `zerocmux rpc system.capabilities` passthrough | DONE |
...
| ID | Scenario | Status |
|---|---|---|
| DP-001 | `zerocmux ssh` creates a persistent daemon slot and PTY session ID | IN PROGRESS |
| DP-002 | closing the local SSH surface detaches the attachment without killing the remote shell | IN PROGRESS |
| DP-003 | `zerocmux ssh-session-list` reports detached persisted sessions with bounded scrollback metadata | IN PROGRESS |
| DP-004 | `zerocmux ssh-session-attach` reattaches to the same remote shell PID and env | IN PROGRESS |
...
| DP-006 | `zerocmux ssh-session-cleanup` terminates persisted PTY sessions explicitly | DONE |
