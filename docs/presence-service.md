...
Realtime device presence for zerocmux: every team member sees which of the team's
devices (Macs, iPhones, and their tagged zerocmux app instances) are online or
...
(https://github.com/kernelalex/zerocmux/pull/5626); this service adds no Aurora
...
A dev/staging instance (`zerocmux-presence-dev`, dev Stack project) is live at
`https://zerocmux-presence-dev.debussy.workers.dev` on the team Cloudflare
...
- **iOS** (`Packages/iOS/CmuxMobileShell/Sources/CmuxMobileShell/PresenceClient.swift`):
  ...
  (https://github.com/kernelalex/zerocmux/pull/5648) rows, writes pushed routes
