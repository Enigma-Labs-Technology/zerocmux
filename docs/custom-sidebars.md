# Custom sidebars: vibe-code your own zerocmux sidebar

zerocmux lets you build your own sidebar UI by writing a small SwiftUI-style file.
It is interpreted at runtime (no Xcode, no build step, no signing), renders as
native SwiftUI in the real sidebar, hot-reloads on save, binds to live zerocmux
state, and can run zerocmux commands on tap. This guide is the authoring contract
...
- Make it interactive by default. Rows that represent something you can open
  should be tappable and run the matching `zerocmux(...)` action (e.g. selecting a
...
- Iterate by saving the file and opening it as a pane with
  `zerocmux sidebar open <name>`; it hot-reloads there while you edit. Verify it
...
for the left sidebar, or run `zerocmux sidebar open <name>` to show it in a pane;
...
Files/Find/Vault): `zerocmux right-sidebar set custom <name>` selects one and adds
a Custom button to the right sidebar's mode bar. The panel gets the same data
keys and `zerocmux(...)` actions as the left sidebar and hot-reloads the same way.
...
as real SwiftUI inside the zerocmux window, so hover styling, focus, keyboard,
...
crash or hang caused by the interpreted file cannot take down zerocmux, but
...
Install one from a zerocmux checkout:
...
Then validate and open it as a Bonsplit pane:
    ...
    zerocmux sidebar validate status-board
    zerocmux sidebar open status-board

`zerocmux sidebar select <name>` still previews a custom sidebar in the left
sidebar picker. Use `zerocmux sidebar open <name>` when you want the sidebar as a
...
## Quick start
    ...
    VStack(alignment: .leading, spacing: 8) {
        ...
        ForEach(workspaces) { w in
            Button(action: { zerocmux("workspace.select", workspace_id: w.id) }) {
                HStack {
...
- `workspaces` — array, one per workspace. Always present: `id`, `title`,
  ...
  the same shape with every pull request zerocmux knows for the workspace),
...
## Actions (run real zerocmux commands on tap)

A button or `.onTapGesture` body calls `zerocmux("<method>", param: value)`. On tap
it runs that zerocmux command through the same dispatcher as the `zerocmux` CLI:

    Button(action: { zerocmux("workspace.select", workspace_id: w.id) }) { ... }
    ...onTapGesture { zerocmux("surface.focus", surface_id: t.id) }

Use real method and parameter names. Common ones: `workspace.select`
(`workspace_id`), `surface.focus` (`surface_id`), `workspace.reorder`
(`workspace_id` + `index`). Run `zerocmux docs api` to discover the full command
surface.
...
runs the `move` command, which both reorders and persists (zerocmux remembers
workspace order):
    ...
    HSplitView {
        VStack(alignment: .leading) {
            for i in 0..<workspaces.count {
                Button(action: { zerocmux("workspace.select", workspace_id: workspaces[i].id) }) {
            ...
            for i in 0..<workspaces.count {
                if workspaces[i].selected {
                    for j in 0..<workspaces[i].tabs.count {
                        Button(action: { zerocmux("surface.focus", surface_id: workspaces[i].tabs[j].id) }) {
...
(`TextField`, `Toggle`, `Slider`, `Picker`) — buttons/taps that run `zerocmux(...)`
...
is live; data zerocmux doesn't track (custom domain collections) won't appear.
