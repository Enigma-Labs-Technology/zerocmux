...
The corresponding universal ReleaseFast GhosttyKit archive is published at
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-366c801e066c37695c2d9be4a6567662bd763ad0-crashsubdir-zerocmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`.
...
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-b4b6d69c82033e16137266a04b364dc53d16c350-crashsubdir-zerocmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`.
...
2. `src/renderer/Thread.zig`: kept zerocmux's iOS external-drain ownership and
...
9. `ghostty_surface_read_screen_tail_vt` lets zerocmux preserve terminal history
...
the zerocmux link-click regression test, the `wasm32-freestanding` libghostty-vt
build, a clean universal GhosttyKit build, tagged zerocmux reloads `gcmp` and
`gsel2`, and live accessibility reads across select-all, endpoint adjustment,
and clearing.
Prebuilt archive:
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-eb500e9f45c8b6ffa6043350ec1488a42d195406-crashsubdir-zerocmux-crash-v1

### Previous pin

The previous zerocmux pin was `5ae712a89`, which added the bounded VT screen-tail
...
Previous zerocmux pinned fork head: `541e5e89d`, which merges the render-grid span
...
zerocmux-only follow-up commit adapts the new shaper tests to this pinned fork's
...
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-541e5e89db0448d5cd85a7b348d8f6a64618c900-crashsubdir-zerocmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`.
...
The previous zerocmux pinned fork head was `1b454eb99`, which retained the
Darwin-only `ghostty_surface_set_renderer_realized` C API (a
`display_realized` renderer-thread mailbox message that drives
`displayUnrealized()`/`displayRealized()`) on top of `5697db81`. zerocmux uses it to
...
https://github.com/kernelalex/zerocmux/issues/4607. The corresponding prebuilt
archive is published at
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-49cb510f759aa109a5b1d30329583195155e58a4-crashsubdir-zerocmux-crash-v1
...
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-1b454eb999d6f4aea28a18ca0e1500c0477383ef-crashsubdir-zerocmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`. The `7a5179843` RTL shaping
head's corresponding prebuilt archive is published at
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-7a51798436fa2cfcfcc9a2ed1e109ba69bdb68f9-crashsubdir-zerocmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`.

The prior head was refreshed from upstream `main` on May 1, 2026.
Earlier zerocmux pinned fork head: `34cbf180d`, merging the surface registry
serialization for https://github.com/kernelalex/zerocmux/issues/5458 (`e5c962a72`,
landed on zerocmux `main`) into the iOS render bounded-acquire line (`f78189ac1`)
combined with the cmd-click link refresh under mouse reporting (`df789cd4b`,
manaflow-ai/ghostty#71 and PRs #74 through #79) for
https://github.com/kernelalex/zerocmux/issues/5128. This keeps the previous head's
manual embedded IO patch in https://github.com/manaflow-ai/ghostty/pull/53,
the Metal renderer row rebuild guard for https://github.com/kernelalex/zerocmux/issues/3369,
...
keeps the zerocmux theme picker hooks, exposes the manual surface IO needed by
...
It also supports Ctrl-N and Ctrl-P in the zerocmux theme picker.
The corresponding prebuilt archive is published at
https://github.com/manaflow-ai/ghostty/releases/tag/xcframework-34cbf180d8917b802d61d9929cfb493594f2ab52-crashsubdir-zerocmux-crash-v1
and pinned in `scripts/ghosttykit-checksums.txt`.
...
- Commits:
  ...
  - `176bd550f` (Add ctrl navigation to zerocmux theme picker)
- Files:
  - `build.zig`
...
- Summary:
  ...
  - Applies the highlighted search result when Enter is pressed from search mode in zerocmux-managed picker sessions.
  - Supports Ctrl-N and Ctrl-P as one-row down/up navigation in zerocmux-managed picker sessions.
...
- Commits:
  - `0b231db94` (Re-export zerocmux selection APIs removed from upstream)
...
- Summary:
  ...
  - The first commit intentionally preserves the panic so zerocmux can keep the
    required failing-test-then-fix history for https://github.com/kernelalex/zerocmux/issues/3369.
...
- Summary:
  - Link hover/highlight state was refreshed in `keyCallback`/`cursorPosCallback`
    ...
    and the Cmd-click was reported to the program — which made zerocmux fall back to
  ...
  - Adds a shared `mouseLinkRefreshAllowed` gate (pure logic in
    ...
    https://github.com/kernelalex/zerocmux/issues/5128.
...
The current zerocmux pin is the merged head `34cbf180d`, which merges the surface
registry serialization (`e5c962a72`, section 14, landed on zerocmux `main` via
...
`xcframework-34cbf180d8917b802d61d9929cfb493594f2ab52-crashsubdir-zerocmux-crash-v1`
...
- `src/Surface.zig` (modifier tracking)
  - `modsChanged` and the key callback's link-highlight gate must compare binding mods against
    binding mods (stored mouse mods are binding-only). zerocmux sends sided modifier bits on key
