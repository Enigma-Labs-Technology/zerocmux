...
Tracks the redesign of how zerocmux tracks whether a terminal surface has a coding
agent (Claude Code / Codex) session, for the iOS GUI ("coding agent") mode.

## Goal

The iOS GUI must reliably know, for any zerocmux surface: is there an agent session,
...
2. **Deterministic binding, no heuristics.** Surface to session binding is
   established by construction at terminal/agent start, keyed by a zerocmux-minted
...
So a hand-typed `claude`/`codex` in any zerocmux terminal already inherits the
surface token.

Hook surface resolution is multi-signal and zerocmux-owned
...
surface zerocmux owns. This is the deterministic binding the redesign blesses.

Entrypoint coverage (each ends with a hook whose surface resolves by the above):
- `zerocmux claude-teams` / hand-typed `claude`: COVERED. Hooks injected via
  `--settings` by `zerocmux-claude-wrapper`; env inherited; tty/pid backstop.
- `zerocmux new-surface --type agent-session` (UI/CLI): COVERED. App injects
...
- `codex` (bare or `codex-teams`): PARTIAL. No `--settings`-style auto-inject
  wrapper; relies on `zerocmux hooks setup codex` having installed `~/.codex` hooks.
  Once installed, the fired hook resolves the surface identically. GAP = ensure
  setup ran; not a per-launch binding gap.
- Degraded mode: `zerocmux-claude-wrapper` execs claude WITHOUT hook injection when
  the socket ping fails (`zerocmux-claude-wrapper:414`). Acceptable: the agent still
...
zerocmux owns every zerocmux terminal's environment. When it spawns a surface's shell,
it injects:

- `CMUX_SURFACE_ID` — stable, zerocmux-owned, the binding key.
- `CMUX_WORKSPACE_ID`.
- the agent hook config, so `zerocmux hooks <agent> <event>` fires for any agent
  started in that shell and carries the surface token.

Because the token lives at the shell/env layer (which zerocmux always controls for
...
fallback: there is no gap for a zerocmux-owned surface. An agent in a terminal zerocmux
does NOT own (ssh/remote/external) is not a zerocmux surface and has nothing to show
in GUI mode, so no fallback is needed.

Open task: enumerate every way an agent can start in a zerocmux surface (new-agent
...
## Identity and the binding key (durability of zerocmux IDs)
...
- **Window**: OS-managed (`NSWindow`), not a zerocmux model identity.
...
```
AgentSession {
  surfaceID        // binding key, stable, zerocmux-owned
  workspaceID
  ...
  pid              // process zerocmux spawned (or adopted via the env token)
...
- Backstops that do NOT depend on hook delivery:
  - **Owned-process exit.** zerocmux spawned (or adopted) the agent pid, so a child
...
- On restore, re-attach by token (the live agent process still carries
  `CMUX_SURFACE_ID`; zerocmux can re-adopt the pid it owns), or mark `ended` if the
...
- **Slice B — Deterministic process-exit backstop. DONE.** Implemented as
  ...
  new iOS UI needed. Design: zerocmux does NOT own a
  ...
  on the agent pid zerocmux already knows (from hook events / the store). On `.exit`,
...
- **Status note (after A/B/D):** Slices A, B, D are implemented, build-green on
  ...
  - **F (codex hook auto-setup): deferred, needs a product decision.** The only
    ...
    `zerocmux hooks setup` (the existing onboarding step). Safe future options: an
...
- **Slice D — Delete the unreliable heuristics. IN PROGRESS.** Remove
  ...
  title-key helpers. REVISED SCOPE: the hook-store stays — it is zerocmux-written,
...
- **Slice F — Codex parity.** Guarantee `zerocmux hooks setup codex` is in effect so
  codex hooks fire and resolve the surface like claude.

## Verification

- Build/compile via tagged `-derivedDataPath /tmp/zerocmux-<tag>` (never untagged).
