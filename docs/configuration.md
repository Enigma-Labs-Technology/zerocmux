...
- `paneBorderColor`: overrides the divider color between zerocmux panes in split workspaces.
- `activePaneBorderColor`: draws a border around the focused zerocmux pane in split workspaces.

Both settings accept 6-digit hex colors (`#RRGGBB`). Omit a key, or set it to `null`, to keep the built-in appearance. These settings apply to zerocmux's multi-surface pane layout, not Ghostty's internal splits; Ghostty settings such as `split-divider-color` still only affect splits inside one Ghostty instance.
...
{
  "app": {
    "windowTitleTemplate": "[zerocmux:{windowToken}] {activeWorkspace}"
  }
}
```
...
- `{defaultTitle}`: the title zerocmux would have used without a template.
- `{appName}`: `zerocmux`.

For tiling window managers such as AeroSpace or yabai, match on the stable token in the title. For example, the template above gives each restored macOS window a title containing `[zerocmux:abcd1234]`, so a rule can match `\\[zerocmux:abcd1234\\]`. The token is stable across relaunches for restored windows because it comes from the persisted window UUID.

## `app.confirmQuit`

Controls when zerocmux asks before quitting:
...
Controls the section order in the titlebar `+` button menu. The custom section comes from `ui.newWorkspace.contextMenu`. The built-in Cloud VM section is feature-flag gated and stays hidden in zerocmux release builds because hosted Cloud VM services are unavailable.
...
Opt-in Agent Hibernation. zerocmux kills idle background agent processes to free RAM and CPU, then resumes each one with its saved session when you visit its tab. See [agent-hooks.md](agent-hooks.md#agent-hibernation) for the full behavior, including the confirmation settle window and how resume works.
...
Enable it from the command palette (`⌘⇧P` -> Enable Agent Hibernation), from **Settings > Terminal > Agent Hibernation**, or with `zerocmux agent-hibernation on`.

## `sidebar.showAgentActivity`

Shows a loading spinner on sidebar workspace rows that currently have running coding agents or active manual loaders (`zerocmux workspace loading on`).
...
The spinner is compositor-driven (a Core Animation transform run by the render server), so it costs no per-frame CPU and pauses automatically while the window is occluded or Reduce Motion is on. Toggle it manually per workspace with `zerocmux workspace loading <on|off> [--id <name>]`; each `--id` is a separate loader and the command prints the workspace state as `before=ON;after=OFF`.
...
- `preservePromptAfterLaunch`: optional boolean for custom launch-only actions. When `true`, zerocmux submits `commandTemplate` as a provider launch command while keeping the TextBox prompt intact for the active agent session.
...
session, zerocmux normally `scp`s it to `/tmp/zerocmux-drop-<uuid>` on the host and types
the remote path. `terminal.uploadCommands` is an ordered list of host-scoped
rules; when the ssh destination matches a rule, zerocmux runs that rule's command
...
- `command`: run through `/bin/sh -c`, **once per file**. It receives the file and
  ...
  (the `/tmp/zerocmux-drop-<uuid>` path zerocmux picked), `CMUX_UPLOAD_DESTINATION`,
...
zerocmux inserts the command's **stdout** at the cursor:
...
- If the command prints **nothing**, zerocmux inserts the shell-escaped remote path it
...
The toolbar layout toggle persists the last user choice for future generated diff viewers. Passing `zerocmux diff --layout split` or `zerocmux diff --layout unified` overrides both the saved toolbar choice and this default for that invocation.

## `sidebar.beta.workspaceTodos.checklistStyle`

Workspace todos are always available. Status is inferred from live signals (agent needs input / agent running / open PR / merged PRs / dirty tree) and can be pinned manually from the glyph's status popover, the row's context menu (Status submenu, Mark as Done), the command palette, or `zerocmux workspace status set <lane|auto>`; checklists are managed from the row, the workspace todo pane (`zerocmux todo open`), `zerocmux todo ...`, or by agents over the control socket.
...
zerocmux also posts a notification when a workspace's status first reaches done, and when its checklist first becomes fully complete, so you can watch agent progress without keeping the pane open.
