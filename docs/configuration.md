...
Routine Agent Hibernation is opt-in. zerocmux hibernates idle background agent
...
zerocmux prefers macOS's resource-coalition physical footprint, which includes the
zerocmux process and its descendants. The private coalition layout is enabled only
on OS releases with a validated ABI; if the API or validation is unavailable,
zerocmux uses a complete, de-duplicated descendant process tree. An incomplete
...
At warning or critical aggregate pressure, zerocmux posts a localized visible
notification. While the same complete pressure remains through the existing
confirmation window, zerocmux considers every currently eligible idle, non-visible
