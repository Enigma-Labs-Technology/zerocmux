# Flaky-test audit — zerocmux `feat-fix-flaky-tests`
...
- **Suggested fix:** Replace the hardcoded path with a UUID-suffixed temp path, e.g. `let socketPath = "/tmp/zerocmux-debug-theme-\(UUID().uuidString).sock"`, and pass that to both the CLI subprocess and any listener.
...
- **wall-clock-timing-assert** · severity high · confidence high
- **Evidence:** Lines 382-386: `RunLoop.current.run(until: Date().addingTimeInterval(0.5))` is the entire observation window before `XCTAssertFalse(app.state == .runningForeground, ...)`. If zerocmux steals focus more than 0.5s after the notify CLI completes, the test passes silently.
...
- **Suggested fix:** Increase `wait_for_text` timeout to at least 15s when `expected_count > 1` with a `sleep`-heavy fake binary, or remove the `sleep 3` from fake_cmux (it is not needed to prove asynchrony since the extension fires-and-forgets the zerocmux call).
...
- **Suggested fix:** Use a temporary zprofile in a per-test tempdir passed via `ZDOTDIR` env override to the launched zerocmux process, instead of mutating the real `~/.zprofile`.
...
- **shared-global-state** · severity low · confidence medium
- **Evidence:** Lines 91-128: creates a workspace with `client.new_workspace()` but never calls `client.close_workspace(ws_id)`. The test runs before test_cmd_ctrl_h_goto_split_left_from_webview (which also creates a workspace), so workspaces accumulate in the live zerocmux instance across the two tests.
...
- **Evidence:** Lines 20-29: `SOCKET_PATH = _resolve_socket_path()` is executed at import time and unconditionally raises `cmuxError` if `CMUX_SOCKET_PATH` is unset or does not match `/tmp/zerocmux-debug-[^/]+\.sock`. All 20 peer files use `os.environ.get("CMUX_SOCKET_PATH", "/tmp/zerocmux-debug.sock")` and accept the default socket.
...
- **Evidence:** Line 18: `SOCKET_PATH = os.environ.get('CMUX_SOCKET', '/tmp/zerocmux-debug.sock')`. The env var name is `CMUX_SOCKET`, but the standard harness sets `CMUX_SOCKET_PATH`. Every other test in both `tests/` and `tests_v2/` reads `CMUX_SOCKET_PATH`.
- **Why flaky:** When run via the standard CI harness that exports `CMUX_SOCKET_PATH=...`, this test silently falls back to `/tmp/zerocmux-debug.sock` — the main (non-tagged) dev socket. The test then fires `vmmap` and thread-count assertions against the live instance, not the tagged build under test, giving false passes when no tagged socket is active and potentially flaky results depending on what the user's running zerocmux is doing.
- **Suggested fix:** Change line 18 to `os.environ.get('CMUX_SOCKET_PATH', '/tmp/zerocmux-debug.sock')` to match the convention used by all other tests.
