...
Flag clear new uses of legacy asynchronous patterns in zerocmux-owned Swift code when Swift concurrency would be the correct design.
...
- Completion-handler APIs in new internal code when the caller and callee are both under zerocmux control and can use `async throws`.
