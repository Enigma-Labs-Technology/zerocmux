...
- A zerocmux-owned package `.gitignore` ignores `Package.resolved`.
- A zerocmux-owned `Package.swift` dependency change resolves new or changed external pins without the matching package-local `Package.resolved` diff.
...
- zerocmux-owned package-local `Package.resolved` files are committed with SwiftPM dependency changes.
