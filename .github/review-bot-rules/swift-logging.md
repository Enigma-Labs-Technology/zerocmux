...
- Ad hoc file logging or stdout/stderr logging for production diagnostics when `Logger` or the existing zerocmux debug log should be used.
...
- Debug-only `NSLog` or zerocmux event logging guarded by `#if DEBUG`.
