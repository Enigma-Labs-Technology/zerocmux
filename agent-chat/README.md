...
Models come from the built-in lists plus models reported by the installed provider CLIs; zerocmux performs no remote catalog fetch. Model IDs are passed to provider CLIs verbatim, so newly released models reported by an updated CLI work without a sidecar update.
