...
struct RestoredSessionHistoryTests {
    /// A sanitizer that treats `zerocmux-diff://` URLs as temporary, mirroring the
    ...
    private func makeSanitizer() -> SessionHistoryURLSanitizer {
        SessionHistoryURLSanitizer { url in
            url?.scheme?.lowercased() == "zerocmux-diff"
        }
    }
    ...
    func restoreRejectsTemporary() {
        var history = RestoredSessionHistory(sanitizer: makeSanitizer())
        let became = history.restore(
            backHistoryURLStrings: ["zerocmux-diff://token", "about:blank", "  "],
            forwardHistoryURLStrings: [],
            currentURLString: "zerocmux-diff://token"
        )
        #expect(!became)
        #expect(!history.usesRestoredSessionHistory)
...
struct SessionHistoryURLSanitizerTests {
    private func makeSanitizer() -> SessionHistoryURLSanitizer {
        SessionHistoryURLSanitizer { $0?.scheme?.lowercased() == "zerocmux-diff" }
    }

    @Test("serializable rejects temporary, empty, and about:blank")
    func serializableRejects() {
        let s = makeSanitizer()
        #expect(s.serializableSessionHistoryURLString(URL(string: "zerocmux-diff://x")) == nil)
    ...
    func sanitizedParses() {
        ...
        #expect(s.sanitizedSessionHistoryURL("zerocmux-diff://x") == nil)
