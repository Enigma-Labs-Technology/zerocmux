...
// composition, the `/tmp/zerocmux-drop-<uuid>` path shape, cleanup script text,
...
extension RemoteSessionCoordinator {
    /// Uploads dropped local files to `/tmp/zerocmux-drop-*` paths on the remote
    ...
    /// The `/tmp/zerocmux-drop-<uuid>[.ext]` remote path a dropped local file
    ...
    public static func remoteDropPath(for fileURL: URL, uuid: UUID = UUID()) -> String {
        ...
        return "/tmp/zerocmux-drop-\(uuid.uuidString.lowercased())\(lowercasedSuffix)"
    }
