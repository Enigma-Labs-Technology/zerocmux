...
/// Resolves which zerocmux-managed Ghostty config file under Application Support is
...
public struct CmuxGhosttyConfigPathResolver {
    /// The bundle identifier of the released zerocmux app, used as the canonical
    /// config location and the fallback for dev/nightly/staging channels.
    public static let releaseBundleIdentifier = "com.kernelalex.zerocmux"
    private static let releaseFallbackChannelSuffixes = ["debug", "nightly", "staging"]

    public init() {}

    /// The path zerocmux writes edits to: always `config.ghostty` under the active
    ...
    /// The ordered list of zerocmux-managed config files to load, applying the
