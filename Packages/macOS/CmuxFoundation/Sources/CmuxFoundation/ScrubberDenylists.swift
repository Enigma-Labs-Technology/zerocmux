...
/// from Sentry's relay `@common` scrubbers, plus zerocmux's own provider-key / JWT /
...
public struct ScrubberDenylists: Sendable {
    ...
    /// sentry-python itself credits as "stolen from relay". zerocmux's substring
    ...
    static let sensitiveKeyMarkers: [String] = [
        ...
        // zerocmux-original markers retained (not in sentry-python's list) so no
    ...
    /// (PII keys added because zerocmux runs with `sendDefaultPii = false`), with the
    ...
    static let sensitiveKeyExactMarkers: Set<String> = [
        ...
        // sentry-python DEFAULT_PII_DENYLIST (zerocmux sends sendDefaultPii = false).
    ...
    ///
    /// The first seven entries are zerocmux-original and partly exceed relay (relay
    ...
    /// value rules zerocmux was missing, ported with no capture group so zerocmux's
    ...
    /// members are intentionally NOT duplicated here: zerocmux already covers email /
    ...
    static let valuePatterns: [SentryRegexPattern] = [
        // --- zerocmux-original rules (kept verbatim; partly exceed relay) ---
        ...
        // (relay-pii/src/regexes.rs:341-345), hand-tuned for zerocmux's free text.
        //
        ...
        // prefixes. No relay equivalent — zerocmux-specific add for dev secrets.
        ...
        // --- relay @common value rules zerocmux was missing (ported, no group) ---
        // @pemkey — a PEM private/public key block. relay captures group 1 (the
        // body) and replaces only the body; zerocmux redacts the WHOLE block (header,
        ...
        // dotall flag (zerocmux's SentryRegexPattern sets only .caseInsensitive).
