...
public struct SentryScrubber: Sendable {
    ...
    ///
    /// This is zerocmux's equivalent of relay's `@common` `@urlauth` rule
    ...
    /// ``ScrubberDenylists/valuePatterns`` because zerocmux's redaction loop treats
    ...
    /// `@common` ports and zerocmux's own provider-key / JWT / AWS rules live in one
    ...
    /// sentry-python `DEFAULT_DENYLIST`). zerocmux's substring-contains semantics
