...
struct SessionSnapshotRepositoryTests {
    ...
    @Test("snapshot file paths derive from the sanitized bundle identifier under zerocmux/")
    func snapshotFileURLShape() throws {
        ...
        #expect(primary.path == dir.appendingPathComponent("zerocmux/session-com.cmux_odd_id.json").path)
        #expect(backup.path == dir.appendingPathComponent("zerocmux/session-com.cmux_odd_id-previous.json").path)
    }

    @Test("nil and blank bundle identifiers fall back to com.kernelalex.zerocmux")
    func bundleIdentifierFallback() throws {
        ...
        for identifier in [nil, "  "] as [String?] {
            ...
            let primary = try #require(repository.defaultSnapshotFileURL())
            #expect(primary.lastPathComponent == "session-com.kernelalex.zerocmux.json")
        }
    }
