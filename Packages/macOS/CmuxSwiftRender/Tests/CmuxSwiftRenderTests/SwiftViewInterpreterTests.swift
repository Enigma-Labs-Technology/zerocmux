...
@Suite struct SwiftViewInterpreterTests {
    ...
    @Test func capturesButtonCmuxActionWithNamedParams() {
        ...
        VStack {
            for i in 0..<2 {
                Button("select \\(i)") { zerocmux("workspace.select", workspace_id: "ws-\\(i)") }
            }
        }
        """)
        #expect(node?.children.count == 2)
        #expect(node?.children.first?.action?.commands == [.zerocmux(method: "workspace.select", params: ["workspace_id": "ws-0"])])
        #expect(node?.children.last?.action?.commands == [.zerocmux(method: "workspace.select", params: ["workspace_id": "ws-1"])])
    }
    ...
    @Test func labelFormButtonCapturesActionAndLabel() {
        let node = interp.evaluate("""
        VStack {
            Button(action: { zerocmux("workspace.select", workspace_id: "w-9") }) {
        ...
        #expect(button?.action?.commands == [.zerocmux(method: "workspace.select", params: ["workspace_id": "w-9"])])
    ...
    @Test func capturesOnTapGestureActionOnRichRow() {
        ...
        VStack {
            HStack { Text("●"); Text("home") }
                .onTapGesture { zerocmux("workspace.select", workspace_id: "abc-123") }
        }
        """)
        #expect(node?.children.first?.kind == .hstack)
        #expect(node?.children.first?.action?.commands == [.zerocmux(method: "workspace.select", params: ["workspace_id": "abc-123"])])
    }

    @Test func memberAccessOnObjectAndArray() {
        let data = SwiftValue.object(["name": .string("zerocmux"), "tabs": .array([.int(1), .int(2), .int(3)])])
        let node = interp.evaluate(#"VStack { Text("\(data.name): \(data.tabs.count)") }"#, state: ["data": data])
        #expect(node?.children.first?.text == "zerocmux: 3")
    }
    ...
    @Test func progressGaugeMenuAndContextMenu() {
        ...
        Menu("Actions") {
            Button("Stop") { zerocmux("agent.stop") }
        }
        """)
        ...
        Text("row").contextMenu {
            Button("Open") { zerocmux("workspace.select") }
        }
        """)
    ...
    /// crashing the process (https://github.com/kernelalex/zerocmux/pull/5275
    /// review finding).
