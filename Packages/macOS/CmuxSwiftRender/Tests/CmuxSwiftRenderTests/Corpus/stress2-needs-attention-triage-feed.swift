    ...
    ForEach(needsAttention) { ws in
      ...
      HStack(alignment: .top, spacing: 9) {
        ...
        VStack(alignment: .leading, spacing: 7) {
          ...
          HStack(spacing: 6) {
            Button { zerocmux("workspace.select", workspace_id: ws.id) } label: {
              HStack(spacing: 4) {
            ...
            Button { zerocmux("workspace.markRead", workspace_id: ws.id) } label: {
            ...
            if ws.pr.number > 0 {
              Menu("PR") {
                Button("Open #\(ws.pr.number)") { zerocmux("pr.open", url: ws.pr.url) }
                Button("Copy branch") { zerocmux("workspace.copyBranch", workspace_id: ws.id) }
              }
              .font(.system(size: 10))
            }
          }
      ...
      .onTapGesture { zerocmux("workspace.select", workspace_id: ws.id) }
      .contextMenu {
        Button("Triage") { zerocmux("workspace.select", workspace_id: ws.id) }
        Button("Mark read") { zerocmux("workspace.markRead", workspace_id: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { zerocmux("workspace.togglePin", workspace_id: ws.id) }
      }
    }
