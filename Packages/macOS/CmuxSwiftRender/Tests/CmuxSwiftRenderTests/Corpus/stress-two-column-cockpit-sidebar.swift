...
func prPill(_ ws: Any) -> some View {
    return Group {
        if ws.pr != nil {
            ...
            .help("PR \(ws.pr.label)")
            .onTapGesture { zerocmux("openURL", param: ws.pr.url) }
        } else {
...
func railRow(_ ws: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus") { zerocmux("workspace.focus", param: ws.id) }
        Button(ws.pinned ? "Unpin" : "Pin") { zerocmux("workspace.togglePin", param: ws.id) }
        Button("Mark read") { zerocmux("workspace.markRead", param: ws.id) }
    }
    .help(ws.directory)
    .onTapGesture { zerocmux("workspace.select", param: ws.id) }
}
...
func tabCard(_ tab: Any) -> some View {
    ...
    .contextMenu {
        Button("Focus tab") { zerocmux("tab.focus", param: tab.id) }
        Button(tab.pinned ? "Unpin tab" : "Pin tab") { zerocmux("tab.togglePin", param: tab.id) }
        Button("Close tab") { zerocmux("tab.close", param: tab.id) }
    }
    .onTapGesture { zerocmux("tab.focus", param: tab.id) }
}
...
func cockpitHeader() -> some View {
    return HStack(spacing: 8) {
        ...
        Spacer()
        Menu("•••") {
            Button("Collapse all reads") { zerocmux("workspace.markAllRead") }
            Button("New workspace") { zerocmux("workspace.create") }
            Section("Sort") {
                Button("By unread") { zerocmux("workspace.sort", param: "unread") }
                Button("By recent") { zerocmux("workspace.sort", param: "recent") }
            }
        }
        .foregroundColor(.secondary)
        Text(clockLabel())
    ...
    HSplitView {
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 6) {
                ...
                Button("+") { zerocmux("tab.create", param: selectedId) }
                    .foregroundColor(.secondary)
