...
func portChip(_ p: Int, _ wsId) -> some View {
    ...
    .onTapGesture { zerocmux("workspace.select", workspace_id: wsId) }
}
...
func cell(_ ws) -> some View {
    ...
    .contextMenu {
        Button(action: { zerocmux("workspace.select", workspace_id: ws.id) }) {
            Label("Focus workspace", systemName: "scope")
        }
        if let pr = ws.pr {
            Button(action: { zerocmux("open.url", url: pr.url) }) {
