    ...
    VStack(alignment: .leading, spacing: 14) {
                ...
                ForEach(Array(reviewable.enumerated()), id: \.offset) { idx, ws in
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack(alignment: .top, spacing: 8) {
                            ...
                            // actions menu
                            Menu("") {
                                Button("Open PR #\(pr.number)") { zerocmux("openURL", param: pr.url) }
                                Button("Focus workspace") { zerocmux("selectWorkspace", param: ws.id) }
                                Button("Approve") { zerocmux("prApprove", param: pr.number) }
                                Button("Request changes") { zerocmux("prRequestChanges", param: pr.number) }
                                Button("Refresh PR") { zerocmux("prRefresh", param: pr.number) }
                                if pr.status == "approved" {
                                    Button("Merge") { zerocmux("prMerge", param: pr.number) }
                                }
                            }
                        }
                    ...
                    .contextMenu {
                        Button("Open PR #\(pr.number)") { zerocmux("openURL", param: pr.url) }
                        Button("Focus workspace") { zerocmux("selectWorkspace", param: ws.id) }
                        Button("Refresh") { zerocmux("prRefresh", param: pr.number) }
                    }
                    .onTapGesture { zerocmux("selectWorkspace", param: ws.id) }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    zerocmux("prRefreshAll", param: total)
                } label: {
                    HStack(spacing: 4) {
