    ...
    VStack(alignment: .leading, spacing: 14) {
                    ...
                    if let pr = w.pr {
                        ...
                        VStack(alignment: .leading, spacing: 9) {
                            ...
                            HStack(alignment: .top, spacing: 8) {
                                ...
                                Menu {
                                    Button("Open #\(pr.number)") { zerocmux("pr.open", url: pr.url) }
                                    Button("Focus workspace") { zerocmux("workspace.select", workspace_id: w.id) }
                                    Divider()
                                    switch pr.status {
                                    case "review_required":
                                        Button("Approve") { zerocmux("pr.approve", number: pr.number) }
                                        Button("Request changes") { zerocmux("pr.requestChanges", number: pr.number) }
                                    case "changes_requested":
                                        Button("Re-review") { zerocmux("pr.review", number: pr.number) }
                                    case "approved":
                                        Button("Merge") { zerocmux("pr.merge", number: pr.number) }
                                    default:
                                        Button("View checks") { zerocmux("pr.checks", number: pr.number) }
                                    }
                                    if pr.stale {
                                        Button("Refresh (stale)") { zerocmux("pr.refresh", number: pr.number) }
                                    }
                                } label: {
                        ...
                        .contextMenu {
                            Button("Open #\(pr.number)") { zerocmux("pr.open", url: pr.url) }
                            Button("Focus workspace") { zerocmux("workspace.select", workspace_id: w.id) }
                            Button("Refresh") { zerocmux("pr.refresh", number: pr.number) }
                        }
                        .onTapGesture {
                            zerocmux(pr.stale ? "pr.refresh" : "workspace.select", workspace_id: w.id)
                        }
        ...
        ViewThatFits {
            HStack(spacing: 10) {
                ...
                Spacer()
                Button {
                    zerocmux("pr.refreshAll", count: total)
                } label: {
                    HStack(spacing: 4) {
            ...
            HStack {
                Text("\(total) in queue")
                    ...
                    .foregroundColor(.secondary)
                Spacer()
                Button("Refresh") { zerocmux("pr.refreshAll", count: total) }
                    .font(.caption)
                    .fontWeight(.semibold)
            }
        }
