...
func row(_ w, _ i) -> some View {
  ...
  .opacity(g == "R" ? 0.6 : 1.0)
  .onTapGesture { zerocmux("workspace.select", workspace_id: w.id) }
  .keyboardShortcut(.return)
  .contextMenu {
    Button("Select") { zerocmux("workspace.select", workspace_id: w.id) }
    Button("Stop agent") { zerocmux("workspace.stop", workspace_id: w.id) }
    Button(w.pinned == true ? "Unpin" : "Pin") { zerocmux("workspace.pin", workspace_id: w.id) }
    if w.pr != nil {
      Button("Open PR") { zerocmux("workspace.openPR", workspace_id: w.id) }
    }
  }
