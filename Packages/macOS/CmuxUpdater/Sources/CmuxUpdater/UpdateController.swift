...
/// Coordinates zerocmux's custom Sparkle update flow: owns the `SPUUpdater` and its
...
public final class UpdateController {
    ...
    /// Whether the running build is a zerocmux DEV/staging build that must never be compared against
    ...
    ///   - settings: The Sparkle defaults/migration configuration. Defaults to zerocmux's hourly check.
    ...
    public init(log: any UpdateLogging,
        ...
        if isDevLikeBundle {
            // DEV (`com.kernelalex.zerocmux.debug[.<tag>]`) and staging (`com.kernelalex.zerocmux.staging[.<tag>]`)
            ...
            // zerocmux's launch/background probe is short-circuited by the `automaticallyChecksForUpdates`
...
extension UpdateController {
    /// Whether `bundleIdentifier` is a zerocmux DEV (`com.kernelalex.zerocmux.debug[.<tag>]`) or staging
    /// (`com.kernelalex.zerocmux.staging[.<tag>]`) build.
    ///
    ...
    static func isDevLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.kernelalex.zerocmux.debug"
            ...
            || bundleIdentifier == "com.kernelalex.zerocmux.staging"
