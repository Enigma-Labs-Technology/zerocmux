...
/// Registers zerocmux's Sparkle preference defaults and performs the one-time migration that
...
/// configuration, so this is a value type constructed with them (defaulting to zerocmux's
...
public struct UpdateSettings: Sendable {
    ...
    /// zerocmux's marker that the v2 automatic-checks migration already ran.
    ...
    /// The scheduled-check interval zerocmux registers (1h by default).
    public let scheduledCheckInterval: TimeInterval

    /// Creates the settings with zerocmux's defaults.
    ///
