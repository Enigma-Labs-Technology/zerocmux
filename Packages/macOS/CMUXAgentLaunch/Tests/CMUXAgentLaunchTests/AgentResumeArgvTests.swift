...
struct AgentResumeArgvTests {
    ...
    func opencodeResumeDropsInternalTUISettingsSelector() {
        ...
        #expect(
            ...
            ) == .resolved(["zerocmux", "omo", "--session", "SID", "--model", "anthropic/claude-sonnet-4-6"])
        )
        #expect(
            AgentResumeArgv().builtInKind(
                kind: "opencode",
    ...
    @Test("zerocmux wrapper launchers resolve before per-kind verbs")
    func launcherWrappers() {
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "claudeTeams", sessionId: "SID", executablePath: nil, arguments: ["zerocmux", "claude-teams"]
            ) == .resolved(["zerocmux", "claude-teams", "--resume", "SID"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
        ...
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "codexTeams", sessionId: "SID", executablePath: nil, arguments: ["zerocmux", "codex-teams"]
            ) == .resolved(["zerocmux", "codex-teams", "resume", "SID", "-c", "check_for_update_on_startup=false"])
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omo", sessionId: "SID", executablePath: nil, arguments: ["zerocmux", "omo"]
            ) == .resolved(["zerocmux", "omo", "--session", "SID"])
        )
        // One-shot wrappers have no resumable form (omx and omc share an arm; exercise each).
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omx", sessionId: "SID", executablePath: nil, arguments: ["zerocmux", "omx"]
            ) == .resolved(nil)
        )
        #expect(
            AgentResumeArgv().launcherResolution(
                launcher: "omc", sessionId: "SID", executablePath: nil, arguments: ["zerocmux", "omc"]
            ) == .resolved(nil)
        )
    ...
    func renderedPortableClaudeResumeShellCommand() {
        ...
        // Launcher resumes that resolve to zerocmux's own CLI emit no bare `claude`:
        // already-portable quoted words stay unwrapped.
        #expect(
            AgentResumeArgv.renderedPortableClaudeResumeShellCommand(
                parts: ["/Applications/zerocmux.app/Contents/Resources/bin/zerocmux", "claude-teams", "--resume", "SID"],
                quote: quote
            ) == "'/Applications/zerocmux.app/Contents/Resources/bin/zerocmux' 'claude-teams' '--resume' 'SID'"
        )
    }
    ...
    func renderedPortableCodexResumeShellCommand() {
        ...
        #expect(
            AgentResumeArgv.renderedPortableCodexResumeShellCommand(
                parts: ["/Applications/zerocmux.app/Contents/Resources/bin/zerocmux", "codex-teams", "resume", "SID"],
                quote: quote
            ) == "'/Applications/zerocmux.app/Contents/Resources/bin/zerocmux' 'codex-teams' 'resume' 'SID'"
        )
    }
}
