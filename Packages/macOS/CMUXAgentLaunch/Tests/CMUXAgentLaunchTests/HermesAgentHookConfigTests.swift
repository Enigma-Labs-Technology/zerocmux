...
struct HermesAgentHookConfigTests {
    ...
    @Test("Coalesces multiple zerocmux hooks for the same missing Hermes event")
    func coalescesMultipleHooksForSameMissingEvent() {
        let events = [
            HermesAgentHookConfig.Event(name: "pre_approval_request", command: "sh -c 'zerocmux hooks hermes-agent notification'"),
            HermesAgentHookConfig.Event(name: "pre_approval_request", command: "sh -c 'zerocmux hooks feed --source hermes-agent --event pre_approval_request'", timeout: 120),
            HermesAgentHookConfig.Event(name: "post_llm_call", command: "sh -c 'zerocmux hooks hermes-agent agent-response'"),
        ]
        ...
        #expect(
            installed.contains("""
              pre_approval_request:
                - command: "sh -c 'zerocmux hooks hermes-agent notification'"
                  timeout: 5
                - command: "sh -c 'zerocmux hooks feed --source hermes-agent --event pre_approval_request'"
                  timeout: 120
            """)
        )
    ...
    @Test("Coalesces multiple zerocmux hooks for the same existing Hermes event")
        ...
        #expect(
            installed.contains("""
              pre_approval_request:
                ...
                - command: "sh -c 'zerocmux hooks feed --source hermes-agent --event pre_approval_request'"
                  timeout: 120
                # zerocmux hooks hermes-agent end
                - command: "echo user"
                  timeout: 10
            """)
        )
