...
struct AgentForkArgvTests {
    ...
    @Test("zerocmux wrapper launchers use fork verbs")
    func launcherWrappersUseForkVerbs() {
        #expect(
            AgentForkArgv().launcherResolution(
                ...
                sessionId: "SID",
                executablePath: nil,
                arguments: ["zerocmux", "claude-teams", "--worktree", "/tmp/team repo"]
            ) == .resolved(["zerocmux", "claude-teams", "--resume", "SID", "--fork-session", "--worktree", "/tmp/team repo"])
        )
        #expect(
            AgentForkArgv().launcherResolution(
                launcher: "codexTeams",
                sessionId: "SID",
                executablePath: nil,
                arguments: ["zerocmux", "codex-teams", "--model", "gpt-5"]
            ) == .resolved(["zerocmux", "codex-teams", "fork", "SID", "--model", "gpt-5"])
        )
        #expect(
            AgentForkArgv().launcherResolution(
                launcher: "omo",
                sessionId: "SID",
                executablePath: nil,
                arguments: ["zerocmux", "omo", "--model", "anthropic/claude-sonnet-4-6"]
            ) == .resolved(["zerocmux", "omo", "--session", "SID", "--fork", "--model", "anthropic/claude-sonnet-4-6"])
        )
        #expect(
            AgentForkArgv().launcherResolution(
                launcher: "omx",
