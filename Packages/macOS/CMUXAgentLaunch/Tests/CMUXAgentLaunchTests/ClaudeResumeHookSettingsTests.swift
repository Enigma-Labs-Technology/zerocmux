...
/// Regression coverage for https://github.com/kernelalex/zerocmux/issues/5427.
///
/// zerocmux injects Claude Code's hooks from the `Resources/bin/claude` wrapper,
/// which is first on `PATH` inside zerocmux terminals and re-injects the hook
...
@Suite("Claude resume routes through the zerocmux wrapper")
struct ClaudeResumeHookSettingsTests {
    ...
    func staleHookSettingsStripped() throws {
        ...
        // dropped; the wrapper re-applies zerocmux's current hooks at exec time.
        let argv = try #require(
            AgentResumeArgv().builtInKind(
                kind: "claude",
                sessionId: "s",
                ...
                arguments: [
                    ...
                    "opus",
                    "--settings",
                    "/tmp/zerocmux-claude-hook-x/settings.json"
                ]
            )
        )
        #expect(argv == ["claude", "--resume", "s", "--model", "opus"])
    ...
    func mergedHookSettingsKeepsUserKeysOnResume() throws {
        let mergedSettings = #"{"effortLevel":"max","preferredNotifChannel":"notifications_disabled","hooks":{"SessionStart":[{"matcher":"","hooks":[{"type":"command","command":"zerocmux hooks claude session-start","timeout":10}]}]}}"#
