...
/// (`AgentResumeCommandBuilder` in the app target) and the standalone `zerocmux-cli` surface-restore
...
///
/// Resolution order mirrors the historical app builder: a zerocmux wrapper launcher
...
public struct AgentResumeArgv: Sendable, Equatable {
    ...
    /// The shell token that resolves zerocmux's `claude` wrapper at exec time.
    ///
    /// The claude resume/fork argv emits a bare `claude` executable and relies on
    /// zerocmux's `claude` wrapper (which re-injects the hook `--settings` on `--resume`)
    /// being reachable. Inside zerocmux terminals the wrapper is reached two ways: a
    ...
    /// `-lic` launcher. https://github.com/kernelalex/zerocmux/issues/5639
    ///
    ...
    /// fallback used when the variable is unset outside zerocmux.
    ///
    ...
    /// The shell token that resolves zerocmux's `codex` wrapper at exec time.
    ///
    ...
    /// `zerocmux-codex-wrapper` and dropping every zerocmux hook (no `SessionStart`, the
    ...
    /// the hooks fire. https://github.com/kernelalex/zerocmux/issues/5639
    ///
    ...
    /// variable is unset outside zerocmux.
    ///
    ...
    /// environment and resolves the token, and outside zerocmux the unset variable still
    ...
    /// composing on the outside. https://github.com/kernelalex/zerocmux/issues/5639
    ...
    ///
    /// Claude-launcher resumes that resolve to zerocmux's own CLI (`zerocmux claude-teams --resume …`)
    /// emit no bare `claude` executable: they were already portable quoted words, the zerocmux
    ...
    /// layer lets the app resume builder and the zerocmux-cli surface-restore publisher share one
    /// wrapper-routing seam. https://github.com/kernelalex/zerocmux/issues/5639
    ...
    /// Call only for the codex kind. https://github.com/kernelalex/zerocmux/issues/5639
    ...
    /// The result of resolving a zerocmux wrapper launcher (the `claude-teams` / `codex-teams` / `omo`
    /// style launchers zerocmux injects), checked before the per-kind verb.
    public enum LauncherResolution: Sendable, Equatable {
        /// The launcher is a zerocmux wrapper; the associated value is its resume argv, or `nil` when the
    ...
    /// Resolves a resume argv from a zerocmux wrapper launcher, or ``LauncherResolution/passthrough`` when
    ...
    public func launcherResolution(
        ...
        switch launcher {
        case "claudeTeams":
            let parts = commandParts(executablePath: executablePath, arguments: arguments, fallbackExecutable: "zerocmux")
            var tail = parts.tail
    ...
    /// Builds the claude resume argv, routing it through zerocmux's `claude` wrapper
    /// so zerocmux hooks fire on the resumed session.
    ///
    /// zerocmux injects Claude Code's hook `--settings` from the `zerocmux-claude-wrapper`,
    ...
    /// (https://github.com/kernelalex/zerocmux/issues/5639). The captured executable is
