...
Strongly-typed, migratable settings storage for zerocmux. Depends only on
...
let tempDir = FileManager.default.temporaryDirectory
    .appending(path: "zerocmux-readme-\(UUID().uuidString)")
...
The zerocmux repo enforces a strict modern-concurrency policy
(see the zerocmux `CLAUDE.md` "Modern Swift concurrency" section): no locks,
