...
/// The per-user directory that holds zerocmux's control-plane runtime state: the
...
/// the zerocmux app (bundle id `com.kernelalex.zerocmux`) and the standalone `zerocmux` CLI
/// installed at `/usr/local/bin/zerocmux`. On macOS Sequoia, a non-sandboxed process
...
/// (https://github.com/kernelalex/zerocmux/issues/5146).
///
/// This directory therefore resolves to `~/.local/state/zerocmux`, a plain dotfolder
/// macOS does **not** treat as protected app data. It is the sibling of the
/// existing `~/.local/state/zerocmux/crash` breadcrumb directory.
///
/// ```swift
...
/// let socket = CmuxStateDirectory.url(homeDirectory: home).appendingPathComponent("zerocmux.sock")
/// ```
public enum CmuxStateDirectory {
    ...
    public static let directoryName = "zerocmux"

    /// The zerocmux state directory: `<home>/.local/state/zerocmux`.
    ///
