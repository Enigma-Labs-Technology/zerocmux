...
public struct SocketControlPasswordStore: Sendable {
    ...
    private static let legacyKeychainService = "com.kernelalex.zerocmux.socket-control"
    ...
    /// The default password file URL within the zerocmux state directory.
    ///
    /// Resolves to `<directory>/socket-control-password`, where `directory`
    /// defaults to ``CmuxStateDirectory`` (`~/.local/state/zerocmux`). The file lives
    /// outside Application Support so the separately-signed `zerocmux` CLI can read it
    ...
    /// (`~/Library/Application Support/zerocmux/socket-control-password`) into the
    ...
    private static func loadLegacyPasswordFromKeychain() -> String? {
        ...
        let query: [CFString: Any] = [
            ...
            kSecAttrService: "com.kernelalex.zerocmux.socket-control",
