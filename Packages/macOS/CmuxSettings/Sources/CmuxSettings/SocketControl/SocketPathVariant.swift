/// A flavor of the zerocmux app, used to derive per-flavor socket and marker file paths.
///
...
public enum SocketPathVariant: Equatable, Sendable {
    ...
    public var tmpPath: String {
        ...
        case .nightly(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/zerocmux-nightly-\(slug)-last-socket-path"
            }
            return "/tmp/zerocmux-nightly-last-socket-path"
        case .staging(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/zerocmux-staging-\(slug)-last-socket-path"
            }
            return "/tmp/zerocmux-staging-last-socket-path"
        case .dev(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/zerocmux-dev-\(slug)-last-socket-path"
            }
            return "/tmp/zerocmux-dev-last-socket-path"
        }
    }
