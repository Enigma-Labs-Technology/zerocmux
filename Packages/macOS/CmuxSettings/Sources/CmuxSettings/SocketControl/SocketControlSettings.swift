...
/// Pure resolution policy for the zerocmux control socket: where its path lives, which
...
public struct SocketControlSettings {
    ...
    public static let baseDebugBundleIdentifier = "com.kernelalex.zerocmux.debug"
    ...
    public static let legacyStableDefaultSocketPath = "/tmp/zerocmux.sock"
    ...
    public static func initialSocketPathBeforeListenerStart(
        ...
        guard !isDebugBuild,
              normalizedBundleIdentifier(bundleIdentifier) == "com.kernelalex.zerocmux",
    ...
    private static func shouldReserveStableSocketPath(bundleIdentifier: String?, isDebugBuild: Bool) -> Bool {
        ...
        return normalizedBundleIdentifier(bundleIdentifier) != "com.kernelalex.zerocmux"
    }
    ...
    /// The per-user stable socket path (`zerocmux-<uid>.sock` in ``CmuxStateDirectory``, `/tmp` fallback).
    ...
    public static func legacyUserScopedStableSocketPath(currentUserID: uid_t = getuid()) -> String {
        "/tmp/zerocmux-\(currentUserID).sock"
    }
    ...
    public static func isDebugLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.kernelalex.zerocmux.debug"
            || bundleIdentifier.hasPrefix("com.kernelalex.zerocmux.debug.")
    }

    /// Whether the bundle identifier is a tagged DEV build (`com.kernelalex.zerocmux.debug.<tag>`).
    ...
    ///
    /// Resolves to ``CmuxStateDirectory`` (`~/.local/state/zerocmux`) rather than
    /// Application Support: the separately-signed `zerocmux` CLI connects to this
    ...
    /// from other apps" prompt (https://github.com/kernelalex/zerocmux/issues/5146).
