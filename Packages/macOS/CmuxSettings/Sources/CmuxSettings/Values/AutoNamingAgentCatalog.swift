...
public struct AutoNamingAgentOption: Sendable, Hashable {
    ...
    /// `<agent>` segment of `zerocmux hooks <agent> auto-name`.
    public let slug: String
    /// Brand display name shown in the picker (proper noun — not localized).
    public let displayName: String
    /// Whether zerocmux currently knows how to drive this agent as a summarizer.
...
/// bundled `zerocmux` CLI) so the picker and the summarizer dispatch share one
...
public enum AutoNamingAgentCatalog {
    ...
    /// Agents whose binary zerocmux knows how to invoke for a one-shot
    ...
    /// agent until zerocmux learns to drive them).
    ...
    /// True only when zerocmux can drive `slug` as a summarizer. Custom/unknown
