...
///
/// Use this shared formatter anywhere zerocmux shows a ``StoredShortcut`` so named
...
public struct ShortcutDisplayFormatter: Sendable {
    ...
    /// Formats modifier booleans in zerocmux's standard `Control`, `Option`, `Shift`, `Command` order.
    ///
    /// - Parameters:
    ///   - command: Whether the Command modifier is present.
    ///   - shift: Whether the Shift modifier is present.
