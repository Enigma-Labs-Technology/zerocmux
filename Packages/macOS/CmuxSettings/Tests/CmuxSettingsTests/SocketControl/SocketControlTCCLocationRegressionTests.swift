...
/// Regression coverage for https://github.com/kernelalex/zerocmux/issues/5146.
///
/// The control socket directory and the socket password file are read by the
/// separately code-signed `zerocmux` CLI on every agent session-start/stop hook. On
...
@Suite struct SocketControlTCCLocationRegressionTests {
    ...
    @Test func controlSocketDirectoryIsOutsideTCCProtectedAppData() throws {
        ...
        #expect(directory.hasSuffix("/.local/state/zerocmux"))
    }
    ...
    @Test func socketPasswordFileIsOutsideTCCProtectedAppData() throws {
        ...
        #expect(path.hasSuffix("/.local/state/zerocmux/socket-control-password"))
    }
}
