...
/// the macOS TCC-protected app-data roots, or the separately-signed `zerocmux` CLI
...
@Suite struct CmuxStateDirectoryTests {
    @Test func resolvesUnderLocalStateNotLibrary() {
        ...
        #expect(url.path == "/Users/test-user/.local/state/zerocmux")
    ...
    @Test func legacyApplicationSupportURLPointsAtOldLocation() {
        ...
        #expect(legacy?.lastPathComponent == "zerocmux")
        #expect(legacy?.path.contains("/Application Support/zerocmux") == true)
    }
}
...
@Suite(.serialized) struct SocketControlPasswordMigrationTests {
    private func tempDir() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-pw-migrate-\(UUID().uuidString)", isDirectory: true)
    }
    ...
    /// TCC-protected directory (https://github.com/kernelalex/zerocmux/issues/5146).
