...
extension TerminalSurface {
    ...
    func createSurface(for view: any TerminalSurfaceNativeViewing, source: RuntimeSurfaceCreationSource) {
        ...
        // Install the PTY tee so the injected byte-tee binding receives every
        // byte the read thread produces, in order, before the VT parser runs.
        // The lease is released alongside `surfaceCallbackContext` when the
        // surface tears down.
        byteTeeLease?.release()
        byteTeeLease = byteTee.installTee(on: createdSurface, surfaceID: id)
