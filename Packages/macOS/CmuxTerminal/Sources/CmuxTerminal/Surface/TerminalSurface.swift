...
/// `RendererRealizationController.shared` /
...
public final class TerminalSurface: Identifiable, ObservableObject {
    ...
    /// zerocmux renderer reclamation: whether the current runtime surface's GPU
    ...
    /// The retained byte-tee lease for the libghostty PTY tee callback (zerocmux
    /// fork extension). Installed in `createSurface` after
    /// `ghostty_surface_new` succeeds. The injected ``TerminalByteTeeBinding``
    /// decides what (if anything) consumes the tee'd bytes.
    ///
    ...
    /// io-reader thread.
    var byteTeeLease: (any TerminalByteTeeLease)?
    ...
    deinit {
        ...
        // Mirror teardownSurface/suspend: release the retained byte-tee
        ...
        // teardown) would otherwise leak the tee userdata and leave stale
        // replay buffers keyed by the old id. If teardown already ran, it nil'd
        // byteTeeLease, so teeLease is nil here and ?.release() no-ops.
        let teeLease = byteTeeLease
        byteTeeLease = nil
