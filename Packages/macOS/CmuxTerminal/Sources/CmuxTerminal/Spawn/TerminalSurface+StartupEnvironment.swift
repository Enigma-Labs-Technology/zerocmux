...
extension TerminalSurface {
    ...
    /// Applies the managed zerocmux context identity keys and protects them.
    ...
    public static func installClaudeCommandShimIfPossible(
        ...
        let shimDirectory = temporaryDirectory
            .appendingPathComponent("zerocmux-cli-shims", isDirectory: true)
        ...
        do {
            ...
            if [[ ! -x "$cmux_wrapper" ]]; then
                cmux_cli="$(command -v zerocmux 2>/dev/null || true)"
                if [[ -n "$cmux_cli" ]]; then
            ...
            for cmux_entry in ${PATH:-}; do
                if [[ "$cmux_entry" == "$CMUX_CLAUDE_WRAPPER_SHIM_ROOT" || "$cmux_entry" == */zerocmux-cli-shims/* || "$cmux_entry" == */zerocmux-cli-shims ]]; then
                    continue
                fi
            ...
            // dir so typed `codex` resolves to zerocmux-codex-wrapper through the
            ...
            // resume through the wrapper and keep zerocmux hooks.
    ...
    /// bundled `zerocmux-codex-wrapper` exists alongside `zerocmux-claude-wrapper`. The
    /// shim resolves and execs the codex wrapper; if the wrapper is gone it
    /// strips every zerocmux shim dir from `PATH` and execs the real `codex`, so the
    ...
    public static func installCodexCommandShimIfPossible(
        ...
        let codexWrapperURL = claudeWrapperURL
            ...
            .appendingPathComponent("zerocmux-codex-wrapper", isDirectory: false)
            .standardizedFileURL
        ...
        do {
            ...
            if [[ ! -x "$cmux_wrapper" && -n "${CMUX_BUNDLED_CLI_PATH:-}" ]]; then
                cmux_candidate="$(dirname "$CMUX_BUNDLED_CLI_PATH")/zerocmux-codex-wrapper"
                if [[ -x "$cmux_candidate" ]]; then
                    cmux_wrapper="$cmux_candidate"
                fi
            fi
            if [[ ! -x "$cmux_wrapper" ]]; then
                cmux_cli="$(command -v zerocmux 2>/dev/null || true)"
                if [[ -n "$cmux_cli" ]]; then
                    cmux_candidate="$(dirname "$cmux_cli")/zerocmux-codex-wrapper"
                    if [[ -x "$cmux_candidate" ]]; then
                        cmux_wrapper="$cmux_candidate"
                    fi
                fi
            ...
            IFS=:
            for cmux_entry in ${PATH:-}; do
                if [[ "$cmux_entry" == "$CMUX_CODEX_WRAPPER_SHIM_ROOT" || "$cmux_entry" == */zerocmux-cli-shims/* || "$cmux_entry" == */zerocmux-cli-shims ]]; then
                    continue
                fi
                if [[ -z "$cmux_path_without_shim" ]]; then
                    cmux_path_without_shim="$cmux_entry"
    ...
    public static func shellIntegrationDirectoryExists(
        ...
        Logger(subsystem: "com.kernelalex.zerocmux", category: "ghostty.initialization")
            .error("zerocmux shell-integration dir missing at \(integrationDir, privacy: .private); spawning shell without zerocmux shell integration so the user's shell config still loads")
        return false
    }
    ...
    public static func applyManagedShellSpecificStartupEnvironment(
        ...
        func bundledBootstrapIsReadable(_ relativePath: String) -> Bool {
            ...
            Logger(subsystem: "com.kernelalex.zerocmux", category: "ghostty.initialization")
                .error("zerocmux \(shellName, privacy: .public) bootstrap unreadable at \(path, privacy: .private); skipping zerocmux shell-startup redirection so the user's shell config still loads")
            return false
        }
        switch shellName {
        case "zsh":
        ...
        case "bash":
            ...
            let bashBootstrapPath = (integrationDir as NSString).appendingPathComponent("zerocmux-bash-bootstrap.bash")
            do {
                ...
                Logger(subsystem: "com.kernelalex.zerocmux", category: "ghostty.initialization")
                    .error("zerocmux bash bootstrap unreadable at \(bashBootstrapPath, privacy: .private): \(error.localizedDescription, privacy: .public); bash shell integration will not load")
            }
        case "fish":
    ...
    /// The managed fish launch command sourcing the zerocmux integration file.
