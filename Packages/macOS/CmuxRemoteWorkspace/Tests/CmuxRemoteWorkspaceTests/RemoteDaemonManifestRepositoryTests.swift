...
struct RemoteDaemonManifestRepositoryTests {
    ...
    private func temporaryHome() throws -> URL {
        let home = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("zerocmux-manifest-repo-tests-\(UUID().uuidString.prefix(8))", isDirectory: true)
    ...
    @Test("the cache path is versioned by platform under the zerocmux state directory")
    func cachePathShape() throws {
        ...
        #expect(url.path == home.path + "/.local/state/zerocmux/remote-daemons/0.62.0/linux-arm64/cmuxd-remote")
        var isDirectory: ObjCBool = false
        let rootExists = FileManager.default.fileExists(
            atPath: home.appendingPathComponent(".local/state/zerocmux/remote-daemons").path,
