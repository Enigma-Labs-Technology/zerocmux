...
/// Minimal one-shot unix socket server standing in for the local zerocmux
...
private final class FakeUnixSocketServer: @unchecked Sendable {
    ...
    init(response: Data) throws {
        ...
        path = NSTemporaryDirectory() + "zerocmux-relay-test-\(UUID().uuidString.prefix(8)).sock"
...
struct RemoteCLIRelayServerTests {
    ...
    func authAndForwardRoundTrip() throws {
        ...
        #expect(challenge["protocol"] as? String == "zerocmux-relay-auth")
