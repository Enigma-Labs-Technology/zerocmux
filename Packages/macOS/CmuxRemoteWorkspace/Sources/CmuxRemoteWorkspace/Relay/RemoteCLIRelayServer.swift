...
/// Loopback TCP relay that lets the remote host's `zerocmux` CLI reach the local
/// zerocmux control socket: it listens on `127.0.0.1`, authenticates each
...
///
/// Wire behavior is pinned: the `zerocmux-relay-auth` challenge JSON (protocol,
...
public final class RemoteCLIRelayServer: @unchecked Sendable {
    ...
    ///
    /// - Parameters:
    ///   - localSocketPath: Path of the local zerocmux control unix socket.
