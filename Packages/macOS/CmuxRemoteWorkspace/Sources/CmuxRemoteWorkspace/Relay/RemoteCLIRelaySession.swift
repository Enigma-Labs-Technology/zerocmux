...
extension RemoteCLIRelayServer {
    ...
    /// local zerocmux unix socket and returns the response (faithful lift of the
    ...
    final class Session: @unchecked Sendable {
        ...
        private let challengeProtocol = "zerocmux-relay-auth"
            ...
            while true {
                ...
                if errno == EAGAIN || errno == EWOULDBLOCK {
                    ...
                    throw NSError(domain: "cmux.remote.relay", code: 5, userInfo: [
                        NSLocalizedDescriptionKey: "timed out waiting for local zerocmux response",
                    ])
                }
                throw NSError(domain: "cmux.remote.relay", code: 6, userInfo: [
                    NSLocalizedDescriptionKey: "failed to read local zerocmux response",
                ])
            }
            return response
        }
