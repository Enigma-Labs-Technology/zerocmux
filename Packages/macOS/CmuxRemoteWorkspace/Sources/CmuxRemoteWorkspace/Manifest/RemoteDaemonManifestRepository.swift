...
public struct RemoteDaemonManifestRepository: Sendable {
    ...
    /// Creates a repository rooted at `homeDirectory`'s zerocmux state cache.
    ///
    /// - Parameters:
    ...
    ///
    /// Cached under the non-TCC zerocmux state directory (matching the CLI's
    /// `remoteDaemonCacheURL`) rather than Application Support, so the
    /// separately-signed CLI can read it on `zerocmux ssh` without tripping the
    ...
    public func fetchManifest(releaseURL: String, version: String) -> WorkspaceRemoteDaemonManifest? {
        ...
        request.setValue("zerocmux/\(version)", forHTTPHeaderField: "User-Agent")
