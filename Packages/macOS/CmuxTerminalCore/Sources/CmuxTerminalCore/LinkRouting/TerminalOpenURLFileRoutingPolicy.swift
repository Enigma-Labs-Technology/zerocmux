...
/// Decides whether a terminal open-URL target may be handled by zerocmux's file UI.
public struct TerminalOpenURLFileRoutingPolicy: Sendable {
    /// Creates the file-routing policy.
    public init() {}

    /// Returns whether zerocmux may attempt to open the target in its file preview UI.
    ///
    ...
    /// open-URL payload represents a local file shape that zerocmux is allowed to
