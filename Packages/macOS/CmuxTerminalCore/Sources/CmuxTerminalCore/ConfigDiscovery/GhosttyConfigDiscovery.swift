...
/// Resolves where zerocmux's Ghostty configuration lives and what it should inject,
...
/// includes) for the font and appearance directives that drive zerocmux's
...
public struct GhosttyConfigDiscovery {
    ...
    /// already covers the ranges zerocmux would otherwise auto-map.
    ...
    /// Returns only the CJK mappings zerocmux should auto-inject after respecting
    ...
    /// application-support config paths zerocmux may load at runtime.
    ...
    /// Whether zerocmux should inject its managed CJK `font-codepoint-map` fallback.
    ...
    /// Whether zerocmux should apply its managed default appearance across the
    ...
    /// Computes the resolved plain-theme override zerocmux must inject when the
    ...
    public func conditionalThemeOverrideConfigContents(
        ...
        // syntax, even when both sides resolve to the same theme. `zerocmux themes
        ...
        // https://github.com/kernelalex/zerocmux/issues/3459. Only override sides the
    ...
    /// summarizes the font directives relevant to zerocmux's injected CJK fallback.
    ...
    /// `config-file` processing, including native Ghostty, legacy, and zerocmux
    ...
    /// Whether zerocmux should load the legacy `config` file because the new
    ...
    /// Resolves the zerocmux Application Support Ghostty config file URLs for the
