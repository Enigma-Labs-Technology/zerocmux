...
/// The resolved zerocmux terminal configuration: fonts, colors, theme, scrollback,
/// and sidebar appearance parsed from the user's ghostty config files plus
/// zerocmux's managed defaults.
///
...
/// resolves themes by light/dark color scheme, and folds in zerocmux's managed
...
public struct GhosttyConfig {
    ...
    /// Creates a config with zerocmux's built-in default appearance, before any
    ...
    /// Applies zerocmux's managed default theme for `preferredColorScheme` by parsing
    ...
    /// The zerocmux managed default theme name for `preferredColorScheme`.
    ...
    /// The zerocmux managed default theme's config contents for `preferredColorScheme`,
    ...
    /// The on-disk URL of the zerocmux managed default theme for `preferredColorScheme`,
    ...
    /// directives that determine whether zerocmux should apply its managed default
    ...
    public struct UserAppearanceConfigSummary {
        ...
        /// Whether zerocmux should apply its managed default appearance: true unless
        ...
        /// (https://github.com/kernelalex/zerocmux/issues/7161).
    ...
    /// Whether zerocmux should inject its managed default appearance: true unless
    ...
    /// `.light` and `nil` for `.dark`. zerocmux injects a resolved plain `theme = X`
    ...
    /// https://github.com/kernelalex/zerocmux/issues/3459). Injecting for an unset
    ...
    /// The ordered list of filesystem paths zerocmux searches for the given theme
