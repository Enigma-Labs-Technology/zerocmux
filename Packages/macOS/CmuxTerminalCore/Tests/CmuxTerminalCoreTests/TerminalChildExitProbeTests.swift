...
@Suite struct TerminalChildExitProbeTests {
    @Test func loadReturnsEmptyPayloadForMissingFile() {
        #expect(TerminalChildExitProbe().load(at: "/tmp/zerocmux-termcore-missing-\(UUID().uuidString)") == [:])
    }

    @Test func loadRoundTripsJSONPayload() throws {
        let path = NSTemporaryDirectory() + "zerocmux-termcore-probe-\(UUID().uuidString).json"
