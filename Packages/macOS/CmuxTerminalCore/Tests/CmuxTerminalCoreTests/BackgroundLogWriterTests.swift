...
struct BackgroundLogWriterTests {
    private func makeTempURL() -> URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-bg-writer-test-\(UUID().uuidString).log")
    }
    ...
    @Test func writesEnqueuedLinesInOrderWithMonotonicSequence() async {
        ...
        for (index, line) in lines.enumerated() {
            ...
            #expect(line.contains("zerocmux bg: message-\(index)"))
    ...
    @Test func formatsLineDeterministicallyFromInjectedClock() async {
        ...
        #expect(line.hasSuffix("zerocmux bg: hello\n"))
    }
    ...
    @Test func capturesCallerThreadLabel() async {
        ...
        #expect(lines.contains { $0.contains("thread=background") && $0.contains("zerocmux bg: from-background") })
    }
    ...
    @Test func boundedBufferDropsOldestUnderFlood() async {
        ...
        #expect(lines.last?.contains("zerocmux bg: SENTINEL") == true)
