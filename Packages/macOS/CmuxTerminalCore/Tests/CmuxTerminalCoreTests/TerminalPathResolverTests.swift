...
@Suite struct TerminalQuicklookPathResolutionTests {
    @Test func fallsBackToStrippedPathWhenLiteralPathIsMissing() {
        let strippedPath = "/tmp/zerocmux-cmdclick-path.md"
        #expect(
    ...
    @Test func prefersLiteralPathThatReallyEndsWithDot() {
        let literalPath = "/tmp/zerocmux-cmdclick-literal-dot.md."
        let strippedPath = "/tmp/zerocmux-cmdclick-literal-dot.md"
        #expect(
    ...
    @Test func prefersLiteralPathThatReallyEndsWithParen() {
        let literalPath = "/tmp/zerocmux-cmdclick-literal-paren)"
        let strippedPath = "/tmp/zerocmux-cmdclick-literal-paren"
        #expect(
            TerminalPathResolver(fileExists: existsIn([literalPath, strippedPath])).resolveQuicklookPath(
                literalPath,
                cwd: "/tmp"
    ...
    @Test func unquotesShellQuotedToken() {
        let existingFile = "/tmp/zerocmux quicklook spaced.md"
        #expect(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveQuicklookPath(
                "\"\(existingFile)\"",
                cwd: "/tmp"
    ...
    @Test func unescapesBackslashEscapedSpaces() {
        let existingFile = "/tmp/zerocmux quicklook escaped.md"
        #expect(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveQuicklookPath(
                "/tmp/zerocmux\\ quicklook\\ escaped.md",
                cwd: "/tmp"
            ) == existingFile
        )
    }
...
@Suite struct TerminalVisibleLineResolutionTests {
    ...
    @Test func resolvesRawSegmentUnderColumn() throws {
        let existingFile = "/tmp/zerocmux-visible-line.md"
        let line = "open /tmp/zerocmux-visible-line.md now"
        ...
        #expect(resolution.rawToken == "/tmp/zerocmux-visible-line.md")
    }

    @Test func resolvesShellEscapedTokenSpanningSpaces() throws {
        let existingFile = "/tmp/zerocmux visible escaped.md"
        let line = "cat /tmp/zerocmux\\ visible\\ escaped.md"
        let resolution = try #require(
            TerminalPathResolver(fileExists: existsIn([existingFile])).resolveVisibleLinePath(
                line,
                column: 6,
