...
/// row-for-row inside a single `SettingsCard`: Enable zerocmux Browser,
...
public struct BrowserSection: View {
        ...
        SettingsCard {
            ...
            if engine.current == .custom {
                ...
                SettingsCardRow(
                    ...
                    subtitle: String(localized: "settings.browser.customSearchEngineURLTemplate.subtitle", defaultValue: "Use {query} or %s for the search terms. Without a placeholder, zerocmux appends q=."),
                    controlWidth: 330
                ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.browser.hiddenWebViewDiscardDelay.subtitle", defaultValue: "How long a browser tab must stay hidden before zerocmux frees its page memory. Active downloads, popups, developer tools, fullscreen, and loading pages are skipped."),
                controlWidth: Self.columnWidth
            ) {
                HStack(spacing: 8) {
    ...
    private func httpAllowlistRow(model: DefaultsValueModel<String>) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(String(localized: "settings.browser.httpAllowlist.description", defaultValue: "Controls which HTTP (non-HTTPS) hosts can open in zerocmux without a warning prompt. Defaults include localhost, *.localhost, 127.0.0.1, ::1, 0.0.0.0, and *.localtest.me. Remove entries to block them; reset to restore defaults."))
                .cmuxFont(.caption)
