...
/// zerocmux, Terminal Config link, Open Markdown in zerocmux Viewer,
...
/// Warn Before Quit, Warn Before Closing Tab /
...
public struct AppSection: View {
    ...
    private var globalFontMagnificationSubtitle: String {
        ...
        return String(
            ...
            defaultValue: "Scale every font in zerocmux by the same percentage. 100% = design size."
        )
    }
    ...
    private var mainCard: some View {
        SettingsCard {
            ...
            SettingsCardRow(
                ...
                subtitle: languageOverrideTouched || (languageAtAppear != nil && language.current != languageAtAppear) ? String(localized: "settings.app.language.restartSubtitle", defaultValue: "Restart zerocmux to apply") : nil,
            ...
            SettingsCardRow(
                ...
                subtitle: firstClick.current
                    ? String(localized: "settings.app.paneFirstClickFocus.subtitleOn", defaultValue: "When zerocmux is inactive, clicking a pane activates the window and focuses that pane in one click.")
                    : String(localized: "settings.app.paneFirstClickFocus.subtitleOff", defaultValue: "When zerocmux is inactive, the first click only activates the window. Click again to focus the pane.")
            ) {
            ...
            // Open Supported Files in zerocmux
            SettingsCardRow(
                configurationReview: .json("app.openSupportedFilesInCmux"),
                String(localized: "settings.app.openSupportedFilesInCmux", defaultValue: "Open Supported Files in zerocmux"),
                subtitle: String(localized: "settings.app.openSupportedFilesInCmux.subtitle", defaultValue: "Cmd-clicking readable files opens text, code, PDFs, images, audio, video, and Quick Look previews in zerocmux.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.app.configWindow.subtitle", defaultValue: "Open the zerocmux terminal config and generated preview in one utility window."),
                controlWidth: Self.columnWidth
            ) {
            ...
            // Open Markdown in zerocmux Viewer
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.app.openMarkdownInCmuxViewer.subtitle", defaultValue: "When supported file routing is on, Cmd-clicking Markdown files opens the rendered zerocmux markdown viewer instead of the generic file preview.")
            ) {
            ...
            SettingsCardRow(
                ...
                subtitle: String(localized: "settings.notifications.paneFlash.subtitle", defaultValue: "Briefly flash an outline when zerocmux highlights a pane.")
            ) {
