...
public struct AutomationSection: View {
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: codexModel.current
                    ...
                    : String(localized: "settings.automation.codex.subtitleOff", defaultValue: "Codex runs without zerocmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.codex.note", defaultValue: "When enabled, zerocmux wraps the codex command to inject session tracking and notification hooks. Disable if you prefer to manage Codex hooks yourself. zerocmux still tracks live Codex sessions it can observe even when this is off. To also track Codex launched through a custom launcher that bypasses the wrapper (e.g. a subrouter), run `zerocmux hooks setup --agent codex`, which installs hooks into ~/.codex/hooks.json."))
        }
    }
    ...
    private var autoNamingCard: some View {
        SettingsCard {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.workspaceAutoNaming.note", defaultValue: "When enabled, zerocmux summarizes supported agent sessions into short workspace and tab names using each agent's own binary, refreshed as the topic shifts. Manual renames always win and stop auto-naming for that workspace or tab. Uses your agent account for the short summarization calls."))
            if autoNamingModel.current,
               !claudeCodeModel.current,
               autoNamingAgentModel.current == AutoNamingAgentCatalog.autoSlug || autoNamingAgentModel.current == "claude" {
                autoNamingFootnote(String(localized: "settings.automation.workspaceAutoNaming.hooksOffWarning", defaultValue: "Claude Code Integration is off, so Claude sessions will not be auto-named. Other supported agents still name when their zerocmux hooks are installed."))
            }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: ampModel.current
                    ...
                    : String(localized: "settings.automation.amp.subtitleOff", defaultValue: "Amp runs without zerocmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.amp.note", defaultValue: "Hooks must be installed with `zerocmux hooks amp install`. They no-op outside zerocmux terminals. When disabled, the installed Amp plugin stays inactive without needing to be removed."))
        }
    }
        ...
        SettingsCard {
            SettingsCardRow(
                ...
                subtitle: kiroModel.current
                    ...
                    : String(localized: "settings.automation.kiro.subtitleOff", defaultValue: "Kiro runs without zerocmux integration.")
            ) {
            ...
            SettingsCardDivider()
            SettingsCardNote(String(localized: "settings.automation.kiro.note", defaultValue: "Hooks must be installed with `zerocmux hooks kiro install`, then run Kiro with `kiro-cli chat --agent zerocmux` (or set it as your default agent). They no-op outside zerocmux terminals."))
        }
    }
