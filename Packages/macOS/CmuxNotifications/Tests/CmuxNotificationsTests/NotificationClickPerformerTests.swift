...
struct NotificationClickPerformerTests {
    ...
    func selectsExistingFile() {
        ...
        finder.existingPaths = ["/tmp/zerocmux/report.txt"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/zerocmux/report.txt")))
        #expect(finder.selected == ["/tmp/zerocmux/report.txt"])
    ...
    func opensContainingDirectory() {
        ...
        finder.existingPaths = ["/tmp/zerocmux"]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "/tmp/zerocmux/missing.txt")))
        #expect(finder.selected.isEmpty)
        #expect(finder.opened == ["/tmp/zerocmux"])
    }
    ...
    func failsWhenNothingExists() {
        ...
        #expect(performer.perform(.revealInFinder(path: "/tmp/zerocmux/missing.txt")) == false)
        #expect(finder.selected.isEmpty)
        #expect(finder.opened.isEmpty)
    }
    ...
    func expandsTilde() {
        ...
        let expectedPath = expandedHome + "/zerocmux-file.txt"
        finder.existingPaths = [expectedPath]
        let performer = NotificationClickPerformer(finder: finder)

        #expect(performer.perform(.revealInFinder(path: "~/zerocmux-file.txt")))
