...
extension SocketTransport {
    ...
    /// reusable lock marker, based on the well-known zerocmux socket filenames
    ...
    func canReplaceUnmarkedRefusedSocket(at path: String) -> Bool {
        let filename = URL(fileURLWithPath: (path as NSString).standardizingPath)
            .lastPathComponent
            .lowercased()
        let recoverablePrefixes = ["zerocmux-debug-", "zerocmux-nightly-", "zerocmux-staging-"]
        return filename == "zerocmux-debug.sock" ||
            filename == "zerocmux-nightly.sock" ||
            filename == "zerocmux-staging.sock" ||
