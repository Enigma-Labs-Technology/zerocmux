...
public enum ControlCommandExecutionPolicy: Sendable, Equatable {
    ...
    ///
    /// `remotes.*` (the `zerocmux remotes` device-registry verbs) and
    ...
    static let socketWorkerMethods: Set<String> = [
        ...
        // here: upstream serves it on the (excluded) mobile-host data plane,
        // not as a control-socket method, so no execution policy applies.
        "notification.create",
