...
struct ControlCommandExecutionPolicyTests {
    ...
    @Test func remotesPrefixedMethodsRunOnTheSocketWorker() {
        // `zerocmux remotes` verbs make blocking authenticated web API calls, so
    ...
    @Test func fixedWorkerSetRunsOnTheSocketWorker() {
        for method in [
            ...
            "sidebar.custom.open",
            "debug.sidebar.simulate_drag", "debug.window.screenshot",
    ...
    @Test func v2NotificationCreateFamilyRunsOnTheWorkerAndIsMainThreadCallable() {
        // notification.reconcile is deliberately absent: upstream serves it
        // on the (excluded) mobile-host data plane, not as a control-socket
        // method.
        for method in [
