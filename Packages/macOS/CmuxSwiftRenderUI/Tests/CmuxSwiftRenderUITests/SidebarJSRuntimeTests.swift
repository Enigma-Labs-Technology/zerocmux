...
struct SidebarJSRuntimeTests {
    ...
    @Test func buttonTapRunsCmuxCommand() async {
        ...
        await pumpActions()
        #expect(captured == [.zerocmux(method: "workspace.select", params: ["workspace_id": "w1"])])
    }
    ...
    @Test func reorderableCarriesItemKeysAndMoveHandler() async {
        ...
        await pumpActions()
        #expect(captured == [.zerocmux(method: "workspace.reorder", params: ["workspace_id": "a", "index": "1"])])
    }
    ...
    @Test func contextMenuAttachesAsMenuChild() async {
        ...
        await pumpActions()
        #expect(captured == [.zerocmux(method: "workspace.action", params: ["action": "pin", "workspace_id": "w1"])])
