...
def main() -> int:
    ...
    with cmux(SOCKET_PATH) as c:
        ...
        _must(token in cli_text, f"zerocmux read-screen output missing token {token!r}: {cli_text!r}")

        cli_ws_only = _run_cli(cli, ["read-screen", "--workspace", ws_target])
        _must(token in cli_ws_only, f"zerocmux read-screen --workspace output missing token {token!r}: {cli_ws_only!r}")

        cli_text_scrollback = _run_cli(cli, ["read-screen", "--workspace", ws_target, "--surface", surface_target, "--scrollback", "--lines", "80"])
        _must(token in cli_text_scrollback, f"zerocmux read-screen --scrollback output missing token {token!r}: {cli_text_scrollback!r}")
