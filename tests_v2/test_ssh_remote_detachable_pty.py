#!/usr/bin/env python3
"""Integration: zerocmux ssh PTY sessions survive local surface detach and reattach."""
...
def _wait_remote_ready(client: zerocmux, workspace_id: str, timeout_s: float = 45.0) -> None:
    deadline = time.time() + timeout_s
    last_status = {}
    while time.time() < deadline:
...
def _resolve_workspace_id(client: zerocmux, payload: dict, *, before_workspace_ids: set[str]) -> str:
...
def _workspace_row(client: zerocmux, workspace_id: str) -> dict:
...
def _run_surface_probe(client: zerocmux, surface_id: str, command: str, token_prefix: str, timeout_s: float = 18.0) -> str:
...
def _open_ssh_workspace(client: zerocmux, cli: str) -> tuple[str, str, str, str]:
    ...
    _must(session_id.startswith("ssh-"), f"zerocmux ssh did not create a persistent PTY session: {payload}")
    _must(persistent_slot.startswith("ssh-"), f"zerocmux ssh did not create a persistent daemon slot: {payload}")
    ...
    try:
        ...
        if workspace_id:
            try:
                ...
                pass
            try:
                with zerocmux(SOCKET_PATH) as cleanup_client:
                    cleanup_client._call("workspace.close", {"workspace_id": workspace_id})
            except Exception:
                pass

    print("PASS: zerocmux ssh PTY survives local detach and reattaches to the same remote shell")
    return 0
