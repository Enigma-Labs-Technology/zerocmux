...
the synced preview with zerocmux overrides on top of Ghostty base values.
"""
...
def run_probe(
    executable: Path,
    home_directory: Path,
    bundle_identifier: str = "com.kernelalex.zerocmux",
) -> dict:
    result = subprocess.run(
...
def test_uses_cmux_config_ghostty_and_removes_standalone_tab(executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="cmux-config-settings-") as tmp:
        home = Path(tmp)
        cmux_legacy_config = home / "Library" / "Application Support" / "com.kernelalex.zerocmux" / "config"
        cmux_config = (
            home
            / "Library"
            / "Application Support"
        ...
        expect(payload["cmux"]["path"] == str(cmux_config), f"unexpected zerocmux path: {payload}")
        ...
        expect(
            "background = #222222  # from: ~/Library/Application Support/com.kernelalex.zerocmux/config.ghostty:1"
            in synced_contents,
            f"synced preview should use zerocmux override for duplicate keys: {synced_contents}",
        )
        expect(
            "copy-on-select = clipboard  # from: ~/Library/Application Support/com.kernelalex.zerocmux/config.ghostty:2"
            in synced_contents,
...
def test_uses_legacy_cmux_config_when_config_ghostty_is_empty(executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="cmux-config-settings-") as tmp:
        ...
        expect(payload["cmux"]["path"] == str(cmux_legacy_config), f"unexpected zerocmux path: {payload}")
        expect(payload["loadPaths"] == [str(cmux_legacy_config)], f"unexpected load paths: {payload}")
        expect("background = #000000" in payload["cmux"]["contents"], f"wrong zerocmux contents: {payload}")
...
def test_falls_back_to_app_support_ghostty_when_dotconfig_missing(executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="cmux-config-settings-") as tmp:
        ...
        synced_contents = str(payload["synced"]["contents"])
        expect(
            "font-size = 17  # from: ~/Library/Application Support/com.kernelalex.zerocmux/config.ghostty:1"
            in synced_contents,
            f"zerocmux override should win over Ghostty base font-size: {synced_contents}",
        )
        expect(
...
def test_debug_bundle_edits_variant_config_and_loads_variant_when_present(executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="cmux-config-settings-") as tmp:
        ...
        nightly_config = (
            ...
            / "Library"
            / "Application Support"
            / "com.kernelalex.zerocmux.debug.nightly.123"
            / "config.ghostty"
        )
        ...
        payload = run_probe(executable, home, "com.kernelalex.zerocmux.debug.nightly.123")
...
def test_nightly_app_edits_nightly_config_and_falls_back_to_release_when_missing(
    ...
    with tempfile.TemporaryDirectory(prefix="cmux-config-settings-") as tmp:
        ...
        payload = run_probe(executable, home, "com.kernelalex.zerocmux.nightly")
