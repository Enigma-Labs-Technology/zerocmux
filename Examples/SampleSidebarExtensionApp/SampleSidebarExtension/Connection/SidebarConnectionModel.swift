...
final class SidebarConnectionModel {
    ...
    func connectionStatusDidChange(_ status: CmuxSidebarConnectionStatus) {
        ...
        case .connected:
            errorText = nil
        case .waitingForHost:
            errorText = String(localized: "sampleSidebar.waitingForHost", defaultValue: "Waiting for zerocmux")
    ...
    private func apply(_ operation: () async throws -> Void) async {
        do {
            ...
            errorText = String(localized: "sampleSidebar.actionDenied", defaultValue: "zerocmux did not allow that action")
        }
    }
