...
struct SampleSidebarView: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                if let insights = model.insights {
                    ...
                    if let errorText = model.errorText {
                        ...
                        statusBanner(String(localized: "sampleSidebar.selectionLimited", defaultValue: "Review access in zerocmux to enable selecting workspaces from this extension."))
                    }
                    actionBar(insights)
    ...
    private func header(_ insights: SidebarInsightModel) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(String.localizedStringWithFormat(
                String(localized: "sampleSidebar.workspaceCount", defaultValue: "%d workspaces shared by zerocmux"),
                insights.totalCount
            ))
            .font(.system(size: 11))
    ...
    private func workspaceList(_ insights: SidebarInsightModel) -> some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            if !insights.hasWorkspaceMetadata {
                Text(String(localized: "sampleSidebar.metadataLimited", defaultValue: "Workspace metadata has not been shared yet. Review access in zerocmux to show workspace rows."))
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            } else if insights.allWorkspaces.isEmpty {
                Text(String(localized: "sampleSidebar.noWorkspaces", defaultValue: "No workspaces were shared by zerocmux."))
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
            } else {
    ...
    private var waitingState: some View {
        VStack(alignment: .leading, spacing: 8) {
            ProgressView()
                .controlSize(.small)
            Text(model.errorText ?? String(localized: "sampleSidebar.waitingForHost", defaultValue: "Waiting for zerocmux"))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
