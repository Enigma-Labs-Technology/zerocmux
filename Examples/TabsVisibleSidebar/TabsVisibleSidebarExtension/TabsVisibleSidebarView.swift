...
struct TabsVisibleSidebarView: View {
    ...
    private func workspaceList(_ snapshot: CmuxSidebarSnapshot) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            if snapshot.workspaces.isEmpty {
                Text(String(localized: "tabsVisible.noWorkspaces", defaultValue: "No workspaces shared by zerocmux"))
                    .font(.system(size: 12))
    ...
    private var waitingState: some View {
        VStack(alignment: .leading, spacing: 8) {
            ProgressView()
                .controlSize(.small)
            Text(String(localized: "tabsVisible.waiting", defaultValue: "Waiting for zerocmux"))
                .font(.system(size: 12))
                .foregroundStyle(.secondary)
        }
    }
