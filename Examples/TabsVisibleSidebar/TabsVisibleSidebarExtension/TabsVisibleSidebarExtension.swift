...
final class TabsVisibleSidebarExtension: @MainActor CmuxSidebarExtension {
    ...
    func connectionStatusDidChange(_ status: CmuxSidebarConnectionStatus) {
        ...
        case .connected:
            errorText = nil
        case .waitingForHost:
            errorText = String(localized: "tabsVisible.waitingForHost", defaultValue: "Waiting for zerocmux")
    ...
    private func apply(_ operation: () async throws -> Void) async {
        do {
            ...
            errorText = String(localized: "tabsVisible.actionDenied", defaultValue: "zerocmux did not allow that action")
        }
    }
}
