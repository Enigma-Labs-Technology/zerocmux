...
extension CMUXCLI {
    ...
    private func buildMemoryPayload(
        ...
        do {
            ...
            throw CLIError(message: String(localized: "cli.memory.error.diagnosticsUnsupported", defaultValue: "zerocmux memory requires a running zerocmux build that supports memory diagnostics"))
        }
    }
    ...
    private func renderMemoryText(
        ...
        lines.append("  \(appName.isEmpty ? "zerocmux" : appName) pid=\(appPID)")
