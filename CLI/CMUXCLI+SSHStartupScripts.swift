...
extension CMUXCLI {
    ...
    func buildSSHPTYAttachScriptBody(
        ...
        return [
            ...
            "if [ -z \"$cmux_ssh_pty_workspace_id\" ]; then printf '%s\\n' '[zerocmux] required workspace context missing for SSH PTY attach.' >&2; exit 1; fi",
            "if [ -z \"$cmux_ssh_pty_surface_id\" ]; then printf '%s\\n' '[zerocmux] required terminal context missing for SSH PTY attach.' >&2; exit 1; fi",
    ...
    func sshAskpassExecShellScript(passwordCredential: String) -> String {
        ...
        return [
            ...
            "  puts stderr {\\n[zerocmux] Cloud VM SSH credential was rejected; reconnecting.}",
            "  catch {close}",
            "  catch {wait}",
            "  exit 255",
            "}",
            ...
            "  }",
            "  timeout {",
            "    puts stderr {\\n[zerocmux] Cloud VM SSH credential prompt timed out; reconnecting.}",
            "    exit 255",
            "  }",
            "  eof { set status [wait]; exit [lindex $status 3] }",
            "}",
    ...
    private func buildSSHSessionEndShellCommand(
        ...
        return [
            ...
            "&& [ -n \"${CMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${CMUX_SURFACE_ID:-}\" ]; then",
            "zerocmux ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${CMUX_WORKSPACE_ID}\" --surface \"${CMUX_SURFACE_ID}\" --session-id \"${CMUX_SSH_PTY_SESSION_ID:-}\" --lifecycle-id \"${CMUX_SSH_PTY_LIFECYCLE_ID:-}\"\(lifecycleOnlyFlag) >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
}
