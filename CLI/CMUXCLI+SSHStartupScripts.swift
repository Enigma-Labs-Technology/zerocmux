...
extension CMUXCLI {
    ...
    private func buildSSHSessionEndShellCommand(
        ...
        return [
            ...
            "&& [ -n \"${CMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${CMUX_SURFACE_ID:-}\" ]; then",
            "zerocmux ssh-session-end --relay-port \(remoteRelayPort) --workspace \"${CMUX_WORKSPACE_ID}\" --surface \"${CMUX_SURFACE_ID}\" --terminal-lifecycle-id \"${CMUX_TERMINAL_LIFECYCLE_ID:-}\" --session-id \"${CMUX_SSH_PTY_SESSION_ID:-}\" --lifecycle-id \"${CMUX_SSH_PTY_LIFECYCLE_ID:-}\"\(lifecycleOnlyFlag) >/dev/null 2>&1 || true;",
            "fi",
        ].joined(separator: " ")
    }
    ...
    private func buildSSHTerminalSessionLaunchingShellCommand() -> String {
        ...
        return [
            ...
            "&& [ -n \"${CMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${CMUX_SURFACE_ID:-}\" ]; then",
            "CMUXTERM_CLI_RESPONSE_TIMEOUT_SEC=2 zerocmux --socket \"${CMUX_SOCKET_PATH}\" \(arguments) >/dev/null 2>&1;",
            "else",
            "false;",
            "fi",
        ].joined(separator: " ")
