...
extension CMUXCLI {
    private static let ompExtensionMarker = "zerocmux-omp-session-extension-marker"
    private static let ompExtensionFilename = "zerocmux-omp-session.ts"
    private static let ompExtensionSource = #"""
// zerocmux-omp-session-extension-marker v2
// Bridges OMP session lifecycle events into zerocmux's restorable session store.
// Installed by `zerocmux hooks omp install` or `zerocmux hooks setup`.
...
function hookInvocation(subcommand: string, ctx: ExtensionContext, extra: Record<string, unknown> = {}): HookInvocation | null {
  ...
  const zerocmux = process.env.CMUX_OMP_CMUX_BIN || "zerocmux";
  return {
    zerocmux,
    cwd,
    sessionId,
...
function startHook(invocation: HookInvocation, subcommand: string): RunningHook {
  ...
  const completion = new Promise<void>((resolve) => {
    ...
    try {
      child = spawn(invocation.zerocmux, ["hooks", "enqueue", "omp", subcommand], {
        ...
        if !existing.isEmpty, !existing.contains(Self.ompExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "%@ exists and is not a zerocmux extension; leaving it alone"
                ),
                extensionURL.path
            ))
        }
    ...
    func uninstallOmpExtensionHooks(_ _: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No OMP zerocmux extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        guard existing.contains(Self.ompExtensionMarker) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Refusing to remove %@: missing zerocmux marker"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed OMP zerocmux extension from %@"
            ),
            extensionURL.path
        ))
    }
