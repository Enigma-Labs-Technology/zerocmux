...
extension CMUXCLI {
    ...
    /// splice ahead of the user's args to enable zerocmux hooks for one Codex
    ...
    /// may depend on zerocmux's handler running first or last. A `-c hooks.<event>=`
    ...
    /// duplicate zerocmux producer. Only explicit `zerocmux hooks codex install` or
