...
extension CMUXCLI {
    ...
    /// zerocmux subcommand they call and the codex hook timeout (ms). Lifecycle
    ...
    /// truth for `zerocmux-codex-wrapper`'s injection, mirrored from the historic
    ...
    /// splice ahead of the user's args to enable + inject zerocmux's fire-and-forget
    ...
    /// hook returns `{}` to codex instantly and backgrounds the real zerocmux call.
    ...
    func emitCodexWrapperInjectArgs() throws {
        ...
        // invocations, so they are written once into a zerocmux-owned dir (~/.cmux/
        ...
        for event in Self.codexWrapperInjectionEvents {
            let ff = Self.codexFireAndForgetAgentHookShellCommand(
                "zerocmux hooks codex \(event.cmuxSubcommand)", for: codexDef
            )
            let command: String
    ...
    /// The zerocmux-owned directory holding the generated codex hook scripts.
    ...
    static func writeCodexHookScript(subcommand: String, body: String, in dir: URL) -> String? {
        ...
        let url = dir.appendingPathComponent("zerocmux-codex-hook-\(safeName).sh", isDirectory: false)
        let contents = "#!/bin/sh\n\(body)\n"
    ...
    static func codexFireAndForgetAgentHookShellCommand(_ command: String, for def: AgentHookDef) -> String {
        ...
        return [
            ...
            "if [ -z \"$cmux_cli\" ] || [ ! -x \"$cmux_cli\" ]; then cmux_cli=\"$(command -v zerocmux 2>/dev/null || true)\"; fi",
            "agent_pid=\"${CMUX_CODEX_PID:-${PPID:-}}\"",
            "if [ -n \"$CMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$cmux_cli\" ]; then payload=\"$(mktemp \"${TMPDIR:-/tmp}/zerocmux-codex-hook.XXXXXX\" 2>/dev/null || mktemp -t zerocmux-codex-hook 2>/dev/null)\" || { echo '{}'; exit 0; }; cat >\"$payload\" || true; if [ -n \"${CMUX_SOCKET_PATH:-}\" ]; then CMUX_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' zerocmux-codex-hook \"$payload\" \"$cmux_cli\" --socket \"$CMUX_SOCKET_PATH\" \(routedArguments) >/dev/null 2>&1 & else CMUX_CODEX_PID=\"$agent_pid\" nohup sh -c '\(runner)' zerocmux-codex-hook \"$payload\" \"$cmux_cli\" \(routedArguments) >/dev/null 2>&1 & fi; echo '{}'; else echo '{}'; fi",
        ].joined(separator: "; ")
    }
}
