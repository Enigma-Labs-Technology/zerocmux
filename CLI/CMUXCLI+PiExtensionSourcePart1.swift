...
// Bridges Pi session lifecycle, tool telemetry, notifications, and resume bindings into zerocmux.
...
function hookEnvironment(cwd: string, includeSocketPassword = false): NodeJS.ProcessEnv {
  ...
  // Only zerocmux CLI children need the socket credential; keep it out of the generic allowlist.
...
function warn(ctx: ExtensionContext | null, message: string, details: Record<string, unknown> = {}): void {
  const payload = { source: "zerocmux-pi-extension", level: "warning", message, ...details };
  try {
  ...
  try {
    ui?.notify?.("zerocmux Pi integration warning - check the terminal for details", "warning");
  } catch (_) {}
}

function cmuxExecutable(): string {
  return process.env.CMUX_PI_CMUX_BIN || "zerocmux";
}
