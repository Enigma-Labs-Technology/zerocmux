...
enum CLISocketPathResolver {
    ...
    static let legacyDefaultSocketPath = "/tmp/zerocmux.sock"
    private static let fallbackSocketPath = "/tmp/zerocmux-debug.sock"
    private static let nightlySocketPath = "/tmp/zerocmux-nightly.sock"
    ...
    private static func discoverTaggedSockets(limit: Int) -> [String] {
        ...
        for directory in socketDiscoveryDirectories() {
            ...
            for name in entries where name.hasPrefix("zerocmux-debug-") && name.hasSuffix(".sock") {
...
#else
        return "com.kernelalex.zerocmux"
#endif
    }
    ...
    ///
    /// Resolves to ``CmuxStateDirectory`` (`~/.local/state/zerocmux`), matching the
    ...
    /// path (https://github.com/kernelalex/zerocmux/issues/5146). The CLI is a
