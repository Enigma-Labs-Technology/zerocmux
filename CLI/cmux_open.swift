...
extension CMUXCLI {
    ...
    private struct DiffViewerURLMapper {
        static let scheme = "zerocmux-diff-viewer"
        static let sessionHistoryMarker = "zerocmux-diff-viewer"
        ...
        func viewerURL(for fileURL: URL) throws -> URL {
            ...
            if origin.scheme == Self.scheme {
                ...
                // (`zerocmux-diff-viewer://<token>`), and the in-app scheme handler
    ...
    /// server process, which has none of the original `zerocmux diff` invocation's
    ...
    func runDiffCommand(
        ...
        guard parsedArgs.inputs.count <= 1 else {
            throw CLIError(message: "diff accepts at most one patch file. Usage: zerocmux diff [patch-file|-] [options]")
        }
    ...
    private func taggedDiffViewerExecutableURL(socketPath: String) -> URL? {
        ...
        let candidate = URL(fileURLWithPath: homePath, isDirectory: true)
            .appendingPathComponent("Library/Developer/Xcode/DerivedData/zerocmux-\(tag)", isDirectory: true)
            .appendingPathComponent("Build/Products/Debug/zerocmux DEV \(tag).app", isDirectory: true)
            ...
            if isParsingOptions {
                ...
                default:
                    if arg.hasPrefix("-"), arg != "-" {
                        throw CLIError(message: "diff: unknown flag '\(arg)'. Usage: zerocmux diff [patch-file|-] [--source <unstaged|staged|branch|last-turn>] [--workspace <id|ref|index>] [--surface <id|ref|index>] [--window <id|ref|index>] [--session <id>] [--cwd <path>] [--base <ref>] [--focus true|false] [--no-focus] [--title <text>] [--layout split|unified] [--font-size <points>]")
                    }
                }
            }
    ...
    private func readDiffInput(
        ...
        guard let rawInput, rawInput != "-" else {
            guard isatty(STDIN_FILENO) == 0 else {
                throw CLIError(message: "diff requires a patch file, piped stdin, or a git source. Usage: zerocmux diff <patch-file>|-|--unstaged|--staged|--branch|--last-turn")
            }
            let data = FileHandle.standardInput.readDataToEndOfFile()
            return DiffInput(
                patch: try decodeDiffData(data, sourceDescription: "stdin"),
                sourceLabel: "stdin",
                defaultTitle: "zerocmux diff",
                emptyMessage: nil,
                externalURL: nil
            )
        }
        ...
        return DiffInput(
            ...
            defaultTitle: filename.isEmpty ? "zerocmux diff" : filename,
            emptyMessage: nil,
            externalURL: nil
        )
    }
    ...
    private func readGitDiffInput(source: DiffSource, context: DiffSourceContext) throws -> DiffInput {
        ...
        case .lastTurn:
            guard let workspaceId = normalizedDiffSourceValue(context.workspaceId),
                ...
                throw CLIError(message: "zerocmux diff --last-turn requires a workspace and surface context. Run it from a zerocmux terminal or pass --workspace and --surface.")
            }
    ...
    private func gitRepoRoot(startingAt directory: String) throws -> String {
        do {
            ...
            throw CLIError(message: "zerocmux diff git sources require a git repository")
        }
    }
    ...
    private func resolvedDiffBranchBase(_ rawBaseRef: String?, in repoRoot: String) throws -> DiffBranchBase {
        ...
        // 1. Recorded creation base written by new-zerocmux-worktree.sh. Read-only.
        if let branchName,
    ...
    /// pattern fixed in https://github.com/kernelalex/zerocmux/pull/5347, and these
    ...
    private func safeTemporaryGitPathURL(relativePath: String) -> (root: URL, file: URL)? {
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-diff-untracked-\(UUID().uuidString)", isDirectory: true)
    ...
    private func agentTurnDiffBaselineCommit(in repoRoot: String) throws -> String {
        let stashResult = CLIProcessRunner.runProcess(
            executablePath: "/usr/bin/env",
            arguments: ["git", "-C", repoRoot, "stash", "create", "zerocmux last turn baseline"],
            timeout: 60
        )
    ...
    private func diffViewerAppearance(socketPath: String, fontSizeOverride: Double?) -> DiffViewerAppearance {
        ...
        appearance.lightTheme.generatedName = "zerocmux-ghostty-light-\(themeSuffix)"
        appearance.darkTheme.generatedName = "zerocmux-ghostty-dark-\(themeSuffix)"
    ...
    private func defaultDiffViewerAppearance() -> DiffViewerAppearance {
        var lightTheme = DiffViewerTheme(
            generatedName: "zerocmux-ghostty-light",
        ...
        var darkTheme = DiffViewerTheme(
            generatedName: "zerocmux-ghostty-dark",
    ...
    /// custom scheme (`zerocmux-diff-viewer://<token>`) it yields a scheme URL whose
    ...
    /// `zerocmux __diff-viewer-refs --repo <root> [--base <ref>]` -> grouped refs JSON
    ...
    /// `zerocmux __diff-viewer-branch --group <g> --repo <root> --base <ref>` ->
    ...
    private func diffViewerDirectory() throws -> URL {
        let directory = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent("zerocmux-diff-viewer-\(getuid())", isDirectory: true)
    ...
    private func startDiffViewerHTTPServer(rootDirectory: URL, runtime: URL? = nil) throws -> URL {
        guard let executableURL = diffViewerExecutableURL(for: runtime) else {
            throw CLIError(message: "Failed to resolve zerocmux executable for diff viewer server")
        }
    ...
    private func writeDiffViewerHTML(
        ...
        <body>
          <script id="zerocmux-diff-viewer-config" type="application/json">\(configLiteral)</script>
          <div id="root"></div>
    ...
    private func diffViewerPrepaintStyle(appearance: DiffViewerAppearance) -> String {
        ...
        return """
        <style id="zerocmux-diff-viewer-prepaint">
          :root {
    ...
    private func ensureDiffViewerAssets(nextTo viewerURL: URL, runtime: URL? = nil) throws -> DiffViewerAssets {
        ...
        guard appAssetPaths.contains("main.mjs") else {
            throw CLIError(message: "Bundled zerocmux diff viewer app entry asset not found")
        }
    ...
    private func diffViewerBundledAppAssetDirectory(
        ...
        let candidates: [(sourceName: String, targetName: String)] = [
            ("webviews-app", "zerocmux-webviews-app"),
            ("diff-viewer-app", "zerocmux-diff-viewer-app")
        ]
        for candidate in candidates {
            ...
            if FileManager.default.fileExists(atPath: appDirectory.path, isDirectory: &isDirectory),
               ...
               FileManager.default.fileExists(atPath: entry.path) {
                // The shared /tmp asset cache is written by every running zerocmux
        ...
        throw CLIError(message: "Bundled zerocmux diff viewer app assets not found")
    }
    ...
    func diffSubcommandUsage() -> String {
        """
        Usage: zerocmux diff [patch-file|-] [options]

        Render a unified diff or patch in a zerocmux browser split.
        With no patch file or source, zerocmux diff reads piped stdin.
        ...
        Examples:
          zerocmux diff changes.patch
          git diff | zerocmux diff
          zerocmux diff --unstaged
          zerocmux diff --staged
          zerocmux diff --branch
          zerocmux diff --branch --base upstream/main --repo ../repo
          zerocmux diff --last-turn
          zerocmux diff pr.patch --layout unified --font-size 15 --focus true
        """
    }
