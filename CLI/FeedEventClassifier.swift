...
/// `isActionable` flag.
///
/// This is the single source of truth behind both the running `zerocmux` CLI
/// (`zerocmux hooks feed …`) and the `FeedEventClassificationTests` regression
/// suite — the file is compiled into the `zerocmux-cli` target and the
...
struct FeedEventClassification: Equatable {
    ...
    /// telemetry — zerocmux must not double-prompt — but the hook bridge must
    ...
    /// request: notifications carry no request identity anywhere in zerocmux,
...
struct FeedEventClassifier {
    ...
    private static let feedEventSemanticRegistry: [String: [String: FeedEventSemantic]] = [
        ...
        "codex": [
            ...
            // still use Codex's auto-review path instead of blocking on zerocmux
