...
extension CMUXCLI {
    ...
    ///
    /// zerocmux hands a respawn/start command to the surface as the pane's process
    ...
    ///
    /// Teammate panes are respawned by zerocmux's surface layer, not by `zerocmux
    ...
    /// can appear in a cwd, quoted value, or other non-flag position): the `zerocmux
    ...
    func tmuxStartupScript(commandTokens: [String], cwd: String?) -> String? {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-tmux-command-\(UUID().uuidString.lowercased()).sh")
        var lines = [
            "#!/bin/sh",
