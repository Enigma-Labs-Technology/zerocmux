...
extension CMUXCLI {
    ...
    func validatedThemeName(_ rawValue: String, availableThemes: [String]) throws -> String {
        ...
        throw CLIError(message: "Unknown theme '\(trimmed)'. Run 'zerocmux themes' to list available themes.")
    }
    ...
    private func removingManagedThemeOverride(from contents: String) -> String {
        let pattern = #"(?ms)\n?# (?:zero)?cmux themes start\n.*?\n# (?:zero)?cmux themes end\n?"#
    ...
    private func bundleIdentifierForThemeReloadSocketPath(_ socketPath: String) -> String? {
        ...
        switch name {
        case "zerocmux.sock", "cmux.sock":
            return Self.cmuxThemeOverrideBundleIdentifier
        case "zerocmux-debug.sock", "cmux-debug.sock":
            return "com.kernelalex.zerocmux.debug"
        case "zerocmux-nightly.sock", "cmux-nightly.sock":
            return "com.kernelalex.zerocmux.nightly"
        case "zerocmux-staging.sock", "cmux-staging.sock":
            return "com.kernelalex.zerocmux.staging"
        default:
            break
        }

        if name.range(of: #"^(zero)?cmux-\d+\.sock$"#, options: .regularExpression) != nil {
            return Self.cmuxThemeOverrideBundleIdentifier
        }

        for prefix in ["zerocmux-debug-", "cmux-debug-"] {
            if let slug = themeReloadSocketSlug(name, prefix: prefix, suffix: ".sock") {
                return "com.kernelalex.zerocmux.debug.\(slug)"
            }
        }
        for prefix in ["zerocmux-nightly-", "cmux-nightly-"] {
            if let slug = themeReloadSocketSlug(name, prefix: prefix, suffix: ".sock") {
                return "com.kernelalex.zerocmux.nightly.\(slug)"
            }
        }
        for prefix in ["zerocmux-staging-", "cmux-staging-"] {
            if let slug = themeReloadSocketSlug(name, prefix: prefix, suffix: ".sock") {
                return "com.kernelalex.zerocmux.staging.\(slug)"
            }
        }
        return nil
    }
