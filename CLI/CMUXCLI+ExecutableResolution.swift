...
extension CMUXCLI {
    ...
    func isCmuxClaudeWrapper(at path: String) -> Bool {
        ...
        return prefix.contains("zerocmux claude wrapper - injects hooks and session tracking")
    ...
    /// `zerocmux __tmux-compat` process and cannot see this argv) re-apply the same
    ...
    /// The whole point of `zerocmux claude-teams` is "just start a team." Claude Code's
    ...
    /// *what*?" — so a plain `zerocmux claude-teams "make a demo team with 5 subagents"`
    ...
    /// restore command) so that stays canonical; restore re-invokes `zerocmux
    ...
    var claudeTeamsTeamSpawnGuidance: String {
        """
        You are Claude Code running inside zerocmux, started with `zerocmux claude-teams`. \
    ...
    private func cmuxAppBundleResourceBinComponentIndex(_ path: String) -> Int? {
        ...
        for index in components.indices {
            guard components[index].hasSuffix(".app"),
                  components[index].lowercased().contains("zerocmux"),
