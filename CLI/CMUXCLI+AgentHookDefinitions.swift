...
extension CMUXCLI {
    ...
    static func agentHookShellCommand(
        ...
        return "cmux_cli=\"\(executableExpression)\"; if [ -z \"$cmux_cli\" ] || [ ! -x \"$cmux_cli\" ]; then cmux_cli=\"$(command -v zerocmux 2>/dev/null || true)\"; fi; if [ -n \"$CMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$cmux_cli\" ]; then \(dispatch); else \(noOpSnippet); fi"
    }
    ...
    private static func pinnedAgentHookShellCommand(
        ...
        if let cliPath = pinnedAgentHookCLIPath() {
            ...
            dispatch = "if \(ambientGuard) && \(ambientInvocation); then :; elif [ -x \(quotedCLIPath) ]; then \(primaryInvocation); elif command -v zerocmux >/dev/null 2>&1; then \(fallbackInvocation); else \(noOpSnippet); fi"
        } else {
            dispatch = "if \(ambientGuard) && \(ambientInvocation); then :; elif command -v zerocmux >/dev/null 2>&1; then \(fallbackInvocation); else \(noOpSnippet); fi"
        }
