...
extension CMUXCLI {
    ...
    struct AgentHookDef {
        ...
        /// https://github.com/kernelalex/zerocmux/issues/5000.
        ///
        ...
        /// Kiro applies its hooks only when run as the `zerocmux` agent.
    ...
    static let agentDefs: [AgentHookDef] = [
        AgentHookDef(
            ...
            hookMarker: "zerocmux hooks codex", format: .nested(timeoutMs: 5),
            events: [
        ...
        AgentHookDef(
            ...
            configDir: ".grok/hooks", configFile: "zerocmux-session.json",
            ...
            hookMarker: "zerocmux hooks grok", format: .nested(timeoutMs: 5000),
            events: [
                .init(agentEvent: "SessionStart", cmuxSubcommand: "session-start"),
                .init(agentEvent: "UserPromptSubmit", cmuxSubcommand: "prompt-submit"),
                .init(agentEvent: "Stop", cmuxSubcommand: "stop"),
        ...
        AgentHookDef(
            ...
            hookMarker: "zerocmux hooks kiro", format: .kiroAgentJSON(timeoutMs: 5000),
            events: [
            ...
            postInstallNote: String(
                ...
                defaultValue: "Kiro applies these hooks only when run as the zerocmux agent. Start Kiro with `kiro-cli chat --agent zerocmux`, or make it the default with `kiro-cli settings chat.defaultAgent zerocmux`."
            )
        ),
        AgentHookDef(
            ...
            hookMarker: "zerocmux hooks antigravity", format: .antigravityJSON(timeoutSeconds: 10),
            events: [
                .init(agentEvent: "SessionStart", cmuxSubcommand: "session-start"),
    ...
    static func hookCommandString(for def: AgentHookDef, event: AgentHookDef.HookEvent) -> String {
        let command = "zerocmux hooks \(def.name) \(event.cmuxSubcommand)"
        let inline: String
    ...
    /// zerocmux-owned hooks dir and returns its path. A bare executable path runs
    ...
    static func feedHookCommandString(for def: AgentHookDef, agentEvent: String) -> String {
        ...
        case .kiroAgentJSON:
            inline = exitTwoPropagatingAgentHookShellCommand(
                "zerocmux hooks feed --source \(def.name) --event \(agentEvent)",
                for: def,
    ...
    private static let grokPinnedHookMarker = "zerocmux-grok-hook-v2"
    private static let antigravityPinnedHookMarker = "zerocmux-antigravity-hook-v2"
    ...
    private static func agentHookShellCommand(
        ...
        let routedArguments = command.hasPrefix("zerocmux ") ? String(command.dropFirst("zerocmux ".count)) : command
        let noOpSnippet = shellNoOpSnippet(noOpCommand)
        return "cmux_cli=\"${CMUX_BUNDLED_CLI_PATH:-}\"; if [ -z \"$cmux_cli\" ] || [ ! -x \"$cmux_cli\" ]; then cmux_cli=\"$(command -v zerocmux 2>/dev/null || true)\"; fi; if [ -n \"$CMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && [ -n \"$cmux_cli\" ]; then { if [ -n \"${CMUX_SOCKET_PATH:-}\" ]; then \"$cmux_cli\" --socket \"$CMUX_SOCKET_PATH\" \(routedArguments); else \"$cmux_cli\" \(routedArguments); fi; } || \(noOpSnippet); else \(noOpSnippet); fi"
    }
    ...
    private static func pinnedAgentHookShellCommand(
        ...
        if let cliPath = pinnedAgentHookCLIPath() {
            ...
            dispatch = "if [ -x \(quotedCLIPath) ]; then \(primaryInvocation); elif command -v zerocmux >/dev/null 2>&1; then \(fallbackInvocation); else \(noOpSnippet); fi"
        } else {
            dispatch = "command -v zerocmux >/dev/null 2>&1 && \(fallbackInvocation) || \(noOpSnippet)"
        }
    ...
    private static func pinnedAgentHookSocketPath(
        ...
        return "/tmp/zerocmux-debug-\(slug).sock"
    }
    ...
    private static func pinnedHookShellTraceLogPath(socketPath: String?) -> String {
        ...
        if socketName.hasPrefix("zerocmux-debug-"), socketName.hasSuffix(".sock") {
    ...
    private static func isLegacyCmuxOwnedHookCommand(_ command: String, for def: AgentHookDef) -> Bool {
        ...
        // Other generic agents can have stale `zerocmux hooks ...` files from
    ...
    private static func isLegacyCmuxOwnedHookTokens(_ tokens: [String], for def: AgentHookDef) -> Bool {
        guard !tokens.isEmpty,
              URL(fileURLWithPath: tokens[0]).lastPathComponent == "zerocmux"
        else {
            return false
        }
    ...
    private static func legacyCmuxCommandTokenLists(from command: String, for def: AgentHookDef) -> [[String]] {
        ...
        let guardedPrefixes = [
            "[ -n \"$CMUX_SURFACE_ID\" ] && [ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v zerocmux >/dev/null 2>&1 && ",
            "[ \"$\(def.disableEnvVar)\" != \"1\" ] && command -v zerocmux >/dev/null 2>&1 && ",
        ]
        let fallbackSuffix = " || echo '{}'"
        var body = command
        if def.name == "grok" {
            let grokPrefix = "printenv \(def.disableEnvVar) | grep -qx 1 && echo '{}' || { command -v zerocmux >/dev/null 2>&1 && "
            let grokSuffix = " || echo '{}'; }"
    ...
    private static func bundledCLICmuxCommandTokenLists(from command: String) -> [[String]]? {
        guard command.contains("CMUX_BUNDLED_CLI_PATH"),
              ...
              command.contains("command -v zerocmux") else {
            return nil
        }
        ...
        for prefix in invocationPrefixes {
            ...
            while let prefixRange = command.range(of: prefix, range: searchStart..<command.endIndex) {
                ...
                if !args.isEmpty,
                   ...
                   !args.contains("`") {
                    tokenLists.append(
                        ["zerocmux"] + args.split(whereSeparator: { $0 == " " || $0 == "\t" }).map(String.init)
                    )
                }
