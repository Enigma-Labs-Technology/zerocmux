...
extension CMUXCLI {
    func sshAutoReconnectNoteFormat(discardsInput: Bool = false) -> String {
        ...
        let status = String(localized: "cli.ssh.autoReconnect.status", defaultValue: "[zerocmux] ssh exited with status %s; reconnecting (attempt %s/%s).", bundle: bundle)
        let stopHint = String(localized: "cli.ssh.autoReconnect.stopHint", defaultValue: "[zerocmux] close this pane or press Ctrl-C to stop reconnecting.", bundle: bundle)
        let inputNotice = String(
            localized: "cli.ssh.autoReconnect.inputDiscard",
            defaultValue: "[zerocmux] input typed while disconnected is discarded.",
            bundle: bundle
        )
    ...
    func sshAutoReconnectRecoveredNoteFormat() -> String {
        ...
        let status = String(
            ...
            defaultValue: "[zerocmux] SSH reconnected (attempt %s/%s).",
            bundle: bundle
        )
    ...
    func sshManualReconnectExitPromptFormat() -> String {
        let bundle = CLIExecutableLocator.enclosingAppBundle() ?? .main
        let status = String(localized: "cli.ssh.manualReconnectPrompt.status", defaultValue: "[zerocmux] ssh exited with status %s.", bundle: bundle)
        let detail = String(localized: "cli.ssh.manualReconnectPrompt.detail", defaultValue: "[zerocmux] the SSH connection ended; the remote session may still be running.", bundle: bundle)
        let prompt = String(localized: "cli.ssh.manualReconnectPrompt.prompt", defaultValue: "[zerocmux] press Enter to close this pane. Press r then Enter to reconnect.", bundle: bundle)
    ...
    func sshRemoteReconnectShellFunction() -> String {
        [
            ...
            "  if [ -z \"$cmux_reconnect_cli\" ] || [ ! -x \"$cmux_reconnect_cli\" ]; then cmux_reconnect_cli=\"$(command -v zerocmux 2>/dev/null || true)\"; fi",
