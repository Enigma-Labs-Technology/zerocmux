...
// the bundled CLI target (which drives it from agent hooks) and the zerocmux-unit
...
/// debounce shape proven by community prior art (kernelalex/zerocmux#2043)
...
/// that would recurse into zerocmux hooks or the parent agent session while
...
struct AutoNamingEnvironmentPolicy: Sendable {
    /// Exact variables marking a live agent session or zerocmux terminal; never pass them to the summarizer.
