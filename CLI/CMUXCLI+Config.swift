...
extension CMUXCLI {
    func runConfigCommand(
        ...
        case "get":
            guard args.count == 2, let key = canonicalFontSizeKey(args[1]) else {
                throw CLIError(message: "Usage: zerocmux config get <sidebar-font-size|surface-tab-bar-font-size>")
            }
            try runConfigGetFontSize(forKey: key, jsonOutput: wantsJSON)
        case "set":
            guard args.count == 3, let key = canonicalFontSizeKey(args[1]) else {
                throw CLIError(message: "Usage: zerocmux config set <sidebar-font-size|surface-tab-bar-font-size> <points>")
            }
        ...
        case CmuxGhosttyConfigSettingEditor.sidebarFontSizeKey, CmuxGhosttyConfigSettingEditor.surfaceTabBarFontSizeKey:
            if args.count == 1 {
                ...
                throw CLIError(message: "Usage: zerocmux config \(subcommand) [points]")
            }
        case "path", "paths":
    ...
    func configUsage() -> String {
        return """
        Usage: zerocmux config <doctor|check|validate|path|paths|docs|documentation|reload|get|set|sidebar-font-size|surface-tab-bar-font-size>
        ...
        Subcommands:
          ...
          reload                                  Reload Ghostty config + cmux.json and refresh terminals (alias for `zerocmux reload-config`).
          get <key>                               Print sidebar-font-size or surface-tab-bar-font-size.
          set <key> <points>                      Set sidebar-font-size (10-20 pt) or surface-tab-bar-font-size (8-24 pt), then reload if zerocmux is running.
        ...
        Examples:
          ...
          zerocmux config set sidebar-font-size 14
          zerocmux config sidebar-font-size 12.5
          zerocmux config set surface-tab-bar-font-size 13
          zerocmux config surface-tab-bar-font-size 11
    ...
    private func runConfigSetFontSize(
        ...
        case "failed":
            ...
            print("Run `zerocmux config reload` after zerocmux is running to apply it.")
        default:
            print("OK \(key) = \(formattedValue) (saved)")
            print("Run `zerocmux config reload` to apply it.")
        }
        print("path: \(Self.tildePath(url.path))")
    }
