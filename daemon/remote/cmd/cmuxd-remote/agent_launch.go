...
// runClaudeTeamsRelay implements `zerocmux claude-teams` on the remote side.
...
func runClaudeTeamsRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "zerocmux claude-teams: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	configureAgentEnvironment(agentConfig{
		...
		focused:        focused,
		tmuxPathPrefix: "zerocmux-claude-teams",
	...
	if claudePath == "" {
		fmt.Fprintf(os.Stderr, "zerocmux claude-teams: claude not found in PATH\n")
		return 1
	}
	argv := append([]string{claudePath}, launchArgs...)
	execErr := syscall.Exec(claudePath, argv, os.Environ())
	fmt.Fprintf(os.Stderr, "zerocmux claude-teams: exec failed: %v\n", execErr)
	return 1
}

// runOMORelay implements `zerocmux omo` on the remote side.
func runOMORelay(socketPath string, args []string, refreshAddr func() string) int {
	rc := &rpcContext{socketPath: socketPath, refreshAddr: refreshAddr}

	shimDir, err := createOMOShimDir()
	if err != nil {
		fmt.Fprintf(os.Stderr, "zerocmux omo: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if opencodePath == "" {
		fmt.Fprintf(os.Stderr, "zerocmux omo: opencode not found in PATH\n"+
	...
	if err := omoEnsurePlugin(originalPath); err != nil {
		fmt.Fprintf(os.Stderr, "zerocmux omo: plugin setup: %v\n", err)
		return 1
	}
	...
	fmt.Fprintf(os.Stderr, "zerocmux omo: exec failed: %v\n", execErr)
	return 1
}

// runOMXRelay implements `zerocmux omx` on the remote side.
func runOMXRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "zerocmux omx: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omxPath == "" {
		fmt.Fprintf(os.Stderr, "zerocmux omx: omx not found in PATH\n"+
	...
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "zerocmux omx: exec failed: %v\n", execErr)
	return 1
}

// runOMCRelay implements `zerocmux omc` on the remote side.
func runOMCRelay(socketPath string, args []string, refreshAddr func() string) int {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "zerocmux omc: failed to create shim directory: %v\n", err)
		return 1
	}
	...
	if omcPath == "" {
		fmt.Fprintf(os.Stderr, "zerocmux omc: omc not found in PATH\n"+
	...
	if restoreModulePath, err := ensureClaudeNodeOptionsRestoreModule(); err == nil {
		...
		fmt.Fprintf(os.Stderr, "zerocmux omc: warning: failed to create NODE_OPTIONS restore module: %v\n", err)
	}

	launchPath, launchArgv := resolveNodeScriptExec(omcPath, args, originalPath, shimDir)
	execErr := syscall.Exec(launchPath, launchArgv, os.Environ())
	fmt.Fprintf(os.Stderr, "zerocmux omc: exec failed: %v\n", execErr)
	return 1
}
...
set -euo pipefail
exec "${CMUX_CLAUDE_TEAMS_CMUX_BIN:-zerocmux}" __tmux-compat "$@"
`
...
exec "${CMUX_OMO_CMUX_BIN:-zerocmux}" __tmux-compat "$@"
`
...
exec "${CMUX_OMX_CMUX_BIN:-zerocmux}" __tmux-compat "$@"
`
...
exec "${CMUX_OMC_CMUX_BIN:-zerocmux}" __tmux-compat "$@"
`
...
exec "${CMUX_OMO_CMUX_BIN:-zerocmux}" notify --title "${TITLE:-OpenCode}" --body "${BODY:-}"
`
...
func ensureClaudeNodeOptionsRestoreModule() (string, error) {
	dir := filepath.Join(os.TempDir(), "zerocmux-claude-node-options")
	if err := os.MkdirAll(dir, 0755); err != nil {
		return "", err
	}
