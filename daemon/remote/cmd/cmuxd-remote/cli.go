...
doneFlags:
	...
	// matching the macOS zerocmux CLI.
	...
	if cmdName == "workspace" {
		...
		fmt.Fprintln(os.Stderr, "zerocmux workspace: only the \"group\" subcommand is supported here. Use list-workspaces, new-workspace, close-workspace, or select-workspace for workspace operations.")
		return 2
	}
...
// runWorkspaceGroupRelay handles "zerocmux workspace group <sub>" (and the
// "workspace-group" alias) by mapping each subcommand to its
// workspace.group.* v2 method, mirroring the macOS zerocmux CLI flags.
func runWorkspaceGroupRelay(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	...
	if len(args) == 0 {
		fmt.Fprintf(os.Stderr, "zerocmux workspace group: requires a subcommand (%s)\n", subcommandHint)
		return 2
	}
	sub := args[0]
	flagKeys, ok := workspaceGroupFlagKeys[sub]
	if !ok {
		fmt.Fprintf(os.Stderr, "zerocmux workspace group: unknown subcommand %q (%s)\n", sub, subcommandHint)
		return 2
	}

	fail := func(format string, a ...any) int {
		fmt.Fprintf(os.Stderr, "zerocmux workspace group %s: %s\n", sub, fmt.Sprintf(format, a...))
		return 2
	}
