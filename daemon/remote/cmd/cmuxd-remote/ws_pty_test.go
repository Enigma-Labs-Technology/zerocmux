...
// for https://github.com/kernelalex/zerocmux/issues/5185, where the failure
...
func TestWebSocketPTYRequiresSessionMatchAndConsumesLeaseOnce(t *testing.T) {
	...
	writeTestLease(t, leasePath, "zerocmux-secret", "sess-good", true, time.Now().Add(time.Minute))
	conn := dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "zerocmux-secret", "sess-other", 80, 24)
	_, _, err := conn.Read(ctx)
	if websocket.CloseStatus(err) != websocket.StatusPolicyViolation {
	...
	conn = dialPTY(t, ctx, server.URL)
	sendAuth(t, ctx, conn, "zerocmux-secret", "sess-good", 100, 30)
