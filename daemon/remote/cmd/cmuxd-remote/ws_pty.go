...
func newPTYAllocationError(err error) error {
	...
	if isPermissionDeniedErr(err) {
		hint = "; the remote devpts denies /dev/ptmx (e.g. mounted ptmxmode=000): remount it writable with `sudo mount -o remount,ptmxmode=0666 /dev/pts` or expose a writable /dev/ptmx so the zerocmux daemon can open a terminal"
	}
