...
6. `cmuxd-remote cli <command> [args...]` — relay zerocmux commands to the local app over the reverse SSH forward

`serve --ws` is explicit opt-in for cloud VM images only. The normal `zerocmux ssh`
...
5. Persistent SSH terminals require the `pty.session.persistent_daemon` capability before zerocmux will restore a saved remote PTY session ID after relaunch.

## Persistent SSH PTY daemon

`zerocmux ssh` uses one persistent daemon slot per CLI-launched SSH workspace. The
...
3. `zerocmux ssh-session-list` calls `pty.list`; `zerocmux ssh-session-attach` creates a new local terminal whose startup script calls `ssh-pty-attach --require-existing`.
4. `zerocmux ssh-session-cleanup` calls `pty.close` to terminate a persisted PTY session explicitly.
...
1. `zerocmux workspace group <sub>` (and the `zerocmux workspace-group <sub>` alias) maps to the `workspace.group.*` v2 methods, with the same subcommands and flags as the macOS CLI: `list`, `create`, `ungroup`, `delete`, `rename`, `collapse`, `expand`, `pin`, `unpin`, `add`, `remove`, `set-anchor`, `new-workspace`, `set-color`, `set-icon`, `move`, and `focus`.
