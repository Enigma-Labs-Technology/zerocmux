...
# it never touches the user's stable zerocmux instance.
#
# Flow (--surface both):
...
#        defaults write com.kernelalex.zerocmux.debug.<tag-id> mobile.iOSPairingHost.enabled -bool true
...
# exact derivation so it stays in sync with reload.sh / zerocmux-debug-cli.sh.
...
# via scripts/zerocmux-debug-cli.sh. The replay engine spawns the debug CLI, which
