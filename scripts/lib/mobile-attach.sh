...
# reload.sh / zerocmux-debug-cli.sh exactly).
#
...
# class is deliberate (matches reload.sh + zerocmux-debug-cli.sh socket/DerivedData
...
cmux_attach_mac_bundle_id() {
  printf 'com.kernelalex.zerocmux.debug.%s' "$(cmux_attach__bundle_seg "$1")"
}

# The tagged Mac app's debug socket path.
cmux_attach_socket_path() {
  printf '/tmp/zerocmux-debug-%s.sock' "$(cmux_attach__slug "$1")"
}
...
# sanitized slug, matching reload.sh (`APP_NAME="zerocmux DEV ${TAG_SLUG}"`); the raw
# tag would miss for any tag whose slug differs (e.g. "Fix Foo" -> "fix-foo").
cmux_attach_mac_app_path() {
  local slug
  slug="$(cmux_attach__slug "$1")"
  printf '%s/Library/Developer/Xcode/DerivedData/zerocmux-%s/Build/Products/Debug/zerocmux DEV %s.app' \
    "$HOME" "$slug" "$slug"
}
...
cmux_attach_ensure_mac() {
  ...
  if [[ -S "$sock" ]]; then
    ...
    pkill -f "zerocmux DEV ${slug}.app/Contents/MacOS/zerocmux DEV" 2>/dev/null || true
  ...
  # binds /tmp/zerocmux-debug-<slug>.sock without extra env.
...
cmux_attach_mint_url() {
  ...
  sock="$(cmux_attach_socket_path "$tag")"
  # zerocmux-debug-cli.sh rejects CMUX_TAG outside [A-Za-z0-9._-] and re-sanitizes it
  ...
  for _i in $(seq 1 "$max"); do
    ...
    payload="$(CMUX_TAG="$slug" "$repo_root/scripts/zerocmux-debug-cli.sh" rpc mobile.attach_ticket.create \
