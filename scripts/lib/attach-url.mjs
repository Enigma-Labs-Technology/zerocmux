// Pure encoder for the zerocmux iOS attach deep link.
//
...
// `zerocmux-ios-dev`, Release (TestFlight beta + App Store) emit `zerocmux-ios`. Both
...
export const DEV_URL_SCHEME = "zerocmux-ios-dev";

/** The pairing/attach URL scheme Release (beta + prod) builds emit. */
export const RELEASE_URL_SCHEME = "zerocmux-ios";
...
export function buildAttachURL(payload, filter = {}) {
  ...
  if (
    ...
    payload.attach_url.startsWith("zerocmux-ios://attach?") &&
