...
def main() -> int:
    ...
    for label in LABELS:
        ...
        if not pid or command is None:
            ...
            continue
        if label == "mac" and "zerocmux DEV swmob.app/Contents/MacOS/zerocmux DEV" not in command:
            failures.append(f"{label}.pid {pid} does not look like tagged macOS zerocmux")
        if label in {"iphone", "ipad"} and "zerocmux.app/zerocmux" not in command:
            failures.append(f"{label}.pid {pid} does not look like zerocmux")
