...
soak_root = Path(os.environ.get("SOAK_ROOT", f"/tmp/zerocmux-mobile-soak-{tag}"))
...
cmux_log_path = Path(os.environ.get("CMUX_DEBUG_LOG", f"/tmp/zerocmux-debug-{tag}.log"))
...
def cmux_rpc(method, params):
    return json.loads(run(["scripts/zerocmux-debug-cli.sh", "rpc", method, json.dumps(params)]))
...
# channel-specific in CmxPairingURLScheme) registers `zerocmux-ios-dev` in its
# CFBundleURLSchemes rather than the release `zerocmux-ios`. `simctl openurl` routes
...
ATTACH_URL_SCHEME = os.environ.get("MOBILE_ATTACH_URL_SCHEME", "zerocmux-ios-dev").strip()
...
def write_failure_diagnostics(ticket, terminal_id, iteration, error):
    ...
    if cmux_log_path.exists():
        try:
            ...
            (diagnostic_root / "zerocmux-debug-tail.log").write_text("\n".join(lines[-cmux_log_tail_lines:]) + "\n")
