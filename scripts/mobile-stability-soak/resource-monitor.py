...
def find_pid(label: str, processes: list[tuple[int, str]]) -> int | None:
    if label == "mac":
        needle = f"zerocmux-{TAG}/Build/Products/Debug/zerocmux DEV {TAG}.app/Contents/MacOS/zerocmux DEV"
        ...
        candidates: list[int] = []
        for pid, command in processes:
            if f"/Devices/{IPHONE_SIM_ID}/" in command and "zerocmux.app/zerocmux" in command:
                candidates.append(pid)
        return max(candidates) if candidates else None
    elif label == "ipad":
        candidates: list[int] = []
        for pid, command in processes:
            if f"/Devices/{IPAD_SIM_ID}/" in command and "zerocmux.app/zerocmux" in command:
                candidates.append(pid)
        return max(candidates) if candidates else None
    return None
