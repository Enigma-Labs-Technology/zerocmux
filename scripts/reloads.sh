...
APP_NAME="zerocmux STAGING"
BUNDLE_ID="com.kernelalex.zerocmux.staging"
BASE_APP_NAME="zerocmux"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
DERIVED_SET=0
...
write_last_socket_path() {
  ...
  local tmp_marker="/tmp/zerocmux-staging-last-socket-path"
  if [[ -n "${STAGING_SLUG:-}" ]]; then
    marker_name="staging-${STAGING_SLUG}-last-socket-path"
    tmp_marker="/tmp/zerocmux-staging-${STAGING_SLUG}-last-socket-path"
  fi
...
staging_slug_from_bundle_id() {
  local bundle_id="$1"
  local suffix=""
  if [[ "$bundle_id" == "com.kernelalex.zerocmux.staging."* ]]; then
    suffix="${bundle_id#com.kernelalex.zerocmux.staging.}"
  fi
...
Release build with isolated "zerocmux STAGING" identity. Runs side-by-side with
the production zerocmux app.
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="zerocmux STAGING ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.kernelalex.zerocmux.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/zerocmux-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  if [[ -n "$STAGING_SLUG" ]]; then
    ...
    CMUX_SOCKET_PATH_VALUE="/tmp/zerocmux-staging-${STAGING_SLUG}.sock"
  else
    CMUXD_SOCKET="${APP_SUPPORT_DIR}/cmuxd-staging.sock"
    CMUX_SOCKET_PATH_VALUE="/tmp/zerocmux-staging.sock"
  fi
