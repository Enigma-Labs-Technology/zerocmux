...
// paired. Everything routes through `scripts/zerocmux-debug-cli.sh` (sibling
// `../zerocmux-debug-cli.sh`), which targets the TAGGED socket only.
//
// --dry-run prints the resolved `zerocmux` argument vectors for each profile
...
const DEBUG_CLI = path.join(SCRIPT_DIR, "..", "zerocmux-debug-cli.sh");
  ...
  if (opts.dryRun) {
    for (const { name, profile } of profiles) {
      ...
      for (const { argv } of resolveSteps(profile, context)) {
        process.stdout.write(`zerocmux ${argv.join(" ")}\n`);
      }
    }
    return;
  }
