// Replay engine for zerocmux dev environment profiles (P3 of turnkey dev builds).
//
...
// TAGGED dev zerocmux instance. Every step is replayed through
// `scripts/zerocmux-debug-cli.sh`, which refuses without `CMUX_TAG` and targets
// `/tmp/zerocmux-debug-<slug>.sock` (never the user's stable app).
//
...
//   }
//
// - `args` is the exact `zerocmux` argument vector for that step (no leading `zerocmux`).
...
//   `${name}`. Captured values are zerocmux refs/ids, never secrets.
//
...
/**
 * Resolve a profile into the concrete `zerocmux` argument vectors that would run,
...
/**
 * Executes resolved profile steps against a tagged dev zerocmux socket.
 *
 * Each step shells out to `scripts/zerocmux-debug-cli.sh` with `CMUX_TAG` set, so
...
export class ProfileReplayer {
   ...
   * @param {string} opts.cliPath Absolute path to `scripts/zerocmux-debug-cli.sh`.
  ...
  runStep(step) {
    ...
    if (res.status !== 0) {
      ...
      throw new Error(`step failed (exit ${res.status}): zerocmux ${display}\n${detail}`);
    }
    ...
    if (step.capture) {
      ...
      try {
        ...
        throw new Error(
          `step declared captures but stdout was not JSON: zerocmux ${display}`,
        );
      }
      for (const [name, jsonPath] of Object.entries(step.capture)) {
        ...
        if (value === undefined || value === null) {
          throw new Error(
            `capture "${name}" path "${jsonPath}" not found in output of: zerocmux ${display}`,
          );
        }
