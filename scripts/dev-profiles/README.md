...
provisions a realistic test environment in a **tagged** dev zerocmux instance so a
...
`scripts/zerocmux-debug-cli.sh`, which refuses without `CMUX_TAG` and targets
`/tmp/zerocmux-debug-<slug>.sock` only, never the user's stable app. Profiles are
...
# Dry-run: print the resolved `zerocmux` arg vectors without touching a socket:
...
- **`steps`** (required): an ordered list. Each step is one `zerocmux` invocation.
- **`args`** (required): the exact `zerocmux` argument vector for the step, with the
  leading `zerocmux` omitted. These are passed verbatim to `zerocmux-debug-cli.sh`.
...
- **`capture`** (optional): maps a local variable name to a dotted JSON path
  ...
  values are zerocmux refs/ids, never secrets.

Only use `zerocmux` verbs that actually exist (see
`skills/zerocmux-workspace/references/commands.md` and the `zerocmux-groups` skill).
Prefer the canonical noun forms (`zerocmux workspace create`,
`zerocmux workspace group create`) which honor `--json`. Use `--focus false` on
creation steps so replay does not yank the dev window around.

The capture key name depends on the id format, because zerocmux renames id fields
...
- `replay.mjs` — the engine. `resolveSteps(profile, context)` is a pure,
  ...
  out to `zerocmux-debug-cli.sh` with `CMUX_TAG` set, so the tagged-socket safety
