#!/usr/bin/env python3
"""Stress the zerocmux CLI and Unix socket API.
...
SKIPPED_SOCKET_METHODS = {
    ...
    "events.stream": "streaming protocol, covered by zerocmux events --limit",
...
class Diagnostics:
    ...
    def app_pids(self) -> list[int]:
        ...
        if self.tag:
            patterns.append(f"zerocmux DEV {self.tag}.app/Contents/MacOS/zerocmux DEV")
            patterns.append(f"zerocmux DEV {self.tag}")
        patterns.append("zerocmux DEV")
        seen: set[int] = set()
    ...
    def capture(self, reason: str, details: dict[str, Any] | None = None) -> pathlib.Path:
        ...
        if self.free_bytes(capture_dir) > MIN_LIGHT_DIAGNOSTIC_FREE_BYTES:
            self._capture_command(
                capture_dir,
                "log-show-cmux.txt",
                ["log", "show", "--last", "2m", "--style", "compact", "--predicate", 'process CONTAINS "zerocmux"'],
                timeout=20,
            )
        else:
    ...
    def _interesting_log_paths(self) -> list[pathlib.Path]:
        paths = []
        if self.tag:
            paths.append(pathlib.Path(f"/tmp/zerocmux-debug-{self.tag}.log"))
        paths.extend(pathlib.Path(path) for path in glob.glob("/tmp/zerocmux-debug*.log"))
        paths.extend(pathlib.Path(path) for path in glob.glob("/tmp/zerocmux-launch-*.out"))
    ...
    def _copy_recent_crash_reports(self, capture_dir: pathlib.Path) -> None:
        ...
        roots = [
            ...
            pathlib.Path.home() / ".local/state/zerocmux/crash",
        ]
        for root in roots:
            if not root.exists():
                continue
            for path in root.glob("*zerocmux*"):
                try:
...
class StressContext:
    def __init__(
        ...
        *,
        ...
        self.markdown_path.write_text("# zerocmux stress\n\nsocket and cli stress fixture\n", encoding="utf-8")
        self.text_path.write_text("zerocmux stress file\n", encoding="utf-8")
        html = "<!doctype html><title>zerocmux stress</title><main><input id=i value=ready><button id=b>go</button><select id=s><option>a</option><option>b</option></select><p>stress body</p></main>"
    ...
    def base_env(self) -> dict[str, str]:
        ...
        if self.tag:
            ...
            env["CMUX_BUNDLE_ID"] = f"com.kernelalex.zerocmux.debug.{self.tag.replace('-', '.')}"
...
def browser_socket_cases() -> list[SocketCase]:
    ...
    return [
        ...
        SocketCase("browser.find.title", "browser.find.title", lambda c: {**p_browser(c), "text": "zerocmux stress"}, expect_ok=None),
...
class StressRunner:
    ...
    def run(self) -> int:
        ...
        heartbeat = threading.Thread(target=self.heartbeat_loop, name="zerocmux-stress-heartbeat", daemon=True)
        heartbeat.start()
...
def resolve_cli_path(raw: str | None, tag: str | None) -> str:
    ...
    if tag:
        candidates.append(os.path.expanduser(f"~/Library/Developer/Xcode/DerivedData/zerocmux-{tag}/Build/Products/Debug/zerocmux DEV {tag}.app/Contents/Resources/bin/zerocmux"))
        candidates.append(os.path.expanduser(f"~/Library/Developer/Xcode/DerivedData/zerocmux-{tag}/Build/Products/Debug/zerocmux"))
    last_cli = pathlib.Path("/tmp/zerocmux-last-cli-path")
    if last_cli.exists():
        try:
            candidates.append(last_cli.read_text(encoding="utf-8").strip())
        except OSError:
            pass
    candidates.extend(glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/zerocmux"), recursive=True))
    for candidate in candidates:
        if candidate and os.path.isfile(candidate) and os.access(candidate, os.X_OK):
            return candidate
    raise SystemExit("error: could not find zerocmux CLI. Pass --cli or set CMUXTERM_CLI.")
...
def resolve_socket_path(raw: str | None, tag: str | None) -> str:
    ...
    if env_socket:
        return env_socket
    if tag:
        return f"/tmp/zerocmux-debug-{tag}.sock"
    last_socket = pathlib.Path("/tmp/zerocmux-last-socket-path")
...
def main() -> int:
    ...
    parser.add_argument("--cli", help="Path to zerocmux CLI binary. Defaults to CMUXTERM_CLI, tagged DerivedData, then recent DerivedData.")
    parser.add_argument("--socket", help="Unix socket path. Defaults to CMUX_SOCKET_PATH or /tmp/zerocmux-debug-<tag>.sock.")
    ...
    parser.add_argument("--artifacts", help="Artifact directory. Default: /tmp/zerocmux-cli-socket-stress-<timestamp>.")
    ...
    artifacts_dir = pathlib.Path(args.artifacts or f"/tmp/zerocmux-cli-socket-stress-{now_slug()}").expanduser()
