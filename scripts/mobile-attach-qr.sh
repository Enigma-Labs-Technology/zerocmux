...
TMP_ROOT="${TMPDIR:-/tmp}"
OUT_DIR="${OUT_DIR:-${TMP_ROOT%/}/zerocmux-mobile-attach-qr-$TAG}"
umask 077
mkdir -p "$OUT_DIR"
chmod 700 "$OUT_DIR"
...
CMUX_TAG="$TAG" "$REPO_ROOT/scripts/zerocmux-debug-cli.sh" rpc mobile.attach_ticket.create "$PARAMS" > "$RAW_JSON_TMP"
...
<title>zerocmux mobile pairing</title>
<style>
...
<main>
  ...
  <section>
    <h1>Scan to pair zerocmux</h1>
    <p>Open zerocmux on iPhone, tap <strong>Scan QR Code</strong>, and scan this code. This ticket is route-filtered for the address below.</p>
    <table>
