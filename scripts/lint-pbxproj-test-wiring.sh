...
#
# Originally surfaced during the https://github.com/kernelalex/zerocmux/issues/4529
# investigation, where SessionIndexJSONLStreamTests.swift on
# https://github.com/kernelalex/zerocmux/pull/4536 looked like a clean two-commit
...
if [ ! -f "$PBXPROJ" ]; then
  ...
  echo "  (run from the zerocmux repo root or pass --repo-root)" >&2
  exit 2
fi
if [ ! -d "$TESTS_DIR" ]; then
...
#      target).
#   3. File is a member of the wrong target (e.g. cmuxUITests or zerocmux). Its
...
echo "Files wired only into cmuxUITests, zerocmux, or the project tree (without"
