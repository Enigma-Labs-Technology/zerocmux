...
# Defaults to 127.0.0.1:17321 to match the existing tools/zerocmux-tag-opener
...
TAG_MARKER_PATH = "/tmp/zerocmux-mobile-attach-qr-tags.json"
...
def _apply_tags(mac_tag: str, ios_tag: str) -> None:
    ...
    TAG = mac_tag
    IOS_TAG = ios_tag
    OUT_DIR = os.path.join(TMP_ROOT, f"zerocmux-mobile-attach-qr-{TAG}")
...
def tagged_app_path() -> str:
    ...
    return os.path.join(
        ...
        "Products",
        "Debug",
        f"zerocmux DEV {TAG}.app",
    )


def tagged_app_executable() -> str:
    return os.path.join(tagged_app_path(), "Contents", "MacOS", "zerocmux DEV")


def app_info() -> dict:
    """Single source of truth for what 'Open zerocmux DEV <tag>' actually does.
    ...
    try:
        out = subprocess.run(
            ["pgrep", "-f", f"zerocmux DEV {TAG}.app/Contents/MacOS/zerocmux DEV"],
            check=False,
            timeout=2,
...
class Handler(http.server.BaseHTTPRequestHandler):
    def log_message(self, format: str, *args) -> None:
        # Stay quiet, the zerocmux helper pane is the visible signal.
        return
    ...
    def _serve_qr_page(self) -> None:
        ...
        with open(html_path, "rb") as fh:
            html = fh.read()
        # Inject a meta refresh + a small banner + an "Open zerocmux DEV <tag>"
        ...
        banner = (
            b'<button class="qr-open-tag" type="button" id="zerocmux-open-btn" disabled '
            ...
            + b'    var btn=document.getElementById("zerocmux-open-btn");\n'
            ...
            + b'      btn.innerHTML="Open <code>zerocmux DEV "+info.tag+"</code> \\u00b7 built "+built+running;\n'
            + b'      btn.title=info.app_path;\n'
            + b'    }else{\n'
            + b'      btn.disabled=true;\n'
            + b'      btn.innerHTML="<code>zerocmux DEV "+info.tag+"</code> not built";\n'
