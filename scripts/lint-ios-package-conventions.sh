...
SCOPES=()
for d in Packages/Shared/CMUXMobileCore Packages/iOS/CmuxMobile* Packages/Shared/CmuxAgentChat Packages/iOS/CmuxAgentChatUI Packages/Shared/CmuxSyncStore ios/cmuxPackage/Sources ios/zerocmux; do
  [ -d "$d" ] && SCOPES+=("$d")
done
...
# only shrink.
NS_TYPE_ROOTS=()
for d in Packages/*/*/Sources ios/cmuxPackage/Sources ios/zerocmux; do
  [ -d "$d" ] && NS_TYPE_ROOTS+=("$d")
done
