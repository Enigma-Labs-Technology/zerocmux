...
ZIG_WORK_PARENT="${RUNNER_TEMP:-/tmp/zerocmux-zig-ci}"
...
ZIG_WORK_ROOT="$(mktemp -d "${ZIG_WORK_PARENT%/}/zerocmux-zig-install-${ZIG_REQUIRED}.XXXXXX")"
cleanup_work_root() {
  rm -rf "$ZIG_WORK_ROOT"
}
...
install_zig_without_sudo() {
  local install_parent="${RUNNER_TEMP:-/tmp/zerocmux-zig-ci}"
