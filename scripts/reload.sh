...
LEGACY_SOCKET_PATH_DIR="${_cmux_account_home:-$HOME}/Library/Application Support/zerocmux"
...
publish_reload_cli_path() {
  ...
  reload_write_cli_pointer "/tmp/zerocmux-last-cli-path" "$cli_path" || return 1
...
XCODEBUILD_ARGS+=(CMUX_CUA_HELPER_DISPLAY_NAME="zerocmux Computer Use")
if [[ "$PROD_AUTH" -eq 1 ]]; then
...
if [[ "$LAUNCH" -eq 1 ]]; then
  ...
  if [[ -n "${TAG_SLUG:-}" ]]; then
    ...
    CMUX_TAG_LAUNCH_LOG_DIRECTORY="$(mktemp -d "${TMPDIR:-/tmp}/zerocmux-launch-${TAG_SLUG}.XXXXXX")"
