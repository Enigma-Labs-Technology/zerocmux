...
# Matches CmuxStateDirectory (non-TCC ~/.local/state/zerocmux) where the app/CLI now
# read the last-socket-path markers (https://github.com/kernelalex/zerocmux/issues/5146).
...
LAST_SOCKET_PATH_DIR="${_cmux_account_home:-$HOME}/.local/state/zerocmux"
...
CLI_PATH_FILE="/tmp/zerocmux-last-cli-path"
SOCKET_ARG=""
...
if [[ -n "\$SOCKET_ARG" ]]; then
  ...
  if [[ "\$SOCKET_NAME" == zerocmux-debug-*.sock ]]; then
    TAG="\${SOCKET_NAME#zerocmux-debug-}"
    TAG="\${TAG%.sock}"
    if [[ "\$TAG" =~ ^[A-Za-z0-9_-]+$ ]]; then
      TAG_CLI="\$HOME/Library/Developer/Xcode/DerivedData/zerocmux-\$TAG/Build/Products/Debug/zerocmux DEV \$TAG.app/Contents/Resources/bin/zerocmux"
...
publish_reload_cli_path() {
  ...
  if [[ -n "${CMUX_SHIM_TARGET:-}" ]]; then
    write_dev_cli_shim "$CMUX_SHIM_TARGET" "/Applications/zerocmux.app/Contents/Resources/bin/zerocmux"
  fi
}
...
write_last_socket_path() {
  ...
  local tmp_marker="/tmp/zerocmux-dev-last-socket-path"
  local bundle_id="${BUNDLE_ID:-}"
  local slug=""

  case "$bundle_id" in
    com.kernelalex.zerocmux)
      marker_name="last-socket-path"
      tmp_marker="/tmp/zerocmux-last-socket-path"
      ;;
    com.kernelalex.zerocmux.nightly)
      marker_name="nightly-last-socket-path"
      tmp_marker="/tmp/zerocmux-nightly-last-socket-path"
      ;;
    com.kernelalex.zerocmux.nightly.*)
      slug="$(sanitize_path "${bundle_id#com.kernelalex.zerocmux.nightly.}")"
      if [[ -n "$slug" ]]; then
        marker_name="nightly-${slug}-last-socket-path"
        tmp_marker="/tmp/zerocmux-nightly-${slug}-last-socket-path"
      else
        marker_name="nightly-last-socket-path"
        tmp_marker="/tmp/zerocmux-nightly-last-socket-path"
      fi
      ;;
    com.kernelalex.zerocmux.staging)
      marker_name="staging-last-socket-path"
      tmp_marker="/tmp/zerocmux-staging-last-socket-path"
      ;;
    com.kernelalex.zerocmux.staging.*)
      slug="$(sanitize_path "${bundle_id#com.kernelalex.zerocmux.staging.}")"
      if [[ -n "$slug" ]]; then
        marker_name="staging-${slug}-last-socket-path"
        tmp_marker="/tmp/zerocmux-staging-${slug}-last-socket-path"
      else
        marker_name="staging-last-socket-path"
        tmp_marker="/tmp/zerocmux-staging-last-socket-path"
      fi
    ...
    com.kernelalex.zerocmux.debug)
      ...
      if [[ -n "$slug" ]]; then
        ...
        tmp_marker="/tmp/zerocmux-dev-${slug}-last-socket-path"
      fi
      ;;
    com.kernelalex.zerocmux.debug.*)
      slug="$(sanitize_path "${bundle_id#com.kernelalex.zerocmux.debug.}")"
      if [[ -n "$slug" ]]; then
        marker_name="dev-${slug}-last-socket-path"
        tmp_marker="/tmp/zerocmux-dev-${slug}-last-socket-path"
      fi
...
Options:
  ...
  --no-global-cli-links  Do not update /tmp/zerocmux-cli, /tmp/zerocmux-last-cli-path,
                         or PATH zerocmux-dev shims. Useful for isolated dogfood.
...
print_tag_cleanup_reminder() {
  ...
  while IFS= read -r -d '' path; do
    ...
    find /tmp -maxdepth 1 -name 'zerocmux-*' -print0 2>/dev/null
    find "$HOME/Library/Developer/Xcode/DerivedData" -maxdepth 1 -type d -name 'zerocmux-*' -print0 2>/dev/null
  )
...
reload_finalize() {
  ...
  if [[ -x "${CLI_PATH:-}" ]]; then
    ...
    echo "CLI helpers:"
    if [[ "$NO_GLOBAL_CLI_LINKS" == "1" ]]; then
      echo "  preserved existing global zerocmux CLI links (--no-global-cli-links)"
    else
      ...
      echo "If your shell still resolves the old zerocmux, run: rehash"
    fi
  fi
...
XCODEBUILD_LOCK_DIR="${TMPDIR:-/tmp}/zerocmux-xcodebuild-$(id -u).locks"
...
if [[ -n "$TAG" && "$APP_NAME" != "$SEARCH_APP_NAME" ]]; then
  ...
  if [[ -f "$INFO_PLIST" ]]; then
    ...
    if [[ -n "${TAG_SLUG:-}" ]]; then
      ...
      set_plist_env "$INFO_PLIST" CMUX_BUNDLED_CLI_PATH "$TAG_APP_FINAL_PATH/Contents/Resources/bin/zerocmux"
...
if [[ "$LAUNCH" -eq 1 ]]; then
  ...
  # Avoid inheriting zerocmux/ghostty environment variables from the terminal that
  ...
  OPEN_CLEAN_ENV=(
    ...
    # Don't leak that into zerocmux, otherwise `git diff` won't page even with PAGER=less.
    -u GIT_PAGER
    -u GH_PAGER
    -u TERMINFO
    -u XDG_DATA_DIRS
  ...
  LAUNCH_AUTH_CALLBACK_SCHEME="zerocmux-dev"
  if [[ -n "${TAG_SLUG:-}" ]]; then
    LAUNCH_AUTH_CALLBACK_SCHEME="zerocmux-dev-${TAG_SLUG}"
  fi
  TAG_LAUNCH_ENV=(
    CMUX_TAG="${TAG_SLUG:-}"
  ...
  if [[ -n "${TAG_SLUG:-}" ]]; then
    ...
    TAG_LAUNCH_LOG="/tmp/zerocmux-launch-${TAG_SLUG}.out"
