#!/usr/bin/env python3
"""Verify zerocmux-owned SwiftPM lockfiles are not ignored."""
...
def main() -> int:
    ...
    for gitignore in sorted(Path(".").rglob(".gitignore")):
        ...
        if is_allowed_vendor_path(rel):
            continue
        errors.append(
            f"{rel} ignores Package.resolved. zerocmux-owned SwiftPM lockfiles must be tracked."
        )
    ...
    for lockfile in tracked_lockfiles:
        ...
        if is_expected_lockfile_path(lockfile, roots):
            continue
        errors.append(f"Unexpected zerocmux Package.resolved location: {lockfile}")
