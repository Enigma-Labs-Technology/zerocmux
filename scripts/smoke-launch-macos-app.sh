...
OPEN_LOG="$(mktemp -t zerocmux-smoke-open.XXXXXX)"
APP_PID=""
...
if [[ "$DIRECT_EXEC" == "1" ]]; then
  CMUX_UI_TEST_MODE="${CMUX_UI_TEST_MODE:-1}" \
    "$EXECUTABLE_PATH" -ApplePersistenceIgnoreState YES --zerocmux-disable-bundle-icon-persistence >"$OPEN_LOG" 2>&1 &
else
  CMUX_UI_TEST_MODE="${CMUX_UI_TEST_MODE:-1}" \
    /usr/bin/open -n -g "$APP_PATH" --args -ApplePersistenceIgnoreState YES --zerocmux-disable-bundle-icon-persistence >"$OPEN_LOG" 2>&1 &
fi
OPEN_PID=$!
...
for _ in $(seq 1 "$STABLE_SECONDS"); do
  ...
  if ! kill -0 "$APP_PID" 2>/dev/null; then
    ...
    STARTUP_LOG="$HOME/Library/Logs/zerocmux/startup-${LOG_NAME}.log"
