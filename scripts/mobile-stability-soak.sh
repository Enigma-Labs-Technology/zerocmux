...
Environment:
  ...
  SOAK_ROOT       Output directory. Defaults to /tmp/zerocmux-mobile-soak-<tag>-<profile>.
...
dev_stack_auth_token="${CMUX_MOBILE_DEV_STACK_AUTH_TOKEN:-zerocmux-dev-mobile-stack-token}"
...
root="${SOAK_ROOT:-/tmp/zerocmux-mobile-soak-${tag}-${profile}}"
mkdir -p "$root"

session_prefix="zerocmux-${tag}-${profile}"
...
cleanup_tagged_soak_processes() {
  ...
  for sim_id in "$iphone_sim_id" "$ipad_sim_id"; do
    ...
    while IFS= read -r pid; do
      [[ "$pid" =~ ^[0-9]+$ ]] || continue
      kill "$pid" >/dev/null 2>&1 || true
    done < <(ps -axo pid=,command= | awk -v sim="$sim_id" 'index($0, "zerocmux.app/zerocmux") && index($0, "/Devices/" sim "/") { print $1 }')
...
app="$HOME/Library/Developer/Xcode/DerivedData/zerocmux-${tag}/Build/Products/Debug/zerocmux DEV ${tag}.app"
if [[ ! -x "$app/Contents/MacOS/zerocmux DEV" ]]; then
  cat >&2 <<EOF
...
for pid in $(pgrep -f "zerocmux-${tag}/Build/Products/Debug/zerocmux DEV ${tag}.app/Contents/MacOS/zerocmux DEV" || true); do
  kill "$pid" >/dev/null 2>&1 || true
done

rm -f "/tmp/zerocmux-debug-${tag}.sock" "/tmp/zerocmux-debug-${tag}.log"
rm -f \
...
for _ in $(seq 1 60); do
  if [[ -S "/tmp/zerocmux-debug-${tag}.sock" ]]; then
    break
  fi
  sleep 1
done
...
if [[ ! -S "/tmp/zerocmux-debug-${tag}.sock" ]]; then
  echo "tagged socket did not appear: /tmp/zerocmux-debug-${tag}.sock" >&2
  exit 1
fi

(
  cd "$repo_root"
  CMUX_TAG="$tag" scripts/zerocmux-debug-cli.sh ping >/dev/null
  if [[ -n "$dev_stack_auth_token" ]]; then
    CMUX_TAG="$tag" scripts/zerocmux-debug-cli.sh rpc mobile.dev_stack_auth.configure \
...
screen -dmS "${session_prefix}-iphone" bash -lc "
  ...
  export MOBILE_FAILURE_LIMIT=1 SOAK_DIAGNOSTICS_DIR='$root/diagnostics' CMUX_DEBUG_LOG='/tmp/zerocmux-debug-${tag}.log'
