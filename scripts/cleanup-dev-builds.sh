...
#   ~/Library/Developer/Xcode/DerivedData/zerocmux-<tag>/      (multi-GB)
#   /tmp/zerocmux-<tag>/                                       (build scratch)
#   /tmp/zerocmux-debug-<tag>.sock                             (control socket)
#   /tmp/zerocmux-debug-<tag>.log                              (debug log)
#   /tmp/zerocmux-reload-<tag>.log                             (build log)
#   ~/Library/Application Support/zerocmux/cmuxd-dev-<tag>.sock (cmuxd socket)
#
# This script removes those artifacts for tags that are safe to clean.
# Safety rules (always on):
#   - Skip any tag whose `zerocmux DEV <tag>` app is currently running.
#   - Skip the tag pointed at by /tmp/zerocmux-last-cli-path (most recent reload).
...
LAST_CLI_PATH_FILE="/tmp/zerocmux-last-cli-path"
...
# Tags come from DerivedData dirs named zerocmux-<tag>. Authoritative because
# reload.sh always creates one there.
discover_tags() {
    [[ -d "$DERIVED_DATA_ROOT" ]] || return 0
    local d name
    for d in "$DERIVED_DATA_ROOT"/zerocmux-*/; do
...
artifact_paths_for_tag() {
    local tag="$1"
    printf '%s\n' \
        "$DERIVED_DATA_ROOT/zerocmux-${tag}" \
        "/tmp/zerocmux-${tag}" \
        "/tmp/zerocmux-${tag}.tar" \
        "/tmp/zerocmux-debug-${tag}.sock" \
        "/tmp/zerocmux-debug-${tag}.log" \
        "/tmp/zerocmux-reload-${tag}.log" \
...
# `/zerocmux-<tag>/` anywhere in the path so we cover paths under DerivedData,
# /tmp, or other locations reload.sh may emit.
active_tag=""
if [[ -r "$LAST_CLI_PATH_FILE" ]]; then
    last_path="$(cat "$LAST_CLI_PATH_FILE" 2>/dev/null || true)"
    if [[ "$last_path" =~ /zerocmux-([A-Za-z0-9._-]+)/ ]]; then
        active_tag="${BASH_REMATCH[1]}"
    fi
fi

# Running zerocmux DEV processes by tag (the app name embeds the tag).
running_tags=()
while IFS= read -r line; do
    # Match "zerocmux DEV <tag>" (with or without .app suffix).
    if [[ "$line" =~ zerocmux\ DEV\ ([A-Za-z0-9._-]+) ]]; then
        running_tags+=("${BASH_REMATCH[1]}")
    fi
done < <(pgrep -fl "zerocmux DEV " 2>/dev/null || true)
...
while IFS= read -r tag; do
    ...
    if (( older_than_days > 0 )); then
        age="$(derived_data_mtime_days "$DERIVED_DATA_ROOT/zerocmux-${tag}")"
