...
agent lives in another zerocmux instance), so the phone never sees it.

Data sources (read live over the tagged debug socket, no app rebuild):
  zerocmux rpc chat.sessions.dump   -> the registry
  zerocmux debug-terminals          -> current surfaces and their workspaces

Usage:  CMUX_TAG=<tag> scripts/zerocmux-chat-debug.py [--live] [--all]
"""
import json
import os
import re
...
CLI = os.path.join(os.path.dirname(__file__), "zerocmux-debug-cli.sh")


def cli(*args):
    if not os.environ.get("CMUX_TAG"):
        sys.exit("CMUX_TAG must be set, e.g. CMUX_TAG=asot scripts/zerocmux-chat-debug.py")
    out = subprocess.run([CLI, *args], capture_output=True, text=True, env=dict(os.environ))
    if out.returncode != 0:
        sys.exit("zerocmux-debug-cli.sh %s failed (rc=%d):\n%s"
