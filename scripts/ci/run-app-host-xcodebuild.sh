...
if [ -z "${CMUX_APP_HOST_TEST_LOCK_ACTIVE:-}" ]; then
  lock_file="${CMUX_APP_HOST_TEST_LOCK_FILE:-${TMPDIR:-/tmp}/zerocmux-app-host-test.lock}"
...
kill_stale_app_host() {
  # Kill app-host executables (matched by their .../Build/Products/.../zerocmux DEV
  ...
  [ -n "$ci_app_host_root" ] && \
    pkill -f "${ci_app_host_root%/}/.*Build/Products/.*zerocmux DEV" 2>/dev/null || true
}
...
while [ "$attempt" -le "$max_attempts" ]; do
  ...
  : >"$log_path"
  # Self-hosted macOS runners reuse the GUI session. A stale "zerocmux DEV" app-host
  ...
  if grep -Fq 'path = "/tmp/zerocmux-debug.sock"' "$log_path"; then
    echo "FAIL: app-host used default debug socket instead of an XCTest-scoped socket" >&2
    exit 1
  fi

  if grep -Fq 'SocketControlServer: Listening on /tmp/zerocmux-debug.sock' "$log_path"; then
