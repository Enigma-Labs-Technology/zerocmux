...
#
# A tagged zerocmux dev app (built by reload.sh --tag <t>) declares a host-scoped
...
Usage: scripts/reload-extension.sh --tag <tag> [--host-bundle-id <id>] [--example sample|tabs|both] [--no-launch]
  ...
  --host-bundle-id   Host app bundle id when reload.sh used --bundle-id.
                     Defaults to com.kernelalex.zerocmux.debug.<sanitized-tag>.
...
HOST_BUNDLE_ID="${HOST_BUNDLE_ID_OVERRIDE:-com.kernelalex.zerocmux.debug.${TAG_ID}}"
...
build_install_example() {
  ...
  local derived="/tmp/zerocmux-ext-${which}-${TAG_ID}"
