...
helper_bundle_id_for_host() {
  ...
  echo "com.kernelalex.zerocmux.cua"
}
...
cleanup() {
  ...
  [[ -n "$TMPDIR_POLICY" ]] && rm -rf "$TMPDIR_POLICY"
...
# The bundled engine must contain neither analytics nor a separate update client.
# Materialize the pinned Git tree, ignoring modifications even in CMUX_CUA_SRC,
# and apply the reviewed patch before any compilation. A changed upstream pin
# that no longer accepts the patch fails the build instead of restoring telemetry.
POLICY_PATCH="$REPO_ROOT/patches/cmux-cua-zero-telemetry.patch"
[[ -r "$POLICY_PATCH" ]] || { echo "error: missing computer-use privacy patch" >&2; exit 1; }
POLICY_REVISION="$(cat "$REPO_ROOT/patches/cmux-cua-zero-telemetry.revision")"
[[ "$POLICY_REVISION" == "$CMUX_CUA_PINNED_SHA" ]] || {
  echo "error: computer-use source revision requires a new privacy audit" >&2
  exit 1
}
POLICY_HASH="$(shasum -a 256 "$POLICY_PATCH" | awk '{print $1}')"
POLICY_KEY="$CMUX_CUA_PINNED_SHA-$POLICY_HASH"
POLICY_SOURCE="$CACHE_DIR/zerocmux-src-$POLICY_KEY"
POLICY_OWNER_FILE=".zerocmux-cua-privacy-source"
POLICY_OWNER_VALUE="zerocmux-cua-privacy-v1 $POLICY_KEY"
acquire_src_lock "$POLICY_SOURCE.lock"
if [[ -L "$POLICY_SOURCE" ]]; then
  echo "error: refusing symlinked computer-use privacy source" >&2
  exit 1
fi
if [[ -e "$POLICY_SOURCE" ]]; then
  [[ -d "$POLICY_SOURCE" && "$(cat "$POLICY_SOURCE/$POLICY_OWNER_FILE" 2>/dev/null || true)" == "$POLICY_OWNER_VALUE" ]] || {
    echo "error: refusing unmanaged computer-use privacy source: $POLICY_SOURCE" >&2
    exit 1
  }
fi
TMPDIR_POLICY="$(mktemp -d "$CACHE_DIR/.zerocmux-cua-policy.XXXXXX")"
git -C "$SRC_ROOT" archive "$CMUX_CUA_PINNED_SHA" libs/cmux-cua | tar -x -C "$TMPDIR_POLICY"
git -C "$TMPDIR_POLICY" apply --no-index --check "$POLICY_PATCH"
git -C "$TMPDIR_POLICY" apply --no-index "$POLICY_PATCH"
printf '%s\n' "$POLICY_OWNER_VALUE" > "$TMPDIR_POLICY/$POLICY_OWNER_FILE"
mkdir -p "$POLICY_SOURCE"
# Preserve timestamps of identical source files for incremental Cargo builds.
# The lock remains held through compilation, so another build cannot rewrite
# these sources while Cargo is reading them. Build outputs live in a sibling.
rsync -a --checksum --delete "$TMPDIR_POLICY/" "$POLICY_SOURCE/"
rm -rf "$TMPDIR_POLICY"
TMPDIR_POLICY=""

CARGO_ROOT="$POLICY_SOURCE/libs/cmux-cua/rust"
...
for arch in "${ARCHS[@]}"; do
  ...
  # Source revision and privacy patch jointly identify the build cache.
  # Keep outputs outside the source tree that is synchronized above.
  target_dir="$CACHE_DIR/zerocmux-target-$POLICY_KEY"
  ...
  python3 "$REPO_ROOT/scripts/verify-cmux-cua-privacy.py" "$arch_output"
  BUILT+=("$arch_output")
done
...
if [ -n "${_cua_contents:-}" ] && [ "$(basename "$_cua_contents")" = "Contents" ]; then
  ...
  HELPER_DISPLAY="${CMUX_CUA_HELPER_DISPLAY_NAME:-zerocmux Computer Use}"
