...
struct SessionEntry: Identifiable, Hashable {
    ...
    private var resumeCommandWithoutWorkingDirectory: String? {
        ...
        case let .claude(model, permissionMode, configDirectoryForResume):
            ...
            // re-injects zerocmux hooks even when the command runs in a shell where the
            ...
            // https://github.com/kernelalex/zerocmux/issues/5639
        ...
        case let .codex(model, approval, sandbox, effort):
            ...
            // auto-resumed codex session re-injects zerocmux hooks even when the
            ...
            // bypassing zerocmux-codex-wrapper, so no SessionStart fires and the iOS
            ...
            // `resumeCommandWithCwd`. https://github.com/kernelalex/zerocmux/issues/5639
    ...
    ///
    /// zerocmux captures Codex's *internal* sandbox-policy `type`, which is a
    ...
    /// (see https://github.com/kernelalex/zerocmux/issues/5262).
    ...
    /// the per-session policy zerocmux captured, always yielding a valid invocation.
    ///
