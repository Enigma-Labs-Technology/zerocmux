...
/// Serial lane for in-process `zerocmux(...)` sidebar actions. Worker-lane methods
...
// zerocmux-coupled action dispatch, the one piece that must reach
...
/// live zerocmux command dispatcher.
///
/// `zerocmux(...)` commands run in-process through
    ...
    SidebarActionDispatch { action in
        ...
        cmuxSidebarWorkerQueue.async {
            for command in commands {
                switch command {
                case let .zerocmux(method, params):
