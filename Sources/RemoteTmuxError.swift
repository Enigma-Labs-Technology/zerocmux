...
enum RemoteTmuxError: Error, Sendable, Equatable {
    /// The `ssh` (or remote) command exited non-zero for a reason zerocmux does
    ...
    var message: String {
        ...
        case let .unsupportedTmux(detected):
            let format = String(
                ...
                defaultValue: "remote session server is too old (found %@; zerocmux needs version %@ or newer)"
            )
            return String(
                format: format,
