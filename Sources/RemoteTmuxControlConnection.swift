...
/// live output callback. zerocmux owns the whole protocol here — so it never depends
...
final class RemoteTmuxControlConnection {
    ...
    /// zerocmux grid. Without this a freshly attached session stays at ssh's default
    ...
    /// usually does NOT: the window still has the size zerocmux itself left behind, so
    ...
    /// (it set before zerocmux attached): scroll region, DEC private modes, the mouse
    ...
    /// zerocmux copy (exactly as a local terminal behaves with a mouse-mode app).
    func capturePane(paneId: Int) {
        ...
        // pane was already on the alt screen before zerocmux attached, so its 1049h is
        ...
        -> (setBuffer: String, pasteBuffer: String)?
    {
        ...
        let buffer = "zerocmux-paste-\(paneId)"
        return (
    ...
    private func handle(_ message: RemoteTmuxControlMessage) {
        ...
        case let .subscriptionChanged(name, value):
            // zerocmux subscribes each pane's working directory as "cmux_cwd_<paneId>".
