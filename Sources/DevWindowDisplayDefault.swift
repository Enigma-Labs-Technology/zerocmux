...
/// Shared, cross-tag default for which display new zerocmux DEV windows open on.
///
...
enum DevWindowDisplayDefault {
    ...
    /// parent), matching `zerocmux window default-display --clear`.
    ...
    /// never steals focus. DEBUG-only: production zerocmux is never auto-moved.
    @MainActor
