...
/// `v2SettingsOpen`, and `v2FeedbackOpen`, minus the per-read `v2MainSync`
/// hops (the coordinator already runs on the main actor inside the
/// socket-command policy scope). `system.identify` and `surface.split_off`
/// stay shared app-side bodies (`v2Identify` feeds `system.top` /
/// `system.memory` and the task-manager snapshot; `v2SurfaceSplitOff` is also
/// driven by the v1 `drag_surface_to_split`), so their witnesses bridge.
