...
enum CmuxExtensionWorktreePrototype {
    static func createWorktree(projectRootPath: String) async throws -> CmuxExtensionWorktreeCreationResult {
        try await Task.detached(priority: .userInitiated) {
            ...
            let branchName = "zerocmux-sidebar-\(Int(Date().timeIntervalSince1970))-\(UUID().uuidString.prefix(8).lowercased())"
            ...
            let samplePath = shellEscaped(worktree.appendingPathComponent("zerocmux-sample-dev", isDirectory: true).path)
    ...
    private static func ensureCmuxWorktreeDirectoryIsLocallyIgnored(projectRoot: URL) async throws {
        ...
        let next = existing + separator + "# zerocmux extension worktrees\n.cmux/\n"
    ...
    private static func writeSampleDevServerFiles(in worktree: URL, projectName: String) throws {
        let sample = worktree.appendingPathComponent("zerocmux-sample-dev", isDirectory: true)
        ...
        <!doctype html>
        <html>
          <head><meta charset="utf-8"><title>zerocmux worktree</title></head>
