...
nonisolated private func v2RemotePTYUserFacingErrorMessage(_ message: String) -> String {
    ...
    if lowered.contains("missing required capability") ||
        ...
        return "remote daemon does not support persistent SSH PTY sessions; reconnect the remote workspace to update zerocmux"
    }
...
class TerminalController {
    ...
    @MainActor var localAgentSessionRegistry: AgentChatSessionRegistry?
    ...
    private nonisolated func socketWorkerV2Response(_ request: V2SocketRequest) -> String {
        ...
        case "auth.status":
            return v2Ok(id: request.id, result: [
                "signed_in": false,
                "available": false,
                "reason": AuthEnvironmentUnavailable.message,
            ])
        case "auth.sign_in_url":
            return v2Error(id: request.id, code: "unavailable", message: AuthEnvironmentUnavailable.message)
    ...
    private func v2LegacyMainActorResponse(
        ...
        // and auth.login handled by ControlCommandCoordinator.
        ...
        // zerocmux: workspace.cloud_vm_open / workspace.cloud_vm_terminal_ready are
        // not served — hosted Cloud VM methods were removed with the web backend.
    ...
    func v2Identify(params: [String: Any]) -> [String: Any] {
        ...
        if let cliPath = Bundle.main.resourceURL?
            ...
            .appendingPathComponent("zerocmux", isDirectory: false)
            .path {
    ...
    private func parseV2WindowRouting(params: [String: Any]) -> (routing: V2WindowRouting?, error: V2CallResult?) {
        ...
        if params["window_id"] != nil && requestedWindowId == nil {
            return (
                ...
                .err(
                    code: "invalid_params",
                    message: "Invalid window selector. Use --window <id|ref|index> to target one window, or run `zerocmux list-windows` to see available windows and retry.",
        ...
        if includeAllWindows, requestedWindowId != nil {
            return (
                ...
                .err(
                    code: "invalid_params",
                    message: "Choose either --window <id|ref|index> or --all-windows, not both. Run `zerocmux list-windows` to see available windows and retry.",
                    data: v2WindowSelectorDetails(params: params)
                )
            )
        }
    ...
    private func v2WindowNotFoundResult(params: [String: Any], windowId: UUID) -> V2CallResult {
        .err(
            code: "not_found",
            message: "Window not found. Run `zerocmux list-windows` to see available windows, then retry with --window <id|ref|index>.",
    ...
    nonisolated func v2VmCall(
        ...
        case .failure(let error):
            ...
            return v2Error(id: id, code: "internal_error", message: error.localizedDescription)
        case nil:
            return v2Error(
                id: id,
                code: "vm_error",
    ...
    /// CLI presentation concern (`zerocmux workspace env --mask`): the local control
    ...
    func v2BrowserOpenSplit(
        ...
        if let urlStr {
            ...
            if let navigable = resolveBrowserNavigableURL(urlStr) {
                ...
                // the trusted zerocmux-diff-viewer:// scheme, and external app/deep-link schemes
        ...
        case .evaluationFailed(let message):
            return .err(
                ...
                data: [
                    ...
                    "hint": "Verify the page loaded with 'zerocmux browser <surface> get url' before waiting"
                ]
            )
        }
    }
    ...
    private nonisolated func v2BrowserScreenshot(params: [String: Any]) -> V2CallResult {
        ...
        let screenshotsDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-browser-screenshots", isDirectory: true)
        ...
        if params.keys.contains("destination_profile") {
            ...
            if let uuid = UUID(uuidString: query),
               profiles.contains(where: { $0.id == uuid }) {
                ...
                return .err(
                    code: "invalid_params",
                    message: "destination_profile does not match a zerocmux browser profile",
                    data: ["param": "destination_profile"]
                )
            }
        } else {
    ...
    func panelSnapshot(_ args: String) -> String {
        ...
        let outputDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("zerocmux-screenshots")
    ...
    func orderedPanels(in tab: Workspace) -> [any Panel] {
        ...
        // ordering. This avoids relying on Dictionary iteration order and keeps
        // the serializer and the reorder gate consistent.
