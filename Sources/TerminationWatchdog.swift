...
/// Pastebot, …) is mid-read of zerocmux's promised clipboard data, that round-trip
/// can wedge for ~30s on the main thread until the OS force-kills the app
/// (https://github.com/kernelalex/zerocmux/issues/6758). The same structural gap —
/// quit having no global "return within N seconds no matter what" guard —
/// produced #6415 (a blocking flush in a will-terminate observer) and #6381
/// (`ghostty` lock).
///
...
/// wedged on exactly such a lock. zerocmux's critical session/state save runs
