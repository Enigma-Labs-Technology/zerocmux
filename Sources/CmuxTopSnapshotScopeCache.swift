...
private nonisolated struct CmuxTopProcessScopeCacheValue {
    // nil means "this process was probed and has no zerocmux scope". A negative entry
    // is honored as a hit only until `negativeExpiresAtNanos`, so a non-zerocmux
    ...
    // ignored when `scope != nil`): a zerocmux scope comes from inherited environment
...
// How long a "no zerocmux scope" result stays cached before the process is probed
...
// `zerocmux hooks … monitor` later, must be re-probed eventually or it would never
// be attributed. The TTL bounds that attribution latency while still collapsing
// the per-poll sysctl storm for the steady-state majority of non-zerocmux processes.
private nonisolated let cmuxTopNegativeScopeTTLNanoseconds: UInt64 = 15 * 1_000_000_000

// Result of probing a single process for its zerocmux scope. `resolved` means the
...
nonisolated extension CmuxTopProcessSnapshot {
    ...
    static func cachedCMUXScope(
        ...
        if let cached = cmuxTopScopeCache.withLock({ cache in cache[cacheKey] }) {
            if let scope = cached.scope {
                // Positive results never expire: a discovered zerocmux scope is stable.
                return scope
            }
            ...
            // execed into a zerocmux-scoped command since it was last sampled.
        }
    ...
    // Probes a single process for its zerocmux scope via sysctl.
    //
    ...
    // "no readable zerocmux scope" — those processes are not zerocmux-scoped and stay in
    ...
    private static func cmuxScopeFromEnvironment(_ environment: [String: String]) -> CmuxTopProcessScope? {
        ...
        return CmuxTopProcessScope(
            ...
            surfaceID: surfaceID,
            attributionReason: "zerocmux-environment"
        )
    }
    ...
    private static func cmuxHookMonitorScope(arguments: [String]) -> CmuxTopProcessScope? {
        ...
        return CmuxTopProcessScope(
            workspaceID: workspaceID,
            surfaceID: surfaceID,
            attributionReason: "zerocmux-hook-arguments"
        )
    }
    ...
    private static func containsSubcommandPath(_ path: [String], in arguments: [String]) -> Bool {
        ...
        let executableName = URL(fileURLWithPath: arguments[0])
            .lastPathComponent
            .lowercased()
        guard executableName == "zerocmux" else { return false }
