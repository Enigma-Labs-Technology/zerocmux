...
extension TerminalNotificationStore {
    ...
    /// Effects for the out-of-band fallback path, where zerocmux plays feedback
    /// itself because the OS will not deliver the banner.
    ///
    /// A user who explicitly turned zerocmux notifications off (`.denied`) asked
    /// for silence, so the direct `NSSound` fallback must not punch through
    /// the denial (https://github.com/kernelalex/zerocmux/issues/5650). Every
