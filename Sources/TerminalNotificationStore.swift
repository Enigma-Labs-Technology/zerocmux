...
extension UNUserNotificationCenter {
    private static let removalQueue = DispatchQueue(
        label: "com.kernelalex.zerocmux.notification-removal",
        qos: .utility
    )
...
final class TerminalNotificationStore: ObservableObject {
    ...
    static let categoryIdentifier = "com.kernelalex.zerocmux.userNotification"
    static let textReplyCategoryIdentifier = "com.kernelalex.zerocmux.userNotification.textReply"
    static let actionShowIdentifier = "com.kernelalex.zerocmux.userNotification.show"
    ...
    /// Superseded-banner dismissal bookkeeping (zerocmux: phone mirroring removed;
    /// the buffer still coalesces dismissals for local consistency), so
    ...
    private func emitNotificationsDismissed(ids: [String]) {
        ...
        // zerocmux: phone mirroring removed; dismissals only update local state.
    }
    ...
    private func emitUnreadBadgeEventIfChanged() {
        // zerocmux: phone mirroring removed; track the count so the seam stays cheap.
        ...
        ensureAuthorization(origin: .settingsTest) { [weak self] authorized, _ in
            ...
            Task { @MainActor [weak self, userNotificationCenter] in
                ...
                content.title = String(
                    ...
                    defaultValue: "zerocmux test notification"
                )
                content.body = String(
    ...
    private func deliverNotificationSideEffects(
        ...
        if shouldSuppressExternalDelivery {
            ...
            // zerocmux: phone mirroring removed. Flush any stale superseded-banner
            // ids immediately so local dismiss bookkeeping stays consistent.
            if effects.desktop {
    ...
    func reportNotificationHookFailure(_ failure: TerminalNotificationPolicyFailure) {
        ...
        ensureAuthorization(origin: .notificationDelivery) { [weak self] authorized, _ in
            ...
            let format = String(
                ...
                defaultValue: "zerocmux used default notification behavior because '%@' failed."
            )
            Task { @MainActor [weak self, userNotificationCenter] in
                let content = UNMutableNotificationContent()
                content.title = title
    ...
    private func presentNotificationSettingsPrompt(attempt: Int) {
        ...
        alert.messageText = String(localized: "dialog.enableNotifications.title", defaultValue: "Enable Notifications for zerocmux")
        alert.informativeText = String(localized: "dialog.enableNotifications.message", defaultValue: "Notifications are disabled for zerocmux. Enable them in System Settings to see alerts.")
