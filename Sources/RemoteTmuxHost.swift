...
/// Identifies a remote host whose tmux server zerocmux mirrors over SSH.
///
/// A host is addressed by its SSH `destination` — either a `~/.ssh/config`
/// alias (e.g. `claude-box`) or an explicit `user@host`. zerocmux multiplexes
...
struct RemoteTmuxHost: Sendable, Equatable, Identifiable {
    ...
    ///
    /// Intended to be run by the `zerocmux ssh-tmux` CLI **inside the user's terminal**
    ...
    static func tmuxRemoteCommand(arguments: [String]) -> String {
        (["/bin/sh", "-c", tmuxResolverShellScript, "zerocmux-remote-tmux"] + arguments)
