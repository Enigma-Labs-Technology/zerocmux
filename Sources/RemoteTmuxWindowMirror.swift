...
/// window, so a single zerocmux tab can render the tmux window's full multi-pane
/// split layout side by side — with the native zerocmux pane chrome (each pane is a
...
final class RemoteTmuxWindowMirror {
    ...
    /// Tells tmux to size this session's windows to the rendered zerocmux area, so
    ...
    /// the zerocmux area doesn't change when tmux reflows).
