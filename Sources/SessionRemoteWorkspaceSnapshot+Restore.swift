    ...
    private func moshReconnectCommand(
        ...
        return MoshTerminalCommandBuilder(
            ...
            remoteBootstrapInstallFailedMessage: String(
                ...
                defaultValue: "[zerocmux] Remote bootstrap install failed; continuing over SSH."
            ),
            remoteMoshAddressFallbackMessage: String(
                localized: "cli.ssh.mosh.addressFallback",
                defaultValue: "[zerocmux] Remote SSH advertised an unusable address; resolving the Mosh address through the SSH connection."
            )
        ).command()
    }
