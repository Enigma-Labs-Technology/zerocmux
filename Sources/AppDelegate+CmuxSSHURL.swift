    ...
    var errorDescription: String? {
        ...
        case .launchServicesRegistrationFailed:
            return String(
                ...
                defaultValue: "zerocmux could not register as the default terminal app."
            )
        }
    }
}
...
enum DefaultTerminalRegistration {
    ...
    static func currentStatus(
        ...
        let matchedURLSchemes = urlSchemes.filter { scheme in
            guard let url = URL(string: "\(scheme)://zerocmux-default-terminal-check") else {
                return false
            }
        ...
        guard let cliURL,
            ...
            presentLaunchFailure(
                summary: String(
                    ...
                    defaultValue: "The bundled zerocmux CLI is missing from this app build."
                ),
                output: "",
        ...
        process.terminationHandler = { [weak preferredWindow] terminatedProcess in
            ...
            Task { @MainActor in
                ...
                let format = String(
                    ...
                    defaultValue: "zerocmux ssh exited with status %d."
                )
        ...
        do {
            ...
            presentLaunchFailure(
                summary: String(
                    ...
                    defaultValue: "zerocmux ssh could not be launched."
                ),
...
final class ProcessOutputCollector {
    ...
    func start() {
        stdout.fileHandleForReading.readabilityHandler = { [weak self] handle in
            self?.appendAvailableData(from: handle)
        }
    ...
    func finish() -> String {
        cancel()
        appendAvailableData(from: stdout.fileHandleForReading)
        appendAvailableData(from: stderr.fileHandleForReading)
        lock.lock()
        let data = chunks.reduce(into: Data()) { combined, chunk in
...
extension AppDelegate {
    ...
    func claimAuthCallbackURLSchemes() {
        // Pin this build's SSH callback schemes so deeplinks route
        // back here instead of an unrelated LaunchServices entry.
        let bundleURL = Bundle.main.bundleURL
        let schemes = CmuxSSHURLRequest.supportedSchemes
    ...
    private func confirmCmuxTextURLRequest(_ request: CmuxTextURLRequest) -> Bool {
        ...
        let messageFormat = request.kind == .prompt
            ? String(
                ...
                defaultValue: "A %@:// link is asking zerocmux to paste a prompt into the current workspace. zerocmux cannot verify which website or app opened this link.\n\ncmux will paste the text into the terminal and will not press Return. Only continue if you trust this prompt."
            )
            : String(
                localized: "dialog.textURL.rules.message",
                defaultValue: "A %@:// link is asking zerocmux to paste rules into the current workspace. zerocmux cannot verify which website or app opened this link.\n\ncmux will paste the rules into the terminal and will not write files or press Return. Only continue if you trust these rules."
            )
        alert.informativeText = String(
    ...
    private func cmuxSSHURLAccessoryView(
        ...
        let checkbox = NSButton(
            checkboxWithTitle: String(
                ...
                defaultValue: "I trust this SSH target and want zerocmux to connect."
            ),
            target: gate,
    ...
    private func showCmuxSSHURLParseError(_ error: CmuxSSHURLParseError) {
        ...
        alert.messageText = String(
            ...
            defaultValue: "zerocmux SSH Link Blocked"
        )
    ...
    private func showCmuxTextURLPasteFailure(_ request: CmuxTextURLRequest) {
        ...
        alert.informativeText = String(
            ...
            defaultValue: "zerocmux could not send the link text to a terminal."
        )
        alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK"))
        alert.runModal()
    }
    ...
    private func showCmuxTextURLParseError(_ error: CmuxTextURLParseError) {
        ...
        alert.messageText = String(
            ...
            defaultValue: "zerocmux Link Blocked"
        )
    ...
    private func cmuxSSHURLParseErrorMessage(_ error: CmuxSSHURLParseError) -> String {
        ...
        case .destinationContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The SSH host or user contains unsupported or hidden characters, so zerocmux refused to use it."
            )
        ...
        case .titleContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The workspace title contains hidden control or formatting characters, so zerocmux refused to use it."
            )
        case .invalidPort:
            return String(
    ...
    private func cmuxTextURLParseErrorMessage(_ error: CmuxTextURLParseError) -> String {
        ...
        case .textContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The link text contains unsupported or hidden characters, so zerocmux refused to use it."
            )
        ...
        case .nameContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The link name contains hidden control or formatting characters, so zerocmux refused to use it."
            )
        case .titleTooLong(let maxLength):
            return String(
        ...
        case .titleContainsUnsafeCharacters:
            return String(
                ...
                defaultValue: "The link title contains hidden control or formatting characters, so zerocmux refused to use it."
            )
        case .invalidBooleanParameter(let parameter):
            return String(
        ...
        case .multipleLinks:
            return String(
                ...
                defaultValue: "Only one zerocmux external link can be opened at a time."
            )
        }
    }
}
