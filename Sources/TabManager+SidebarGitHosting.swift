...
extension TabManager: SidebarGitHosting {
    ...
    // zerocmux: the mobile host was removed, so background git/PR work is
    // never deferred for mobile traffic. The package seam still requires
    // these; report "never active" so polling always proceeds.
