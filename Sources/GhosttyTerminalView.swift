...
class GhosttyApp {
    ...
    //
    // CmuxTerminalEngine and CmuxTerminalServices ship singleton-free; zerocmux
    ...
    // zerocmux owns one process-lifetime GhosttyApp, so the registry avoids singleton re-entry
    ...
    private static func resolveBackgroundLogURL(
        ...
        return URL(fileURLWithPath: "/tmp/zerocmux-bg.log")
    }
    ...
    #if DEBUG
    private static let initLogPath = "/tmp/zerocmux-ghostty-init.log"
    ...
    private func initializeGhostty() {
        ...
        if let created = ghostty_app_new(&runtimeConfig, primaryConfig) {
            ...
            // zerocmux still launches with working terminals.
            ...
            loadInlineGhosttyConfig(
                ...
                into: fallbackConfig,
                prefix: "zerocmux-renderer-bg",
                logLabel: "renderer background (fallback)"
            )
            loadInlineGhosttyConfig(
                "macos-titlebar-proxy-icon = hidden",
                into: fallbackConfig,
                prefix: "zerocmux-titlebar-proxy-icon",
                logLabel: "titlebar proxy icon (fallback)"
            )
            loadInlineGhosttyConfig(
                "shell-integration = none",
                into: fallbackConfig,
                prefix: "zerocmux-shell-integration-override",
    ...
    private func loadCmuxDefaultAppearanceConfig(
        ...
        loadInlineGhosttyConfig(
            ...
            into: config,
            prefix: "zerocmux-default-appearance",
    ...
    private func loadCmuxManagedTerminalSettingsConfig(_ config: ghostty_config_t) {
        ...
        loadInlineGhosttyConfig(
            contents,
            into: config,
            prefix: "zerocmux-managed-terminal-settings",
    ...
    private func loadStartupPreviewProfile(
        ...
        loadInlineGhosttyConfig(
            contents,
            into: config,
            prefix: "zerocmux-startup-preview",
    ...
    private func loadConditionalThemeOverrideIfNeeded(
        ...
        loadInlineGhosttyConfig(
            contents,
            into: config,
            prefix: "zerocmux-conditional-theme",
    ...
    func loadDefaultConfigFilesWithLegacyFallback(
        ...
        // Let zerocmux own the window-level backdrop once, while Ghostty keeps
        ...
        // overriding NSWindow.representedURL on every zerocmux main window.
        loadInlineGhosttyConfig(
            "macos-titlebar-proxy-icon = hidden",
            into: config,
            prefix: "zerocmux-titlebar-proxy-icon",
        ...
        // Prevent Ghostty from overriding ZDOTDIR — zerocmux handles shell
    ...
    func loadGlobalFontMagnificationConfig(_ config: ghostty_config_t) {
        ...
        loadInlineGhosttyConfig(
            ...
            prefix: "zerocmux-global-font-magnification",
    ...
    private func loadNoActiveDisplayVsyncFallbackIfNeeded(_ config: ghostty_config_t) {
        ...
        loadInlineGhosttyConfig(
            "window-vsync = false",
            into: config,
            prefix: "zerocmux-no-active-display-vsync-fallback",
    ...
    /// hardcoded (https://github.com/kernelalex/zerocmux/issues/5189). Ghostty registers
    ...
    ///
    /// See: https://github.com/kernelalex/zerocmux/pull/1017
    private func loadCJKFontFallbackIfNeeded(_ config: ghostty_config_t) {
        ...
        loadInlineGhosttyConfig(
            lines,
            into: config,
            prefix: "zerocmux-cjk-font-fallback",
    ...
    /// Returns only the CJK mappings zerocmux should auto-inject. Forwards to
    ...
    /// Returns the top-level Ghostty config paths zerocmux may load before recursive
    ...
    func reloadConfiguration(
        ...
        // Use the appearance preference while loading conditional theme pairs. For zerocmux
    ...
    private func handleAction(target: ghostty_target_s, action: ghostty_action_s) -> Bool {
        ...
        if action.tag == GHOSTTY_ACTION_SHOW_CHILD_EXITED {
            // Ghostty otherwise prints "Process exited. Press any key..."; zerocmux closes the panel.
        ...
        case GHOSTTY_ACTION_OPEN_URL:
            ...
            // match path-like text as URLs; route existing local files through zerocmux first.
            ...
            #endif
            // Route local file paths into zerocmux when the file-routing toggle is on.
            ...
            case let .embeddedBrowser(url):
                ...
                // From here zerocmux owns the open attempt and the deferred path falls back externally.
    ...
    func logBackground(_ message: String) {
        ...
        // guard, direct callers wrote to /tmp/zerocmux-bg.log on every theme/OSC color
        ...
        // https://github.com/kernelalex/zerocmux/issues/5833.
...
class GhosttyNSView: NSView, NSUserInterfaceValidations {
    ...
    private func handleCommandClickRelease(
        ...
        // path on the main thread for a remote workspace. When the zerocmux route
