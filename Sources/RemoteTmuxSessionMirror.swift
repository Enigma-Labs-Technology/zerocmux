...
/// Mirrors one remote tmux session into a dedicated zerocmux sidebar workspace.
///
...
final class RemoteTmuxSessionMirror {
    ...
    /// (the reverse of the zerocmux→tmux `rename-session` push). Uses TabManager's
    ...
    /// The zerocmux workspace mirroring this session (if still alive).
    ...
    /// content-derived (like every other zerocmux tab title) so the name itself is
    ...
    /// Used to target a tmux paste at the pane behind a zerocmux surface.
