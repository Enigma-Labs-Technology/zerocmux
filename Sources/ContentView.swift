...
struct VerticalTabsSidebar: View {
    ...
    let windowId: UUID
    // zerocmux: no hosted feedback composer; the footer plumbing keeps the
    // upstream signature but the sidebar is constructed with a no-op.
    ...
    private func refreshWorkspaceSnapshots(workspaceIds: Set<UUID>) {
        ...
        let showsAgentActivity = showAgentActivity
