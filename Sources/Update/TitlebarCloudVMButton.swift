...
struct TitlebarCloudVMButton: View {
    ...
    // zerocmux: hosted Cloud VM services are removed from this fork, so the
    // Cloud VM menu never shows.
    private static let isCloudVMUIEnabled = false

    @MainActor
    static func showCloudVMMenu(anchorView: NSView, event: NSEvent) {
        guard Self.isCloudVMUIEnabled else { return }
