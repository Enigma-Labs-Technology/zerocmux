...
extension Workspace {
    ...
    private func closedPanelHistoryEntry(panelId: UUID, tabId: TabID, pane: PaneID) -> ClosedPanelHistoryEntry? {
        ...
        // zerocmux-launched agents reopen correctly regardless of cache freshness.
...
/// `zerocmux hooks` CLI writes when an agent session starts or updates. The cache reloads
...
final class SharedLiveAgentIndex: ObservableObject {
    ...
    private let watchQueue = DispatchQueue(label: "com.kernelalex.zerocmux.sharedLiveAgentIndexWatch")
    ...
    private func ensureWatchingHookStoreDirectory() {
        ...
        // session could stay invisible behind the fallback TTL. This is zerocmux's own state
        // directory (the `zerocmux hooks` CLI writes here too), so creating it empty is benign.
        ...
        // zerocmux's existing CmuxConfig watcher, which relies on the same atomic-write
...
final class Workspace: Identifiable, ObservableObject {
    ...
    private static func shouldRunPromptedSurfaceResumeOnMain(_ binding: SurfaceResumeBindingSnapshot) -> Bool {
        ...
        alert.informativeText = String(
            format: String(
                ...
                defaultValue: "zerocmux is restoring a terminal with this resume command:\n\n%@\n\nWorking directory: %@"
            ),
            binding.command,
    ...
    /// Ephemeral remote tmux mirror; excluded from zerocmux session restore.
    ...
    /// inside a single tab, so the pane gets the full native zerocmux pane chrome —
    ...
    /// a manual `move-window` / a `new-window` inserted mid-list). The zerocmux→tmux
    ...
    func reorderRemoteTmuxMirrorTabs(toPanelOrder panelOrder: [UUID]) -> Bool {
        ...
        // last-reordered tab). zerocmux's own focus/selection were never touched (the
    ...
    private static func remoteDisconnectPlaceholderScript(target: String, reconnectCommand: String?) -> String {
        ...
        let scriptURL = tempDir.appendingPathComponent(
            "zerocmux-remote-disconnect-\(UUID().uuidString.lowercased()).sh"
        )
        ...
        if [ -n "$cmux_disconnect_reconnect_command" ]; then
          ...
          if [ -z "$cmux_reconnect_cli" ] || [ ! -x "$cmux_reconnect_cli" ]; then
            cmux_reconnect_cli="$(command -v zerocmux 2>/dev/null || true)"
          fi
    ...
    private func installLayoutFollowUpObservers() {
        ...
        // structural observers below (https://github.com/kernelalex/zerocmux/issues/6790).
    ...
    /// the agent itself (Claude / Codex / OpenCode) instead of any zerocmux-side history copy.
    @discardableResult
