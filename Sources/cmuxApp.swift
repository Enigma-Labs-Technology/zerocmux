...
struct cmuxApp: App {
    ...
    init() {
        ...
        // apps" prompt; the password now lives in the non-protected zerocmux state
        // directory (https://github.com/kernelalex/zerocmux/issues/5146). The app
        ...
        // Secrets live in their own 0600 files under the zerocmux state directory,
        ...
        // share the name `zerocmux`, so if the GUI's Contents/MacOS leaks onto $PATH
        // (which happens for any shell descended from this process), bare `zerocmux`
        // resolves here instead of the CLI. See
        // https://github.com/kernelalex/zerocmux/issues/4678.
        // zerocmux ships a universal binary so it still supports Intel Macs, but a
        ...
        // launches. See https://github.com/kernelalex/zerocmux/issues/753.
        ...
        let localAgentSessions = AgentChatSessionRegistry()
        let tabManager = TabManager(
            workspaceCustomizationStore: workspaceCustomizationStore,
            nativeSSHConnectionBroker: TerminalController.shared.nativeSSHConnectionBroker,
            agentChatResumeIntentRecorder: AgentChatTranscriptResumeIntentRecorder(registry: localAgentSessions)
        )
    ...
    static func resolvedGhosttyResourcesDirectory(
        ...
        // Tagged zerocmux builds may inherit GHOSTTY_RESOURCES_DIR from another running
        // zerocmux instance. Prefer this app's bundled resources when they are present.
    ...
    var body: some Scene {
        ...
        .commands {
            CommandGroup(replacing: .appSettings) {
                ...
                Button(String(localized: "menu.app.makeDefaultTerminal", defaultValue: "Make zerocmux the Default Terminal")) {
...
enum BuildFlavor: String, Sendable {
    ...
    static func detect(bundleNames: [String], bundleIdentifier: String?) -> BuildFlavor {
        ...
        if normalizedBundleIdentifier == "com.kernelalex.zerocmux.nightly"
            || normalizedBundleIdentifier?.hasPrefix("com.kernelalex.zerocmux.nightly.") == true {
            return .nightly
        }
...
enum TelemetrySettings {
    // zerocmux: telemetry was removed from the fork entirely; this anchor is kept
    // only so legacy call sites read a constant `false` and can be deleted safely.
    static let enabledForCurrentLaunch = false
}
