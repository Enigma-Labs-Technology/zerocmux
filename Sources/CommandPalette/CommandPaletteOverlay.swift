            ...
            LazyVStack(spacing: 0) {
                if state.rows.isEmpty {
                    ...
                    ForEach(state.rows.indices, id: \.self) { index in
                        CommandPaletteCommandListRowView(
                            index: index,
...
private struct CommandPaletteCommandListRowView: View {
    let index: Int
    let row: CommandPaletteRenderResultRow
    let isSelected: Bool
    ...
    private var rowBackground: Color {
        if isSelected {
            return cmuxAccentColor().opacity(0.12)
        }
    ...
    var body: some View {
        Button {
            ...
            CommandPaletteRenderResultLabelContent(
                title: row.title,
...
private struct CommandPaletteRenderResultLabelContent: View {
    let title: String
    let matchedIndices: Set<Int>
    let trailingLabel: CommandPaletteRenderTrailingLabel?
    ...
    var body: some View {
        HStack(spacing: 8) {
            highlightedTitleText
                ...
                .lineLimit(1)
            Spacer()
            CommandPaletteRenderTrailingLabelView(trailingLabel: trailingLabel)
        }
    }
...
private struct CommandPaletteRenderTrailingLabelView: View {
    let trailingLabel: CommandPaletteRenderTrailingLabel?
