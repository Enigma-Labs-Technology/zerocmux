...
enum SidebarWorkspaceGroupConfigOpener {
    /// Opens the zerocmux config file (`~/.config/cmux/cmux.json`) in the user's
    ...
    /// Testable seam: resolves the zerocmux config path under `home`, materializes
    ...
    static func openWorkspaceGroupsDocs() {
        guard let url = URL(
            string: "https://github.com/kernelalex/zerocmux/blob/main/docs/workspace-groups.md"
        ) else { return }
