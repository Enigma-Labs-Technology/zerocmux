...
class TabManager: ObservableObject {
    ...
    @Published var selectedTabId: UUID? {
        ...
        didSet {
            ...
            diagnosticsBreadcrumb("workspace.switch", data: [
                "tabCount": tabs.count
            ])
    ...
    private let workspacePlacementDefaults: UserDefaults
#if DEBUG
    ...
    init(
        ...
        workspacePlacementDefaults: UserDefaults = .standard
    ) {
        self.workspacePlacementDefaults = workspacePlacementDefaults
        addWorkspace(
    ...
    func startSearch() -> Bool {
        ...
        guard let focusedBrowserPanel else { return false }
    ...
    func addWorkspace(
        ...
        return withExtendedLifetime((capturedTabs, sourceWorkspace)) {
            ...
            diagnosticsBreadcrumb("workspace.create", data: ["tabCount": nextTabCount])
        ...
        if let terminalPanel = workspace.focusedTerminalPanel,
           terminalPanel.surface.surface != nil {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                terminalPanel.sendText("zerocmux welcome\n")
            }
            return
        }
    ...
    func newTabInsertIndex(
        ...
        let placement = WorkspacePlacementSettings.effectivePlacement(
            placementOverride: placementOverride,
            defaults: workspacePlacementDefaults
        )
    ...
    func closeWorkspace(_ workspace: Workspace) {
        ...
        diagnosticsBreadcrumb("workspace.close", data: ["tabCount": tabs.count - 1])
    ...
    private func closePanelWithConfirmation(tab: Workspace, panelId: UUID) {
        ...
        if closesWorkspaceOnLastSurfaceShortcut {
            _ = closeWorkspaceFromCloseTabGesture(tab)
            return
        }

        let requiresConfirmation = tab.panelNeedsConfirmClose(panelId: panelId)
    ...
    func createSplit(tabId: UUID, surfaceId: UUID, direction: SplitDirection, focus: Bool = true) -> UUID? {
        ...
        tab.clearSplitZoom()
        diagnosticsBreadcrumb("split.create", data: ["direction": String(describing: direction)])
...
extension TabManager {
    ...
    func sessionSnapshot(
        ...
        let restorableTabs = tabs
            .filter { $0.isRestorableInSessionSnapshot }
