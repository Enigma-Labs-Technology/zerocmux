...
actor RemoteTmuxSSHTransport {
    ...
    /// (zerocmux treats "no server running" / "no sessions" as zero sessions, not
    ...
    /// runs independently of zerocmux, so the master is torn down even as the app exits
    ...
    /// the user an interactive `ssh` (run in their terminal by `zerocmux ssh-tmux`) that
    ...
    private static func runProcess(
        ...
        // the fast auth-failure exits the `zerocmux ssh-tmux` flow classifies.
        let exitCode: Int32
        do {
