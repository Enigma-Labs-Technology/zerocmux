...
extension MenuBarProfilingProgressWindowController {
    ...
    private func makePrivateSubmitInputs(email: String, note: String) throws -> (replyToFile: URL, noteFile: URL) {
        var createdURLs: [URL] = []
        do {
            let replyToFile = try writePrivateSubmitInput(prefix: "zerocmux-profile-reply-to", text: email)
            createdURLs.append(replyToFile)
            let noteFile = try writePrivateSubmitInput(prefix: "zerocmux-profile-note", text: note)
    ...
    private func runSubmitProcess(_ process: Process) {
        do {
            let outputLog = try makeTemporaryLogFile(prefix: "zerocmux-profile-submit-output")
            let errorLog = try makeTemporaryLogFile(prefix: "zerocmux-profile-submit-error")
