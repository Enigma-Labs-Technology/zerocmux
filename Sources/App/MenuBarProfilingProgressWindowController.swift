...
final class MenuBarProfilingProgressWindowController: NSWindowController {
    ...
    private init() {
        ...
        window.title = String(localized: "statusMenu.profiling.title", defaultValue: "Profiling zerocmux")
    ...
    func startProfiling(
        ...
        do {
            let outputLog = try makeTemporaryLogFile(prefix: "zerocmux-profile-output")
            let errorLog = try makeTemporaryLogFile(prefix: "zerocmux-profile-error")
    ...
    private func buildInterface() {
        ...
        titleLabel.stringValue = String(localized: "statusMenu.profiling.reviewTitle", defaultValue: "Send a zerocmux profile")
        ...
        permissionLabel.stringValue = String(
            ...
            defaultValue: "macOS may ask for administrator permission because Instruments samples the running zerocmux process."
        )
    ...
    private func parseOutputURL(from text: String) {
        for line in text.components(separatedBy: .newlines) {
            if let range = line.range(of: "zerocmux profiling capture written to ") {
