...
nonisolated private let rosettaRelaunchLogger = Logger(subsystem: "com.kernelalex.zerocmux", category: "RosettaRelaunch")

/// Launch-time self-heal that re-execs zerocmux natively when the process is
/// running translated under Rosetta on Apple Silicon.
///
/// zerocmux ships a universal (`x86_64 arm64`) binary so it keeps supporting Intel
...
/// `zsh`, and every tool launched in a zerocmux terminal — runs translated. macOS 26
...
enum RosettaNativeRelaunch {
    ...
    /// Re-exec zerocmux as a native arm64 process when it is running translated.
    ///
