...
final class CmuxMainWindow: NSWindow {
    ...
    /// zerocmux creates its main window programmatically (never from a nib), so it
    ...
    /// `.fullScreenNone`) so a zerocmux main window can always enter a native
    ...
    /// zerocmux owns main-window placement: it persists and restores window frames
    ...
    /// arrangements. Because zerocmux never re-asserts the saved frame after wake,
