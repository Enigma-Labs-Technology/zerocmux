...
nonisolated private let cliForwardingLogger = Logger(subsystem: "com.kernelalex.zerocmux", category: "CLIForwarding")
...
enum CLIForwardingLaunchRouter {
    ...
    /// `Contents/Resources/bin/zerocmux` and never return. macOS-launch arguments
    /// (`-psn_...`, other `-` flags) and `zerocmux://` URLs are left to the GUI.
    static func forwardToBundledCLIIfNeeded(
        ...
        guard let cliURL = bundledCLIURL(bundle: bundle, fileManager: fileManager) else {
            #if DEBUG
            let resourcePath = bundle.resourceURL?.appendingPathComponent("bin/zerocmux").path ?? "<missing>"
    ...
    static func bundledCLIURL(
        ...
        let bundleCandidate = bundle.resourceURL?.appendingPathComponent("bin/zerocmux")
        ...
        let executableCandidate = resourcesURL.appendingPathComponent("bin/zerocmux")
    ...
    private static func localizedMissingBundledCLIError() -> String {
        String(
            ...
            defaultValue: "zerocmux could not run this command from the app bundle. Reinstall zerocmux or run the command from a standard zerocmux CLI installation."
        )
    }
    ...
    private static func localizedArgumentAllocationError() -> String {
        String(
            ...
            defaultValue: "zerocmux could not start this command. Try again, or reinstall zerocmux if the problem continues."
        )
    }
    ...
    private static func localizedExecFailureError() -> String {
        String(
            ...
            defaultValue: "zerocmux could not start the command-line tool from the app bundle. Reinstall zerocmux or run the command from a standard zerocmux CLI installation."
        )
    }
