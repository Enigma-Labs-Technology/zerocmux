...
final class SettingsWindowPresenter: NSObject {
    ...
    /// (https://github.com/kernelalex/zerocmux/issues/5770) become attributable from
    /// `log show --predicate 'subsystem == "com.kernelalex.zerocmux" && category == "Settings"'`.
    /// Internal (not private) so the geometry/recovery extension file logs
    /// through the same channel.
    nonisolated static let log = Logger(subsystem: "com.kernelalex.zerocmux", category: "Settings")
    ...
    /// (https://github.com/kernelalex/zerocmux/issues/5081).
