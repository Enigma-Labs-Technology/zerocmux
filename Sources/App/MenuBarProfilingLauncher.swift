...
nonisolated private let menuBarProfilingLogger = Logger(subsystem: "com.kernelalex.zerocmux", category: "MenuBarProfiling")
...
enum MenuBarProfilingLauncher {
    ...
    static func bundledScriptURL(bundle: Bundle = .main) -> URL? {
        bundle.url(forResource: "start-zerocmux-profiling", withExtension: nil, subdirectory: "bin")
    }

    static func bundledSubmitterURL(bundle: Bundle = .main) -> URL? {
        bundle.url(forResource: "submit-zerocmux-profile", withExtension: nil, subdirectory: "bin")
    }
    ...
    static func start(
        ...
        do {
            try process.run()
            menuBarProfilingLogger.notice("Started zerocmux profiling for pid \(pid)")
            return true
        } catch {
            menuBarProfilingLogger.error("Failed to start zerocmux profiling for pid \(pid): \(error.localizedDescription, privacy: .public)")
            NSSound.beep()
            return false
        }
    }
