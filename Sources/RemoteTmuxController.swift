...
/// Coordinates zerocmux's mirroring of remote tmux servers.
///
...
final class RemoteTmuxController {
    ...
    nonisolated static let logger = Logger(subsystem: "com.kernelalex.zerocmux", category: "RemoteTmux")
    ...
    /// the "one zerocmux window per remote endpoint" mirror mode (Option 1), owned by
    ...
    /// Opens a NEW zerocmux window dedicated to `host` and mirrors every tmux session
    ...
    ///   the caller (the `zerocmux ssh-tmux` CLI) runs `sshArgv` in the user's terminal to
    ...
    func mirrorHostInNewWindow(
        ...
        // the interactive `ssh` argv so the `zerocmux ssh-tmux` CLI authenticates in the
    ...
    ///
    /// `placement` mirrors zerocmux's `newTabPosition` for the workspace tab strip so
    ...
    ///   inherits the active tab's directory the way local zerocmux does. A
    ...
    /// - id → `new-window -a -t @id`: insert right after that window. zerocmux never
    ...
    /// tmux session name tracks the zerocmux workspace title.
    ...
    func handleMirrorSessionNameChanged(
        ...
        // Reverse of the zerocmux→tmux rename push: a remote `rename-session` (or an
    ...
    /// mirror workspace — causing zerocmux to auto-seed a stray local terminal tab.
    ...
    func handleMirrorWindowsReordered(workspaceId: UUID, orderedPanelIds: [UUID]) {
        ...
        // `swap-window` changes window indices but emits no notification zerocmux
    ...
    /// (bounded by `timeout`) so the session is gone before zerocmux exits. No
    ...
    func handleWorkspaceClosed(workspaceId: UUID) {
        ...
        Task {
            ...
            if isLastSession {
                ...
                // round-trip (a concurrent `zerocmux ssh-tmux` rebuilds on the same
    ...
    /// ControlMasters, so quitting zerocmux closes the ssh connections it opened (the
    ...
    func detachAll() {
        ...
        // socket and runs independently of zerocmux, so the masters are torn down even as
