...
extension TerminalController {
    ...
    /// `remote.tmux.window` — open a dedicated zerocmux window mirroring every tmux
    /// session on a host (the `zerocmux ssh-tmux` CLI entry point).
    ///
    ...
    /// authentication. zerocmux's control client uses plain pipes and cannot prompt,
    ...
    ///
    /// Diagnostics surface for verifying the ghostty → zerocmux event pipe end to end.
