...
/// Returns whether `window` is one of zerocmux's main windows.
///
...
/// Returns the visible main zerocmux window best suited to host a modal sheet.
///
...
/// when no main zerocmux window is currently on screen, in which case callers
/// should fall back to an app-modal presentation.
///
/// - Parameter preferredWindow: A window to consider ahead of the
///   key/main/any search, used when it is visible and a zerocmux main window
...
/// alert as a sheet attached to the main zerocmux window when one is available,
/// falling back to an app-modal `runModal()` only when there is no eligible
/// host window.
///
/// - Parameters:
///   - alert: The configured alert to present.
///   - presentingWindow: An explicit host window. When `nil`, the main zerocmux
