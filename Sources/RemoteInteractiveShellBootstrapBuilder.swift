...
enum RemoteInteractiveShellBootstrapBuilder {
    static func script(
        ...
        zshShellLines.append(
            #"if [ "${CMUX_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${CMUX_SHELL_INTEGRATION_DIR}/zerocmux-zsh-integration.zsh" ]; then . "${CMUX_SHELL_INTEGRATION_DIR}/zerocmux-zsh-integration.zsh"; fi"#
        )
        var bashShellLines = commonShellExportLines
        bashShellLines.append(
            #"if [ "${CMUX_SHELL_INTEGRATION:-1}" != "0" ] && [ -r "${CMUX_SHELL_INTEGRATION_DIR}/zerocmux-bash-integration.bash" ]; then . "${CMUX_SHELL_INTEGRATION_DIR}/zerocmux-bash-integration.bash"; fi"#
        )
        ...
        if let bundledZshIntegration {
            outerLines += [
                "cat > \"$cmux_shell_dir/zerocmux-zsh-integration.zsh\" <<'CMUXCMUXZSH'",
        ...
        if let bundledBashIntegration {
            outerLines += [
                "cat > \"$cmux_shell_dir/zerocmux-bash-integration.bash\" <<'CMUXCMUXBASH'",
    ...
    static func terminalSetupLines(terminfoSource: String?) -> [String] {
        ...
        // in another zerocmux ssh session sharing $HOME never observes a partially
