...
///    `.combinedSessionState` — so synthetic events (zerocmux agents driving the
...
extension MacPresenceMonitor {
    ...
    /// live monitor is owned and evaluated by its main-actor caller;
    /// `liveConsoleSessionActiveAndUnlocked` asserts that isolation
    ...
    private static func liveConsoleSessionActiveAndUnlocked() -> Bool {
        ...
        // owned by a @MainActor caller); assumeIsolated makes
