# vim:ft=zsh
#
# zerocmux ZDOTDIR bootstrap for zsh.
#
# GhosttyKit already uses a ZDOTDIR injection mechanism for zsh (setting ZDOTDIR
# to Ghostty's integration dir). zerocmux also needs to run its integration, but
...
# - CMUX_ZSH_ZDOTDIR (set by zerocmux when it overwrote a user-provided ZDOTDIR)
...

{
    ...
    if [[ -o interactive \
       ...
       && -r "${CMUX_SHELL_INTEGRATION_DIR}/zerocmux-zsh-integration.zsh" \
       && "${TERM:-}" == "xterm-256color" \
    ...
    if [[ -o interactive ]]; then
        ...
        # bootstrap unsets it before chaining into this zerocmux wrapper.
        ...
        # Load zerocmux integration (unless disabled)
        if [[ "${CMUX_SHELL_INTEGRATION:-1}" != "0" && -n "${CMUX_SHELL_INTEGRATION_DIR:-}" ]]; then
            builtin typeset _cmux_integ="$CMUX_SHELL_INTEGRATION_DIR/zerocmux-zsh-integration.zsh"
