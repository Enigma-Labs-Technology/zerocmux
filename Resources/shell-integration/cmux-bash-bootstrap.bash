# zerocmux bash prompt bootstrap.
#
# macOS ships /bin/bash 3.2, where Ghostty's automatic bash integration is
# unsupported and HOME-based wrapper startup is not reliable. zerocmux instead
# exports this script as PROMPT_COMMAND so it runs once on the first interactive
# prompt: it sources zerocmux's bash integration and then hands control to
...
# after the first one (https://github.com/kernelalex/zerocmux/issues/5164).
#
...
# let zerocmux-bash-integration.bash's PROMPT_COMMAND merge prepend _cmux_prompt_command.
#
...
if [[ "${CMUX_SHELL_INTEGRATION:-1}" != "0" && -n "${CMUX_SHELL_INTEGRATION_DIR:-}" ]]; then
    _cmux_bash_integration="$CMUX_SHELL_INTEGRATION_DIR/zerocmux-bash-integration.bash"
