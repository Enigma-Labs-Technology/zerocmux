# zerocmux shell integration for fish
...
if test "$_cmux_integration_enabled" != 0
    ...
    function _cmux_install_cli_command_shim --argument-names command_name wrapper_path
        ...
        if test -z "$shim_root"; or not string match -q "*/zerocmux-cli-shims/$surface_component" -- "$shim_root"
            set -l tmp_root /tmp
            if set -q TMPDIR; and test -n "$TMPDIR"
                set tmp_root "$TMPDIR"
            end
            set shim_root "$tmp_root/zerocmux-cli-shims/$surface_component"
        end
    ...
    _cmux_install_cli_wrapper claude zerocmux-claude-wrapper
